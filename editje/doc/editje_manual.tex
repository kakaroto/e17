...
group starts with just one group, with no parts and with a default
size of 480x800 pixels. Group templates will come as an option to
start new files, shortly.
...
%TODO: bad pic! we gotta fix the centering issue asap
\begin{figure}[h!]
  \centering
...
naturally, will remove that group from the file's group collection.
...
editing the fresh new group. This group will also be 480x800 sized and
have no parts.

\section{Editje's Window}

Once you open a file, you'll reach Editje's main window, which is depicted
on figure \ref{fig:main_window}.

%Notes: - program used to draw the overlaying objects: Kolourpaint
%       - font used to overlay numbers: Free Sans, 48
\begin{figure}[h!]
  \centering
...
\begin{description}
\item[1 -- Actions on edje (\texttt{.edc} and \texttt{.edj} files)]
  ...
  The \GUIIcon{``Swap''} one will raise the same box you get when
  calling Editje with just a file name argument -- the group selection
  box. It will list all the groups in the current file, and if you
  select a different one, the edje editor will load it and close the
  one which was under edition before. Note that, at this stage, it
  \emph{won't} ask the user if he or she wants to save the
  modifications on the group being left, so make sure you have them
  saved before switching groups.
...
\item[2 -- File and group names] On the top of this area you'll see
  the full path to the file being edited. On the bottom, after the
  label \GUILabel{``Group:''}, you see the name of the group under
  edition. You may change the group's name by editing it directly:
  it's a text entry. Just hit the return key after you're done and it
  will be applied.
...
\item[3 -- Mode dependent actions] This region of the main screen is
  ...
  (``Parts'') mode, this region presents an icon that will lead the
...
\item[5 -- Parts list] This is, while in ``Parts'' mode, an area which
...
\item[6 -- The group displaying (and manipulation) area] This is where
...
\item[7 -- Group and selected part properties] This is the region on
...
wish. The ``External Widget'' type is special: it's a meta type which
will let you select objects of external modules (exporting edje
objects) as parts of your interface. Currently, the only module
supported is Elementary, which has some of its widgets with properties
externalized to edje-edition.
...
one lists the actual widgets for the module selected on the left. For
now, as said before, we have just the Elementary widget library
exporting widgets this way. Its entry is called \GUIButton{``elm''},
as seen in figure \ref{fig:external_modules}.

\begin{figure}[h!]
  \centering
  \includegraphics[width=1.0\textwidth]{images/external_modules.png}
  \caption{``New Part'' dialog: external widgets}
  \label{fig:external_modules}
\end{figure}
...
%TODO: edit pic and make region 2's rectangle go till the bottom
\begin{figure}[h!]
  \centering
...
Screen regions of interest have been numbered, in that figure. If you
look at region 2, the parts list, you'll see it populated with our new
part.

%FIXME: the fscking \times, instead of 'x' (not working)
The region with number 3 is the actual new rectangle object. By
default, Editje will put it centered in the group's region, with an
initial size of 151x151, as seen on region 6, the ``part state''
collapsable property group (check out the \GUILabel{``current''} and
\GUILabel{``align''} labels). Note that all the collapsable property
groups are expanded on figure \ref{fig:new_rectangle}.

Note, also, the eight black squares over \texttt{Rectangle00}'s
corners. They, together with the fact that \texttt{Rectangle00} is
...
\ref{fig:rectangle_grow_left}, the squared marked in red has been
dragged to the right, enlarging \texttt{Rectangle00} to that direction
...
why you see \texttt{Rectangle00} now larger in width and shorter in
height.
...
\texttt{Rectangle00} has been moved to a different position. As you
...
The parts list, region 3 of figure \ref{fig:main_window}, will exhibit
three more action buttons, over the actual list of parts, besides the
\GUIIcon{``+''} one (see region 1 in figure
\ref{fig:new_rectangle}. Their actions are:
...
\item[\GUIIcon{downwards arrow}] This is the re-stack \emph{below}
  ...
  naturally. If you click on it with \texttt{Rectangle00} selected,
  being it on the state shown in figure \ref{fig:rectangle_move},
  you'll get what is depicted on figure \ref{fig:restack}.
\item[\GUIIcon{``x''}] This is the part removal action. When clicked,
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.5\textwidth]{images/rectangle_stack.png}
...
labels which give you the name of the selected part and its
type. Additionally, it has a \GUIEditable{``clip to''} field, which,
when clicked, will raise a box with a list of clipping objects
(rectangles) to choose from.

For example, let's add another rectangle besides \texttt{Rectangle00},
named \texttt{Rectangle01}. Place them so that they overlap, like in
figure \ref{fig:overlap}.

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.5\textwidth]{images/rectangle_overlap.png}
  \caption{Overlapping objects}
  \label{fig:overlap}
\end{figure}

Now, we'll have to advance a little bit and describe one entry of the
part state properties group, namely the \GUIEditable{``color''}
one. With \texttt{Rectangle01} selected, expand the ``part state''
collapsable and look at the color label. It contains a four integer
numbers text entry and a little rectangular box, which itself is
colored with the same color as the one the selected object has.
...
and hitting the return key. Another easier way is by clicking over the
colored box besides it. A floating box will appear, just as depicted
in figure \ref{fig:color_box}.
...
color changed, actually.

We're describing this property's use in order to set
\texttt{Rectangle01}'s color to a default clipper color: 255 255 255
255. After doing that, you'll get something like figure
\ref{fig:overlap_white}.
...
\texttt{Rectangle00} and click on this field. Another floating box
...
selected object. We'll choose \texttt{Rectangle01}, here, and the
effect should be something like figure \ref{fig:overlap_clipped}:
\texttt{Rectangle00} has only the area overlapping
\texttt{Rectangle01} colored green. The rest is shown transparent and
what you see is actually the \texttt{background} rectangle below it.
...
If you call, to \texttt{Rectangle00}, the clipper selection box again
and click on the \GUIButton{``None''} button, you'll get again to the
state we were in figure \ref{fig:overlap_white}.
...
Let's now go into the details of the region 6, seen in figure
...
Region 2, in that same figure, has general properties, like
...
filled for the two regions above, 3 and 4. These two regions denote,
...
\texttt{Rectangle00}, which coincide with two of the part resizing
squares.
...
Properties seem inside regions 2, 3 and 4 have the exact meaning of
the homonymous building blocks found in \texttt{.edc} files.  Their
meanings should be straightforward to readers with a minimal prior
knowledge of Edje Data Collections. If you're not familiar with it,
the document ``The Enlightenment Foundation Libraries -- A Big
Picture'' gives a good introduction on them. For the ones willing to
skip it, we quote that text's part on object relative positioning on
appendix \ref{app:edje_pos}.

%% Figure \ref{fig:part_state_group} is an enlargement of the top-left
%% and bottom-right properties seen on figure
%% \ref{fig:rectangle_rel1rel2}.
...
%% What follows is a series of figures
%% illustrating to effect one gets by modifying some of them.

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.88\textwidth]{images/visibility_false.png}
  \caption{Making \texttt{Rectangle00} invisible for state
    \texttt{default 0.0}}
...
\begin{figure}[h!]
  ...
  \caption{Changing \texttt{Rectangle00}'s positioning relative to its
    group}
...
\begin{figure}[h!]
  ...
  \caption{Now making \texttt{Rectangle00} relative to another one,
    \texttt{Rectangle01}, whose position can be seen on the group
    displaying area.}
...
\begin{figure}[h!]
  ...
  \caption{This is what one gets by just grabbing \texttt{Rectangle01}
    from the position it had in figure
    \ref{fig:relative_to_other_part} to the one shown here. Because
    \texttt{Rectangle00} is positioned relatively to it, it is also
...
\ref{fig:part_state_group}, has a region which wasn't cited yet: 5. If
...
\emph{new state} to \texttt{Rectangle00}, being the default one the
...
described, then. Other types of parts have extra properties (which
reflect on extra GUI entries in Editje) which haven't been discussed,
yet.
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=1.0\textwidth]{images/text_effect.png}
...
\item[\GUIEditable{``font''}:] The name of the (system) font to use.
\item[\GUIEditable{``size''}:] The text size.
\item[\GUIEditable{``fit''}:] When any of these entries is set to
  ``True'', Editje will resize the text for it to fit in it's
  container, in the respective axis. Both are disabled by default.
\item[\GUIEditable{``align''}:] Changes the alignment of the actual
  text inside the part's container, ranging from 0 to 1.
...
\begin{figure}[h!]
  ...
  \caption{Text part with ``OUTLINE SOFT SHADOW'' effect, customized
    ...
    finally, customized ``change2'' and ``change3'' colors.  }
  \label{fig:text_misc}
\end{figure}
...
made available to edje-edition as common part objects. As previously
said, the screen you're going to be at when adding them to your group
is the one seen in figure \ref{fig:external_modules}.
...
in Editje is to add as many of them as needed to your group, assign a
value to each of them and, then, group them together.
...
%% These are all text entries and, in order to modify them, just enter
%% the new values and hit the return key.
%------------------------------
...
some are greyed out. The black ones are existing frames in the
