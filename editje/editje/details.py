...
class EditjeDetails(elementary.Layout):
    ...
    def _aux_geometry_parse_in(self, param):
        return ("%dx%d" % param, )

    def _aux_geometry_parse_out(self, param):
        try:
            w, h = param.replace(" ", "x").split("x")
            if w == '':
                w = 0
            else:
                w = int(w)
    ...
    def _aux_parse_int_out(self, param):
        try:
            return int(param)
        except Exception, e:
            return None

    def _aux_parse_float_out(self, param):
        try:
            return float(param)
        except Exception, e:
            return None

    def _aux_splitted_parse_in(self, param):
        return map(str, param)

    def _aux_splitted_parse_int_out(self, *param):
        try:
            ret = map(int, param)
        except Exception, e:
            return None
    ...
    def _aux_splitted_parse_float_out(self, *param):
        try:
            ret = map(float, param)
        except Exception, e:
            return None
