...
libensure_init(void)
{
   ...
   if (p)
     {
        if (verbose)
          fprintf(stderr, "ENSURE_FD=%s", p);
        sscanf(p, "%d:%d", &sendfd, &commandfd);
        if (verbose)
          fprintf(stderr, "LibEnsure: Send: %d Command %d!\n", sendfd, commandfd);
   ...
   ecore_main_fd_handler_add(commandfd, ECORE_FD_READ | ECORE_FD_ERROR, libensure_command_recv, NULL, NULL, NULL);

   if (!outfile)
     {
        if (verbose)
          fprintf(stderr, "LibEnsure: Warning using stdout\n");
        outfile = stdout;
     }
   fprintf(outfile, "ensure init\n");
   if (verbose)
     fprintf(stderr, "LibEnsure: Init done\n");
...
libensure_dump(void)
{
   ...
   if (verbose)
     fprintf(stderr, "Starting ensure dump\n");
   ...
   EINA_LIST_FOREACH (ees, l, ee)
     {
        ...
        if (!name)
          name = "";
        if (verbose)
          fprintf(stderr, "E: %p '%s' %d %d\n", e, name, w, h);
        fprintf(outfile, "E: %p '%s' %d %d\n", e, name, w, h);
        os = evas_objects_in_rectangle_get(e, SHRT_MIN, SHRT_MIN, USHRT_MAX, USHRT_MAX, true, true);
...
//      if (type) fprintf(outfile, "Name: '%s' ",type);
   ...
   if (parent)
     fprintf(outfile, "Parent: %p ", parent);
   evas_object_geometry_get(o, &x, &y, &w, &h);
   fprintf(outfile, "Geometry: %+d%+d(%dx%d) ", x, y, w, h);
   clip = evas_object_clip_get(o);
   if (clip)
     fprintf(outfile, "Clip: %p ", parent);
   ...
   if (strcmp(type, "image") == 0)
     {
        ...
        if (err != EVAS_LOAD_ERROR_NONE)
          {
             fprintf(outfile, "ImageErr: '%s' ", evas_load_error_str(err));
          }
   ...
   else if (strcmp(type, "text") == 0)
     {
        ...
        if (font)
          fprintf(outfile, "FontSource: '%s' %d ", font, size);
     }
   ...
   else if (strcmp(type, "edje") == 0)
     {
        ...
        if (err != EDJE_LOAD_ERROR_NONE)
          {
             fprintf(outfile, "EdjeErr: '%s' ", edje_load_error_str(err));
          }
     }
...
libensure_highlight(uintptr_t addr)
{
   ...
   evas_object_color_set(r, HIGHLIGHT_R, HIGHLIGHT_G, HIGHLIGHT_B, HIGHLIGHT_A);
   evas_object_show(r);
