...
static void        add_heading(Evas_Object *win, Evas_Object *tbl, int *pos, const char *label);
static void        add_text(Evas_Object *win, Evas_Object *tbl, int *pos, const char *label, const char *value);
static void        add_id(struct ensure *ensure, Evas_Object *win, Evas_Object *tbl, int *pos, const char *label, uintptr_t id, bool viewable);
static void        add_fmt(Evas_Object *win, Evas_Object *tbl, int *pos, const char *label, const char *fmt, ...);
static const char *sev_string(enum ensure_severity sev);

static void        enobj_del(void *enobjv, Evas_Object *obj, void *event_info);
static void        display_hidden_object_toggle(void *enobjv, Evas_Object *obj, void *event __UNUSED__);
...
void
display_enobj_cb(void *enobjv, Evas_Object *obj __UNUSED__, void *event __UNUSED__)
{
   ...
   if (enobj->name)
     add_text(win, tbl, &pos, "Name:", enobj->name);
   add_id(ensure, win, tbl, &pos, "ID:", enobj->id, false);
   add_text(win, tbl, &pos, "Type:", enobj->type);
   if (enobj->parent)
     add_id(ensure, win, tbl, &pos, "Parent:", enobj->parent, true);
   if (enobj->clip)
     add_id(ensure, win, tbl, &pos, "Clip:", enobj->clip, true);
   add_fmt(win, tbl, &pos, "Position:", "%+d%+d", enobj->x, enobj->y);
   add_fmt(win, tbl, &pos, "Size:", "(%dx%d)", enobj->w, enobj->h);
   add_fmt(win, tbl, &pos, "Colour (rgba):", "argb(%d,%d,%d,%d)", enobj->a, enobj->r, enobj->g, enobj->b);
   add_fmt(win, tbl, &pos, "Colour (#hex):", "%02.2x%02.2x%02.2x%02.2x", enobj->a, enobj->r, enobj->g, enobj->b);

   if (strcmp(enobj->type, "text") == 0)
     {
        add_heading(win, tbl, &pos, "Text");
        add_text(win, tbl, &pos, "String:", enobj->data.text.text ? : "<em>Empty</em>");
        add_text(win, tbl, &pos, "Font:", enobj->data.text.font);
        if (enobj->data.text.source)
          add_text(win, tbl, &pos, "Source:", enobj->data.text.source);
        add_fmt(win, tbl, &pos, "Size", "%d", enobj->data.text.size);
     }
   else if (strcmp(enobj->type, "image") == 0)
     {
        add_heading(win, tbl, &pos, "Image");
        add_text(win, tbl, &pos, "File:", enobj->data.image.file ? : "<em>None</em>");
             ...
             if (!enobj->data.image.file)
             ...
             else
               {
                  ...
                  if (streq(enobj->data.image.file + len, ".edj"))
                    {
                       enobj->data.image.edjefile = enedj_get_imagename(enobj->data.image.file, enobj->data.image.key);
                       enobj->data.image.edjemember = EDJEMEMBER_TRUE;
                    }
                  else
                    {
                       enobj->data.image.edjemember = EDJEMEMBER_FALSE;
        ...
        if (enobj->data.image.edjefile)
          {
             add_text(win, tbl, &pos, "Original:", enobj->data.image.edjefile);
          }
     }
   else if (strcmp(enobj->type, "edje") == 0)
     {
        add_heading(win, tbl, &pos, "Edje Object");
        add_text(win, tbl, &pos, "File:", enobj->data.edje.file ? : "<em>None</em>");
   ...
   if (enobj->bugs)
     {
        ...
        EINA_LIST_FOREACH (enobj->bugs, l, bug)
          {
             add_text(win, tbl, &pos, sev_string(bug->severity), bug->desc);
          }
     }
   ...
   evas_object_smart_callback_add(tgl, "changed", display_hidden_object_toggle, enobj);
   pos++;
   ...
   evas_object_smart_callback_add(but, "clicked", display_show_object, enobj);
   pos++;
...
static void
add_text(Evas_Object *win, Evas_Object *tbl, int *pos, const char *label, const char *value)
{
   Evas_Object *l;
   char *b;
   int len;
...
static void
add_id(struct ensure *ensure, Evas_Object *win, Evas_Object *tbl, int *pos, const char *label, uintptr_t id, bool viewable)
{
   char buf[30];
   Evas_Object *but;
   struct enobj *enobj;
   ...
   if (viewable)
     {
        ...
        elm_table_pack(tbl, but, 2, *pos, 1, 1);
        evas_object_show(but);
        evas_object_smart_callback_add(but, "clicked", display_enobj_cb, enobj);
     }
...
static void
add_fmt(Evas_Object *win, Evas_Object *tbl, int *pos, const char *label, const char *fmt, ...)
{
   va_list ap;
   char *buf;
   int len;
...
sev_string(enum ensure_severity sev)
{
   switch (sev)
     {
      case ENSURE_CRITICAL:
        return "<font color='green'>Critical:</font>";

      case ENSURE_BUG:
        return "<font color='green'>Bug</font>";

      case ENSURE_BADFORM:
        return "Bad Form"; //"<font color='green'>Bad Form</font>";

      case ENSURE_PEDANTIC:
        return "Pedantic"; //"<font color='blue'>Pedantic</font>";

      case ENSURE_POLICY:
        return "Black"; //"<font color='black'>Policy</font>";
...
static void
display_hidden_object_toggle(void *enobjv, Evas_Object *obj, void *event __UNUSED__)
{
   struct enobj *enobj = enobjv;
   bool visible;
...
display_show_object(void *enobjv, Evas_Object *obj __UNUSED__, void *event __UNUSED__)
{
   ...
   printf("Sending %s\n", buf);
   if (write(ensure->commandfd, buf, n) != n)
     perror("write");
   printf("Done\n");
}
