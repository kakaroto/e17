...
object_check(struct ensure *en, struct enobj *obj, void *data __UNUSED__)
{
   ...
   if (!clipp)
     {
        ensure_bug(obj, assurance.severity, "Not smart member", "Object's clip not smart member");
        return 1;
     }
   else if (clipp != parent)
     {
        ensure_bug(obj, assurance.severity, "Clip and member differ", "Object's clip member of difference smart " "Obj: %" PRIuPTR "  Clip: %" PRIuPTR "", parent->id, clip->id);
        return 1;
     }
