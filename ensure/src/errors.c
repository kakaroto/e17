...
void
errors_view_set(void *ensurev, Evas_Object *button __UNUSED__, void *event_info __UNUSED__)
{
   struct ensure *ensure = ensurev;

   if (ensure->current_view == ENVIEW_ERROR)
     return;
 ...
 */
static void
enobj_select(void *data __UNUSED__, Evas_Object *obj __UNUSED__, void *itemv)
{
...
enobj_label_get(void *data, Evas_Object *obj __UNUSED__, const char *part __UNUSED__)
{
   ...
   if (enobj->name)
     {
        snprintf(buf, sizeof(buf), "%s (%" PRIuPTR ") - %s", enobj->name, enobj->id, enobj->type);
     }
   else
     {
        snprintf(buf, sizeof(buf), "%" PRIuPTR " - %s", enobj->id, enobj->type);
     }
   return strdup(buf);
}
...
static Eina_Bool
enobj_state_get(void *data __UNUSED__, Evas_Object *obj __UNUSED__, const char *part __UNUSED__)
{
   return false;
}
...
static Eina_Bool
enwin_state_get(void *data __UNUSED__ __UNUSED__, Evas_Object *obj __UNUSED__, const char *state __UNUSED__)
{
   return false;
}
