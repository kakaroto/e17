/* gcc -O3 -Wall -o fractale-fp fractale-fp.c `pkg-config --cflags --libs ecore-evas ecore evas eina` */
/*
 ...
 *                    Cedric Bail
 *
 * This program show how to use Ecore_Thread and Eina_F16p16 in a CPU heavy task.
 */
        ...
        for (j = 0, y = d->y; j < AREA_SIZE; j++, y++)
          {
             ...
             Za = SUB(DIV(ITOFP(4 * x), tw), f2);
             Zb = ADD(DIV(ITOFP(-4 * y), th), f2);
		    ...
		    + (255 << 8) + 255;
             ...
             else
               {
                  ...
                  for (k = 0; 
		       ADD(MUL(Za, Za), MUL(Zb, Zb)) <= f4
			 && k < N;
		       k++)
                    {
                       temp = Za;

                       Za = ADD(SUB(MUL(Za, Za), MUL(Zb, Zb)),
				ca);
                       Zb = ADD(SCALE(MUL(temp, Zb), 2), cb);
                    }

                  rouge = 128 - k * 128 / N;
                  vert = 174 - k * 255 / N; if (vert < 0) vert+=256;
                  bleu = 128 - k * 255 / N; if (bleu < 0) bleu+=256;
                  d->ptr[j][i] = (rouge << 24) + (vert << 16) + (bleu << 8) + 255;
               }
          }
     }
}
...
main(int argc, char **argv)
{
   ...
   ecore_evas_show(ee);
   ecore_evas_title_set(ee, "EFL Fixed Point - Fractale Julia");
