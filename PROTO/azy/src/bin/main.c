                      ...
                      Eina_Bool     header)
{
   ...
   if (header)
     {
        if (t->type == TD_STRUCT)
          {
             EL(0, "Azy_Value *%s(%s _nstruct) EINA_WARN_UNUSED_RESULT;", t->march_name,
                   ((!strcmp(t->ctype, "int")) || (!strcmp(t->ctype, "Eina_Bool"))) ? "int32_t" : t->ctype);
             EL(0, "Eina_Bool %s(Azy_Value *_struct, %s* _nstruct) EINA_WARN_UNUSED_RESULT;", t->demarch_name, t->ctype);
          }
        else
          {
             if (!strcmp(t->ctype, "Eina_Bool"))
               return;
             EL(0, "Azy_Value *%s(%s _narray) EINA_WARN_UNUSED_RESULT;", t->march_name, t->ctype);
             EL(0, "Eina_Bool %s(Azy_Value *_array, %s* _narray) EINA_WARN_UNUSED_RESULT;", t->demarch_name, t->ctype);
          }
        return;
     }
   ...
   if (t->type == TD_STRUCT)
     {
        EL(0, "Azy_Value *%s(%s _nstruct)", t->march_name,
           ((!strcmp(t->ctype, "int")) || (!strcmp(t->ctype, "Eina_Bool"))) ? "int32_t" : t->ctype);
        EL(0, "{");
        ...
        EL(0, "Eina_Bool %s(Azy_Value *_struct, %s* _nstruct)", t->demarch_name, t->ctype);
        EL(0, "{");
     ...
     {
        EL(0, "Azy_Value *%s(%s _narray)", t->march_name, t->ctype);
        EL(0, "{");
        EL(1, "Eina_List *_item;");
        EL(1, "%s v;", ((!strcmp(t->item_type->ctype, "int")) || (!strcmp(t->item_type->ctype, "Eina_Bool"))) ? "int32_t*" : t->item_type->ctype);
        ...
        EL(0, "Eina_Bool %s(Azy_Value *_array, %s* _narray)", t->demarch_name, t->ctype);
        EL(0, "{");
   ...
   else if (t->type == TD_ARRAY)
     {
        /* free */
         ...
         if (t->item_type->copy_func)
           EL(2,
              "copy = eina_list_prepend(copy, %s((%s)orig->data));",
...
gen_errors_header(Azy_Server_Module *s)
{
   ...
   if (s)
     EL(0, "void azy_err_init_%s(void);", s->name);
   else
   ...
   if (!s)
     {
        EINA_LIST_FOREACH(azy->modules, j, s)
          if (s->errors)
            EL(0, "extern void azy_err_init_%s(void);", s->name);
     }
...
gen_server_headers(Azy_Server_Module *s)
{
   ...
   EL(0, "typedef struct _%s%sModule %s%sModule;",
      azy->name, s->name, azy->name,
      s->name);
   NL;
   ...
   if (s->stub_init)
     {
        ...
        EL(0, " */ ");
        EL(0,
           "Eina_Bool %s%sModule_init(Azy_Server_Module* _module);",
           azy->name, s->name);
        NL;
     }
   ...
   if (s->stub_shutdown)
     {
        ...
        EL(0, " */ ");
        EL(0,
           "void %s%sModule_shutdown(Azy_Server_Module* _module);",
           azy->name, s->name);
        NL;
     }
   ...
   if (s->stub_pre)
     {
        ...
        EL(0, " */ ");
        EL(0,
           "Eina_Bool %s%sModule_pre(Azy_Server_Module* _module, Azy_Content* _content);",
           azy->name, s->name);
        NL;
     }
   ...
   if (s->stub_post)
     {
        ...
        EL(0, " */ ");
        EL(0,
           "Eina_Bool %s%sModule_post(Azy_Server_Module* _module, Azy_Content* _content);",
           azy->name, s->name);
        NL;
     }
   ...
   if (s->stub_fallback)
     {
        ...
        EL(0, " */ ");
        EL(0,
           "Eina_Bool %s%sModule_fallback(Azy_Server_Module* _module, Azy_Content* _content);",
           azy->name, s->name);
        NL;
     }
   ...
   if (s->stub_download)
     {
        ...
        EL(0, " */ ");
        EL(0,
           "Eina_Bool %s%sModule_download(Azy_Server_Module* _module);",
           azy->name, s->name);
        NL;
     }
   ...
   if (s->stub_upload)
     {
        ...
        EL(0, " */ ");
        EL(0,
           "Eina_Bool %s%sModule_upload(Azy_Server_Module* _module);",
           azy->name, s->name);
        NL;
     }
   ...
   EINA_LIST_FOREACH(s->methods, j, method)
     {
        ...
        E(0, "%s %s%sModule_%s(Azy_Server_Module* _module",
          method->return_type->ctype, azy->name, s->name,
          method->name);
   ...
   EINA_LIST_FOREACH(s->methods, j, method)
     {
        ...
        EL(0, "static Eina_Bool __method_%s(Azy_Server_Module* _module, Azy_Content* _content)", method->name);
        EL(0, "{");
...
// free native types and return
   ...
   if (s->stub_init)
     {
        EL(0, "Eina_Bool %s%sModule_init(Azy_Server_Module* _module)",
           azy->name, s->name);
        EL(0, "{");

        if (strstr(s->stub_init, "_priv"))
          EL(1, "%s%sModule* _priv = azy_server_module_data_get(_module);", azy->name, s->name);
        else
          EL(1, "(void)_module;");
   ...
   if (s->stub_shutdown)
     {
        ...
        if (strstr(s->stub_shutdown, "_priv"))
          EL(1, "%s%sModule* _priv = azy_server_module_data_get(_module);",
             azy->name, s->name);
        else
          EL(1, "(void)_module;");
   ...
   if (s->stub_pre)
     {
        EL(0, "Eina_Bool %s%sModule_pre(Azy_Server_Module* _module, Azy_Content* _content)", azy->name, s->name);
        EL(0, "{");
   ...
   if (s->stub_post)
     {
        ...
        if (strstr(s->stub_post, "_priv"))
          EL(1,
             "%s%sModule* _priv = azy_server_module_data_get(_module);",
             azy->name, s->name);
        else
          EL(1, "(void)_module;");
   ...
   if (s->stub_fallback)
     {
        EL(0, "Eina_Bool %s%sModule_fallback(Azy_Server_Module* _module, Azy_Content* _content)",
           azy->name, s->name);
        EL(0, "{");
...
azy_write(void)
{
   ...
   EINA_LIST_FOREACH(azy->modules, i, s)
     {
        if (client_headers)
...
main(int argc, char *argv[])
{
   ...
   if (debug)
     azyParserTrace(stdout, "azy_parser: ");
   err = EINA_FALSE;
