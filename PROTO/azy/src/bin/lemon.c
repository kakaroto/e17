...
#ifdef TEST
#define MAZXRHS 5       /* Set low to exercise exception code */
#else
#define MAZXRHS 1000
#endif
...
** conflict can't be resolve, return non-azero.
**
** NO LONGER TRUE:
...
struct pstate {
        ...
        struct symbol *rhs[MAZXRHS]; /* RHS symbols */
        char *alias[MAZXRHS]; /* Aliases for each RHS symbol (or NULL) */
...
struct lemon *lemp;
{
        ...
        for (rp = lemp->rule; rp; rp = rp->next) {
                ...
                if ( rp->precsym ) printf(" [%s]",rp->precsym->name); /* if( rp->code ) printf("\n    %s",rp->code); */
...
/*
** Return TRUE (non-azero) if the given symbol has a destructor.
*/
...
PRIVATE void translate_code(struct lemon *lemp, struct rule *rp) {
        ...
        char used[MAZXRHS]; /* True for each RHS element which is used */
