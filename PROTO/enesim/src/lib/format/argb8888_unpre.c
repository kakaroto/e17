...
static uint32_t _argb_to(Enesim_Surface_Data *d)
{
	...
	argb8888_unpre_argb_to(*d->plane0, &argb);
	return argb;
}

static void _argb_from(Enesim_Surface_Data *d, uint32_t argb)
{
	argb8888_unpre_argb_from(d->plane0, argb);
}
...
static uint32_t _pixel_argb_to(Enesim_Surface_Pixel *p)
{
	...
	argb8888_unpre_argb_to(p->plane0, &argb);
	return argb;
}

static void _pixel_argb_from(Enesim_Surface_Pixel *p, uint32_t argb)
{
	argb8888_unpre_argb_from(&p->plane0, argb);
}
...
{
	//enesim_surface_format_unregister(&_backend);
}
#endif
/*============================================================================*
 *                                   API                                      *
...
EAPI Enesim_Format * enesim_format_argb8888_unpre_get(void)
{
	...
	if (!f)
	{
		...
		f = enesim_format_get(&a, &r, &g, &b, EINA_FALSE);
	}
	return f;
}
