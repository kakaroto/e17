		...
		unsigned int len, uint32_t *dst)
{
	...
	if (rect->draw_mode == ENESIM_FIGURE_DRAW_MODE_STROKE)
	  {
		icolor = 0;
		fpaint = NULL;
	  }
	if (rect->draw_mode == ENESIM_FIGURE_DRAW_MODE_FILL)
	  {
		ocolor = icolor;
		fill_only = 1;
		do_inner = 0;
	...
	if ((rect->draw_mode == ENESIM_FIGURE_DRAW_MODE_STROKE_FILL) && do_inner && fpaint)
		fpaint->span(fpaint, x, y, len, dst);
...
static Eina_Bool _state_setup(Enesim_Renderer *p)
{
	...
	if (rect->fill.paint &&
	    ((rect->draw_mode == ENESIM_FIGURE_DRAW_MODE_FILL) ||
	     (rect->draw_mode == ENESIM_FIGURE_DRAW_MODE_STROKE_FILL)))
	{
...
EAPI void enesim_renderer_rectangle_outline_weight_set(Enesim_Renderer *p, float weight)
{
	Rectangle *rect;
...
EAPI void enesim_renderer_rectangle_outline_color_set(Enesim_Renderer *p, unsigned int stroke_color)
{
	...
	rect = (Rectangle *) p;
	if (!rect) return;
	if (rect->stroke.color == stroke_color)
		return;
...
EAPI void enesim_renderer_rectangle_fill_paint_set(Enesim_Renderer *p, Enesim_Renderer *paint)
{
	...
	if (!rect) return;
	if (p == paint) return;
	if (rect->fill.paint == paint)
		return;
	rect->fill.paint = paint;
//	p->changed = EINA_TRUE;
}

EAPI void enesim_renderer_rectangle_draw_mode_set(Enesim_Renderer *p, Enesim_Figure_Draw_Mode draw_mode)
{
	Rectangle *rect;
