...
{
	uint32_t argb_unpre;
	uint8_t pa = argb8888_alpha_get(color->pixel.argb8888.plane0);

	if ((pa > 0) && (pa < 255))
	{
		if (a) *a = pa;
		if (r) *r = (argb8888_red_get(color->pixel.argb8888.plane0) * 255) / pa;
		if (g) *g = (argb8888_green_get(color->pixel.argb8888.plane0) * 255) / pa;
		if (b) *b = (argb8888_blue_get(color->pixel.argb8888.plane0) * 255) / pa;
	}
	else
	{
		if (a) *a = pa;
		if (r) *r = argb8888_red_get(color->pixel.argb8888.plane0);
		if (g) *g = argb8888_green_get(color->pixel.argb8888.plane0);
		if (b) *b = argb8888_blue_get(color->pixel.argb8888.plane0);
	}
}
static inline void argb8888_pixel_blend(Enesim_Surface_Data *d, Enesim_Surface_Pixel *p)
{
	...
	a = 256 - argb8888_alpha_get(p->pixel.argb8888.plane0);
	argb8888_blend(d->data.argb8888.plane0, a, p->pixel.argb8888.plane0);
}
