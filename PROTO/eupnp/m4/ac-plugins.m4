dnl Copy of lightmediascanner's 0.3.0 file m4/ac-plugins.m4 written by
dnl Gustavo Sverzut Barbieri.
...
dnl AC_CHECK_PKG(name, lib [>= version], [action-if, [action-not]])
...
dnl AC_OPTIONAL_MODULE(name, [initial-status, [check-if-enabled]])
