...
EAPI int
exotic_access(const char *pathname, int mode)
{
...
EAPI ssize_t
exotic_readlink(const char *path,
                char       *buf,
                size_t      bufsize)
{
   errno = EINVAL;
...
EAPI int
exotic_symlink(const char *path1, const char *path2)
{
   errno = EINVAL;
   return -1;
}
...
{
   return exotic_access (pathname, mode);
}
