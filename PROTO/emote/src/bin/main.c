...
static void _em_main_shutdown_push(Eina_Bool (*func)(void));
...
/* local variables */
static Eina_Bool (*_em_main_shutdown_func[EM_MAX_LEVEL])(void);
...
/* local functions */
static void
_em_main_shutdown_push(Eina_Bool (*func)(void))
{
   _em_main_level++;
      ...
      case EMOTE_EVENT_CHAT_PARTED:
          {
             ...
             d = event;
             if ((EMOTE_EVENT_T(event)->type == EMOTE_EVENT_CHAT_JOINED))
               type = EINA_TRUE;
             em_gui_channel_status_update(EMOTE_EVENT_SERVER_T(d)->server,
                                          d->channel, d->user,
                                          EMOTE_EVENT_T(d)->protocol, type);
             break;
          }
