...
Enlil_Photo *
enlil_photo_copy_new(Enlil_Photo *photo)
{
 ...
 */
void
enlil_photo_copy(Enlil_Photo *photo_src,
                 Enlil_Photo *photo_dest)
{
   ...
   enlil_photo__time_set(photo_dest, enlil_photo__time_get(photo_src));
...
void
enlil_photo_free(Enlil_Photo **photo)
{
...
void
enlil_photo_list_print(const Eina_List *l_photos)
{
   const Eina_List *l;
...
void
enlil_photo_print(const Enlil_Photo *photo)
{
   ASSERT_RETURN_VOID(photo != NULL);
   printf("# Photo %s\n", photo->name);
   printf("\tFile name\t:\t%s\n", photo->file_name);
...
void
enlil_photo_album_set(Enlil_Photo *photo,
                      Enlil_Album *album)
{
   ASSERT_RETURN_VOID(photo != NULL);
   photo->album = album;
...
int
enlil_photo_netsync_id_get(Enlil_Photo *photo)
{
   ASSERT_RETURN(photo != NULL);
...
int
enlil_photo_netsync_version_header_get(Enlil_Photo *photo)
{
   ASSERT_RETURN(photo != NULL);

   return photo->netsync.version_header;
}

int
enlil_photo_netsync_version_tags_get(Enlil_Photo *photo)
{
   ASSERT_RETURN(photo != NULL);

   return photo->netsync.version_tags;
}

int
enlil_photo_netsync_version_file_get(Enlil_Photo *photo)
{
   ASSERT_RETURN(photo != NULL);

   return photo->netsync.version_file;
}

int
enlil_photo_netsync_version_header_net_get(Enlil_Photo *photo)
{
   ASSERT_RETURN(photo != NULL);

   return photo->netsync.version_header_net;
}

int
enlil_photo_netsync_version_tags_net_get(Enlil_Photo *photo)
{
   ASSERT_RETURN(photo != NULL);

   return photo->netsync.version_tags_net;
}

int
enlil_photo_netsync_version_file_net_get(Enlil_Photo *photo)
{
   ASSERT_RETURN(photo != NULL);

   return photo->netsync.version_file_net;
}

void
_enlil_photo_netsync_version_header_inc(Enlil_Photo *photo)
{
   ASSERT_RETURN_VOID(photo != NULL);

   photo->netsync.version_header++;
   ...
   if(library && _enlil_library_photo_version_header_increase_cb_get(library))
     _enlil_library_photo_version_header_increase_cb_get(library) (
       _enlil_library_photo_version_header_increase_data_get(library), photo);
}

void
_enlil_photo_netsync_version_tags_inc(Enlil_Photo *photo)
{
   ASSERT_RETURN_VOID(photo != NULL);

   photo->netsync.version_tags++;
...
void
_enlil_photo_netsync_version_file_inc(Enlil_Photo *photo)
{
   ASSERT_RETURN_VOID(photo != NULL);

   photo->netsync.version_file++;
...
void
_enlil_photo_netsync_version_header_both_set(Enlil_Photo *photo,
                                             int          version)
{
   ASSERT_RETURN_VOID(photo != NULL);

   photo->netsync.version_header = version;
   photo->netsync.version_header_net = version;
...
void
_enlil_photo_netsync_version_file_both_set(Enlil_Photo *photo,
                                           int          version)
{
   ASSERT_RETURN_VOID(photo != NULL);

   photo->netsync.version_file = version;
   photo->netsync.version_file_net = version;
...
void
_enlil_photo_netsync_version_tags_both_set(Enlil_Photo *photo,
                                           int          version)
{
   ASSERT_RETURN_VOID(photo != NULL);

   photo->netsync.version_tags = version;
   photo->netsync.version_tags_net = version;
...
void
_enlil_photo_netsync_id_set(Enlil_Photo *photo,
                            int          id)
{
   ASSERT_RETURN_VOID(photo != NULL);
...
void
enlil_photo_name_set(Enlil_Photo *photo,
                     const char  *name)
{
   ASSERT_RETURN_VOID(photo != NULL);
   ...
   _enlil_photo_netsync_version_header_inc(photo);
}

void
enlil_photo_author_set(Enlil_Photo *photo,
                       const char  *author)
{
   ASSERT_RETURN_VOID(photo != NULL);
   ASSERT_RETURN_VOID(author != NULL);

   if(author[0] == '\0' && photo->author == NULL)
     return;

   const char *new_author = eina_stringshare_add(author);

   if(photo->author == new_author)
     {
        EINA_STRINGSHARE_DEL(new_author);
        return;
     }
   EINA_STRINGSHARE_DEL(photo->author);

   photo->author = new_author;

   enlil_photo_iptc_add(photo, "Byline", photo->author, EINA_FALSE);

   _enlil_photo_netsync_version_header_inc(photo);
}

void
enlil_photo_description_set(Enlil_Photo *photo,
                            const char  *desc)
{
   ASSERT_RETURN_VOID(photo != NULL);
...
void
enlil_photo_exif_clear(Enlil_Photo *photo)
{
   Enlil_Exif *exif;
...
void
enlil_photo_iptc_clear(Enlil_Photo *photo)
{
   Enlil_IPTC *iptc;
 ...
 */
double
enlil_photo_longitude_get(const Enlil_Photo *photo)
{
   ASSERT_RETURN(photo != NULL);
 ...
 */
double
enlil_photo_latitude_get(const Enlil_Photo *photo)
{
   ASSERT_RETURN(photo != NULL);
...
const Eina_List *
enlil_photo_exifs_get(const Enlil_Photo *photo)
{
   ASSERT_RETURN(photo != NULL);
...
const Eina_List *
enlil_photo_iptcs_get(const Enlil_Photo *photo)
{
   ASSERT_RETURN(photo != NULL);
 ...
 */
Enlil_Exif *
enlil_photo_exif_get_from_tag(const Enlil_Photo *photo,
                              const char        *tag)
{
   const Eina_List *l;
   Enlil_Exif *exif;
 ...
 */
Enlil_IPTC *
enlil_photo_iptc_get_from_name_and_value(const Enlil_Photo *photo,
                                         const char        *name,
                                         const char        *value)
{
   const Eina_List *l;
...
void
enlil_photo_copy_exif_in_file(Enlil_Photo *photo,
                              const char  *file)
{
   char buf[PATH_MAX];
   JPEGData *data;
...
void
enlil_photo_save_iptc_in_file(Enlil_Photo *photo)
{
   char buf[PATH_MAX];
...
void
enlil_photo_save_iptc_in_custom_file(Enlil_Photo *photo,
                                     const char  *file)
{
   IptcData *d;
   IptcDataSet *ds;
   ...
   snprintf (_tmpfile, PATH_MAX, "%s.%d", file, getpid());
   outfile = fopen (_tmpfile, "w");
   if (!outfile)
     {
...
void
enlil_photo_exifs_set(Enlil_Photo *photo,
                      Eina_List   *exifs)
{
   Enlil_Exif *exif;
...
void
enlil_photo_iptcs_set(Enlil_Photo *photo,
                      Eina_List   *iptcs)
{
   Enlil_IPTC *iptc;
   Eina_List *l, *l2;
   ...
   //load the author
   iptc_tag_find_by_name ("Byline", &iptcrecord, &iptctag);
   ...
   EINA_LIST_FOREACH(photo->iptcs->l, l, iptc)
     {
        ...
        EINA_STRINGSHARE_DEL(photo->author);
        photo->author = eina_stringshare_add(enlil_iptc_value_get(iptc));
        break;
     }
...
void
enlil_photo_user_data_set(Enlil_Photo        *photo,
                          void               *user_data,
...
void
enlil_photo_path_set(Enlil_Photo *photo,
                     const char  *path)
{
   ASSERT_RETURN_VOID(photo != NULL);
...
void
enlil_photo_thumb_fdo_normal_set(Enlil_Photo *photo,
                                 const char  *thumb)
{
   ASSERT_RETURN_VOID(photo != NULL);
...
void
enlil_photo_thumb_fdo_large_set(Enlil_Photo *photo,
                                const char  *thumb)
{
   ASSERT_RETURN_VOID(photo != NULL);
...
int
enlil_photo_is(const char *file)
{
...
int
enlil_video_is(const char *file)
{
   ASSERT_RETURN(file != NULL);
...
int
enlil_photo_jpg_is(const char *file)
{
   ASSERT_RETURN(file != NULL);
 ...
 */
void
enlil_photo_tag_process(Enlil_Photo *photo)
{
   Eina_List *l;
   Enlil_Photo_Tag *photo_tag;
   ASSERT_RETURN_VOID(photo != NULL);
 ...
 */
void
enlil_photo_tag_add(Enlil_Photo *photo,
                    const char  *tag_name)
{
   Enlil_Photo_Tag *photo_tag;
   Eina_List *l;
...
Eina_Bool
enlil_photo_iptc_add(Enlil_Photo *photo,
                     const char  *name,
                     const char  *value,
 ...
 */
void
enlil_photo_tag_remove(Enlil_Photo     *photo,
                       Enlil_Photo_Tag *photo_tag)
{
   ASSERT_RETURN_VOID(photo != NULL);
...
void
enlil_photo_tag_clear(Enlil_Photo *photo)
{
   ASSERT_RETURN_VOID(photo != NULL);
...
Eina_Bool
enlil_photo_iptc_remove(Enlil_Photo *photo,
                        const char  *name,
                        const char  *value)
{
   Enlil_IPTC *iptc;
 ...
 */
Enlil_Photo *
enlil_photo_eet_load(const char *eet_path,
                     const char *key)
{
...
int
enlil_photo_eet_remove(const char *eet_path,
                       const char *key)
{
   Eet_File *f;
   char buf[PATH_MAX];
...
int
enlil_photo_eet_save(Enlil_Photo *photo)
{
   Eet_Data_Descriptor *edd, *edd_tag;
   Eet_File *f;
 ...
 */
const char *
_enlil_photo_exif_datetimeoriginal_get(const Enlil_Photo *photo)
{
   ASSERT_RETURN(photo != NULL);
...
static int
_photo_eet_exif_save(Enlil_Photo *photo)
{
   Eet_Data_Descriptor *edd, *edd_exif;
   Eet_File *f;
   char buf[PATH_MAX], path[PATH_MAX];
...
static int
_photo_eet_iptc_save(Enlil_Photo *photo)
{
   Eet_Data_Descriptor *edd, *edd_iptc;
   Eet_File *f;
   char buf[PATH_MAX], path[PATH_MAX];
...
_enlil_photo_file_name_edd_new(void)
{
   ...
   EET_DATA_DESCRIPTOR_ADD_BASIC(edd, Enlil_Photo, "netsync.version_header", netsync.version_header, EET_T_INT);
   EET_DATA_DESCRIPTOR_ADD_BASIC(edd, Enlil_Photo, "netsync.version_tags", netsync.version_tags, EET_T_INT);
   EET_DATA_DESCRIPTOR_ADD_BASIC(edd, Enlil_Photo, "netsync.version_file", netsync.version_file, EET_T_INT);
   EET_DATA_DESCRIPTOR_ADD_BASIC(edd, Enlil_Photo, "netsync.version_header_net", netsync.version_header_net, EET_T_INT);
   EET_DATA_DESCRIPTOR_ADD_BASIC(edd, Enlil_Photo, "netsync.version_tags_net", netsync.version_tags_net, EET_T_INT);
   EET_DATA_DESCRIPTOR_ADD_BASIC(edd, Enlil_Photo, "netsync.version_file_net", netsync.version_file_net, EET_T_INT);
...
enlil_photo_edd_new(Eet_Data_Descriptor *edd_tag)
{
   ...
   EET_DATA_DESCRIPTOR_ADD_BASIC(edd, Enlil_Photo, "path", path, EET_T_STRING);
   EET_DATA_DESCRIPTOR_ADD_BASIC(edd, Enlil_Photo, "file_name", file_name, EET_T_STRING);
   EET_DATA_DESCRIPTOR_ADD_BASIC(edd, Enlil_Photo, "author", author, EET_T_STRING);
   EET_DATA_DESCRIPTOR_ADD_BASIC(edd, Enlil_Photo, "description", description, EET_T_STRING);
   EET_DATA_DESCRIPTOR_ADD_BASIC(edd, Enlil_Photo, "_time", _time, EET_T_LONG_LONG);
