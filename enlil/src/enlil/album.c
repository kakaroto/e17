...
void _enlil_album_flickr_id_set(Enlil_Album *album, const char *id)
{
   ...
   if(id)
     album->flickr.id = eina_stringshare_add(id);
   else
     album->flickr.id = NULL;
}
...
Eet_Data_Descriptor * _enlil_album_header_edd_new(Eet_Data_Descriptor *edd_collection)
{
    ...
    EET_DATA_DESCRIPTOR_ADD_BASIC(edd, Enlil_Album, "flickr.id", flickr.id, EET_T_STRING);
    EET_DATA_DESCRIPTOR_ADD_BASIC(edd, Enlil_Album, "flickr.need_sync", flickr.need_sync, EET_T_UCHAR);
