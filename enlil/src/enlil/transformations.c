...
static void _enlil_trans_history_add(Enlil_Trans_History *h,
                                     const char          *file,
                                     Enlil_Trans_Job_Type type);
...
Enlil_Trans_Job *
enlil_trans_job_add(Enlil_Trans_History *h,
                    const char          *file,
                    Enlil_Trans_Job_Type type,
                    Enlil_Trans_Done_Cb  cb,
                    void                *data)
...
void
enlil_trans_job_del(Enlil_Trans_Job *job)
{
...
Enlil_Trans_History *
enlil_trans_history_new(const char *file)
{
   char buf[PATH_MAX];
   int i = 0;
...
void
enlil_trans_history_free(Enlil_Trans_History *h)
{
...
const Eina_List *
enlil_trans_history_get(const Enlil_Trans_History *h)
{
   ASSERT_RETURN(h != NULL);
   return h->history;
}

const Enlil_Trans_History_Item *
enlil_trans_history_current_get(const Enlil_Trans_History *h)
{
   ASSERT_RETURN(h != NULL);
   return h->current;
}
...
void
enlil_trans_history_clear(Enlil_Trans_History *h)
{
...
const char *
enlil_trans_history_goto(Enlil_Trans_History *h,
                         int                  pos)
{
   ASSERT_RETURN(h != NULL);
...
const char *
enlil_trans_history_item_file_get(const Enlil_Trans_History_Item *item)
{
   ASSERT_RETURN(item != NULL);
   return item->file;
}

Enlil_Trans_Job_Type
enlil_trans_history_item_type_get(const Enlil_Trans_History_Item *item)
{
   ASSERT_RETURN(item != NULL);
   return item->type;
}
...
static void
_enlil_trans_history_add(Enlil_Trans_History *h,
                         const char          *file,
                         Enlil_Trans_Job_Type type)
{
   Enlil_Trans_History_Item *item;
...
static Eina_Bool
_exe_del_cb(void *data __UNUSED__,
            int   type __UNUSED__,
