...
entranced_cookie_new(char *cookie)
{
   ...
   {
      entranced_debug
         (_("Cookie generation failed: could not open %s\n"), ENTRANCE_RANDOM);
      return 0;
   }
   ...
   {
      entranced_debug
         (_("Cookie generation failed: could not read %s\n"), ENTRANCE_RANDOM);
      return 0;
   }
...
_entranced_auth_purge(Entranced_Display * d, FILE * auth_file)
{
   ...
   {
      entranced_debug(_("entranced_auth_purge: Write failed!\n"));
      return;
   }
...
_entranced_auth_generate(void)
{
   ...
   if (auth)
   {
      entranced_debug(_("Generated MIT-MAGIC-COOKIE-1 "));
      for (i = 0; i < auth->data_length; ++i)
         entranced_debug(" %02x", auth->data[i] & 0xff);
      entranced_debug("\n");
   }
   else
      entranced_debug(_("Auth generation failed.\n"));
                          ...
                          const char *addr, int addrlen)
{
   ...
   {
      entranced_debug(_("_entrance_auth_entry_add: Auth write failed!\n"));
      return 0;
   }
   ...
   {
      entranced_debug
         (_("_entrance_auth_entry_add: Could not add auth entry to list!\n"));
      return 0;
   }
...
entranced_auth_display_secure(Entranced_Display * d)
{
   ...
   entranced_debug
      (_("entranced_auth_display_secure: Setting up access for display %s\n"),
       d->name);
   ...
   entranced_debug
      (_("entranced_auth_display_secure: Successfully set up access for %s (localhost)\n"),
       d->name);
...
entranced_auth_user_add(Entranced_Display * d, const char *homedir)
{
   ...
   entranced_debug(_("entranced_auth_user_add: Adding auth cookie\n"));
   ...
   while (1)
   {
      ...
      {
         entranced_debug
            (_("entranced_auth_user_add: Unable to write auth file %s\n"),
             d->client.authfile);
   ...
   entranced_debug(_("entranced_auth_user_add: Opened %s for writing cookies\n"),
                   d->client.authfile);
   ...
   entranced_debug
      (_("entranced_auth_user_add: Finished writing auth entries to %s\n"),
       d->client.authfile);
   seteuid(0);
...
entranced_auth_user_remove(Entranced_Display * d)
{
   ...
   entranced_debug(_("entranced_auth_user_remove: Removing cookie from %s\n"),
                   d->client.authfile);
