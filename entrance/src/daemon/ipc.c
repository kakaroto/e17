...
_entranced_ipc_client_add(void *data, int type, void *event)
{
   ...
   entranced_debug(_("_entranced_ipc_client_add: Received event\n"));
   return 1;
}
...
_entranced_ipc_client_del(void *data, int type, void *event)
{
   ...
   entranced_debug(_("_entranced_ipc_client_del: Received event\n"));
   return 1;
}
...
_entranced_ipc_client_data(void *data, int type, void *event)
{
          ...
          && (_display->client.homedir))
      {
                                        ...
                                        homedir))
         {
            ecore_ipc_client_send(e->client, E_XAUTH_ACK, 0, 0, 0, 0, NULL,
                                  0);
            entranced_debug
               (_("_entranced_ipc_client_data: All tokens received; wrote cookie to user dir %s\n"),
                _display->client.homedir);
         }
         else
         {
            ecore_ipc_client_send(e->client, E_XAUTH_NAK, 0, 0, 0, 0, NULL,
                                  0);
            entranced_debug
               (_("_entranced_ipc_client_data: Failed to write cookie to %s\n"),
                _display->client.homedir);
         }
      }
   }
   ...
   entranced_debug
      (_("_entranced_ipc_client_data: Received [%i] [%i] [%i] [%i] \"%s\"\n"),
...
entranced_ipc_init(pid_t pid)
{
   ...
   {
      entranced_debug(_("entranced_ipc_init: Failed to open socket %s.\n"),
                      ipc_title);
      return 0;
   }
   ...
   entranced_debug(_("entranced_ipc_init: Success\n"));
...
entranced_ipc_shutdown(void)
{
   ...
   entranced_debug(_("entranced_ipc_shutdown: Success\n"));
}
