...
get_my_hostname(void)
{
   ...
   if (!(gethostname(buf, 255)))
   {
      ...
      if (dot)
         *dot = '\0';
      if (session->config->use_default_greeting)
        snprintf(message, PATH_MAX, "%s %s", _("Welcome to"), buf);
      else
        snprintf(message, PATH_MAX, "%s %s %s", session->config->before.string, buf, session->config->after.string);
   }
   else
      if (session->config->use_default_greeting)
        snprintf(message, PATH_MAX, _("Welcome to Localhost"));
      else
        snprintf(message, PATH_MAX, "%s Localhost %s", session->config->before.string, session->config->after.string);
...
focus(void *data, Evas_Object * o, const char *emission, const char *source)
{
   ...
   if ((oo = (Evas_Object *) data))
   {
      ...
      else if (!strcmp(emission, "Out"))
      {
         ...
         fprintf(stderr, _("Unknown signal Emission(%s)"), emission);
   }
}
   ...
   if ((container = data))
   {
      ...
      switch (esmart_container_direction_get(container))
      {
        ...
        case CONTAINER_DIRECTION_VERTICAL:
           ...
           break;
        default:
           fprintf(stderr, _("Unknown Container Orientation\n"));
           break;
      }
   }
}
...
main(int argc, char *argv[])
{
   ...
   if (nonargs < 0)
      {
         fputs(_("ERROR: could not parse command line options.\n"), stderr);
	 return -1;
      }
   ...
   if (nonargs != argc)
      {
         fputs(_("Invalid non-option argument"), stderr);
   ...
   {
      fprintf(stderr, _("Fatal error: Could not initialize ecore_evas!\n"));
      exit(EXITCODE);
   }
   return (0);
}
...
static void
_set_userpass_string(void *data, Evas_Object *obj, const char *emission, const char *source)
{
   Evas_Object *edje = obj;
   const char *string = data;
   edje_object_part_text_set(edje, "entrance.userpass", string);
}

static void
_session_list_scroll_up(void *data, Evas_Object *obj, const char *emission, const char *source)
{
  Evas_Object *cont = data;
  esmart_container_scroll_start(cont, 5);
}

static void
_session_list_scroll_down(void *data, Evas_Object *obj, const char *emission, const char *source)
{
  Evas_Object *cont = data;
  esmart_container_scroll_start(cont, -5);
}
...
static void
_session_list_scroll_stop(void *data, Evas_Object *obj, const char *emission, const char *source)
{
  Evas_Object *cont = data;
...
setup_ecore_evas(int wx, int wy, int ww, int wh, int fullscreen)
{
   ...
   if (session->testing)
      ecore_evas_title_set(e, _("Entrance - Testing Mode"));
   else
   ...
   for (i = 0; i < screens; i++)
   {
      ...
      if (i == 0)
      {
         ...
         if (edje_object_part_exists(edje, "entrance.xsessions.list"))
         {
            ...
            edje_object_signal_callback_add(edje, "entrance,sessions,scroll,up", "", _session_list_scroll_up, session->session_container);
            edje_object_signal_callback_add(edje, "entrance,sessions,scroll,down", "", _session_list_scroll_down, session->session_container);
            edje_object_signal_callback_add(edje, "entrance,sessions,scroll,stop", "", _session_list_scroll_stop, session->session_container);
         }
	  ...
	  */
         ...
         edje_object_signal_callback_add(edje, "entrance,label,user",
                                         "", _set_userpass_string, _("Please enter your user name"));
         edje_object_signal_callback_add(edje, "entrance,label,pass",
                                         "", _set_userpass_string, _("Please enter your password"));
