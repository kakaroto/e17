/**
 * A graphical tool to view iterations of eina tiler.
 *
 * Eina tiler is the eina functionality that receives a bunch of input
 * rectangle and split them to avoid overlaping areas, thus producing
 * tiles that can be laid out side by side. In order to avoid a huge
 * number of output rectangles, it does some tricks such as accepting
 * a small amount of error and then merge couple rectangles into
 * single one.
 *
 * Compile:
 * gcc -o eina-tiler-visualizer eina-tiler-visualizer.c -Wall `pkg-config --cflags --libs ecore evas ecore-evas eina-1`
 */
