...
#ifndef lint
static const char sccsid[] = "@(#)edb_region.c	10.53 (Sleepycat) 11/10/98";
#endif /* not lint */
...
static int __edb_growregion __P((REGINFO *, size_t));

/*
 * __edb_rattach --
 *	Optionally create and attach to a shared memory region.
 *
 * PUBLIC: int __edb_rattach __P((REGINFO *));
 */
int
__edb_rattach(infop)
	REGINFO *infop;
{
	...
	if (infop->size > DB_REGIONSIZE_MAX) {
		__edb_err(infop->edbenv, "__edb_rattach: cache size too large");
		return (EINVAL);
	}
#endif
...
loop:	infop->addr = NULL;
	 ...
	 * is being used).  The edbenv contains the current DB environment,
	 ...
	 */
	if ((ret = __edb_appname(infop->edbenv, infop->appname, infop->path,
	    infop->file, infop->edbflags, &infop->fd, &infop->name)) != 0)
		return (ret);
	if (infop->fd != -1)
	 ...
	 * one.
	 */
	if (infop->fd == -1 && infop->edbflags & DB_CREATE) {
		flags = infop->edbflags;
		LF_SET(DB_EXCL);
		if ((ret = __edb_open(infop->name,
	...
	if (F_ISSET(infop, REGION_CREATED)) {
		 ...
		 */
		if (!DB_GLOBAL(edb_region_anon) &&
		    (ret = __edb_growregion(infop, infop->size)) != 0)
			goto err;
	} else {
		/*
	 ...
	 */
	if ((ret = __edb_mapregion(infop->name, infop)) != 0)
		if (ret == EAGAIN) {
			/*
...
region_init:
	...
	if (F_ISSET(infop, REGION_CREATED)) {
		 ...
		 *
		 * Check the return of __edb_mutex_init() and __edb_mutex_lock(),
		 ...
		 * database home.)
		 */
		if ((ret = __edb_mutex_init(&rlp->lock,
		    MUTEX_LOCK_OFFSET(rlp, &rlp->lock))) != 0 ||
		    (ret = __edb_mutex_lock(&rlp->lock, infop->fd)) != 0)
			goto err;

		/* Initialize the remaining region information. */
		rlp->refcnt = 1;
		rlp->size = infop->size;
		edb_version(&rlp->majver, &rlp->minver, &rlp->patch);
		rlp->panic = 0;
		...
		/* Get the region lock. */
		(void)__edb_mutex_lock(&rlp->lock, infop->fd);
	...
	if (0) {
errmsg:		__edb_err(infop->edbenv, "%s: %s", infop->name, strerror(ret));

err:
retry:		/* Discard the region. */
		if (infop->addr != NULL) {
			(void)__edb_unmapregion(infop);
			infop->addr = NULL;
		}
	 ...
	 *
	 * The underlying UNIX __edb_mapregion() code preferentially uses the
 ...
 *
 * PUBLIC: int __edb_rdetach __P((REGINFO *));
 */
int
__edb_rdetach(infop)
	REGINFO *infop;
{
	...
	if ((t_ret = __edb_unmapregion(infop)) != 0 && ret == 0)
		ret = t_ret;
 ...
 *	Remove a region.
 *
 * PUBLIC: int __edb_runlink __P((REGINFO *, int));
 */
int
__edb_runlink(infop, force)
	REGINFO *infop;
	int force;
{
	RLAYOUT rl, *rlp;
	 ...
	 */
	if ((ret = __edb_appname(infop->edbenv, infop->appname,
	    infop->path, infop->file, infop->edbflags, NULL, &name)) != 0)
		return (ret);
	...
	/*
	 * See the comments in __edb_rattach -- figure out if this is a regular
	...
	ret = __edb_unlinkregion(name, infop);
 ...
 *	Extend a region.
 *
 * PUBLIC: int __edb_rgrow __P((REGINFO *, size_t));
 */
int
__edb_rgrow(infop, new_size)
	REGINFO *infop;
	size_t new_size;
{
	RLAYOUT *rlp;
	...
	if ((ret = __edb_growregion(infop, increment)) != 0)
		return (ret);
	...
	return (__edb_rreattach(infop, new_size));
}
 ...
 */
static int
__edb_growregion(infop, increment)
	REGINFO *infop;
	size_t increment;
{
	edb_pgno_t pages;
...
eio:	ret = EIO;
err:	__edb_err(infop->edbenv, "region grow: %s", strerror(ret));
	return (ret);
}
 ...
 *
 * PUBLIC: int __edb_rreattach __P((REGINFO *, size_t));
 */
int
__edb_rreattach(infop, new_size)
	REGINFO *infop;
	size_t new_size;
{
	int ret;
...
#ifdef DIAGNOSTIC
	if (infop->name == NULL) {
		__edb_err(infop->edbenv, "__edb_rreattach: name was NULL");
		return (EINVAL);
	}
#endif
	/*
	 ...
	 */
	if ((ret = __edb_unmapregion(infop)) != 0)
		return (ret);
	...
	ret = __edb_mapregion(infop->name, infop);
