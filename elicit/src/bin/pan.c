...
static void _smart_child_fill(E_Smart_Data *sd);
...
void
pan_child_set(Evas_Object *obj, Evas_Object *child)
{
   API_ENTRY return;
...
void
pan_fill_policy_set(Evas_Object *obj, int fill_x, int fill_y)
{
   API_ENTRY return;
   if (sd->fill_x == fill_x && sd->fill_y == fill_y) return;
   sd->fill_x = fill_x;
   sd->fill_y = fill_y;

   _smart_child_fill(sd);
}

void
pan_set(Evas_Object *obj, Evas_Coord x, Evas_Coord y)
{
   API_ENTRY return;
...
void
pan_get(Evas_Object *obj, Evas_Coord *x, Evas_Coord *y)
{
   API_ENTRY return;
   if (x) *x = sd->px;
   if (y) *y = sd->py;
...
void
pan_max_get(Evas_Object *obj, Evas_Coord *x, Evas_Coord *y)
{
   API_ENTRY return;
   if (x)
     {
...
void
pan_child_size_get(Evas_Object *obj, Evas_Coord *w, Evas_Coord *h)
{
   API_ENTRY return;
   if (w) *w = sd->child_w;
   if (h) *h = sd->child_h;
...
static void
_smart_reconfigure(E_Smart_Data *sd)
{
