...
void enlil_win_bg_set(Enlil_Win *win,
		      const char *file);

Evas_Object *menu_photo_new(Evas_Object *parent);
Album_Menu  *album_collection_menu_new(Evas_Object *parent,
                                       Enlil_Album *album);
Album_Menu  *album_tag_menu_new(Evas_Object *parent,
                                Enlil_Album *album);
Album_Menu  *album_menu_new(Evas_Object *parent,
                            Enlil_Album *album);
Album_Menu  *album_sorts_menu_new(Evas_Object *parent,
                                  Enlil_Album *album);
void
                 album_menu_free(Album_Menu *album_menu);

Collection_Menu *collection_menu_new(Evas_Object *parent,
				     Enlil_Collection *col);
void
          collection_menu_free(Collection_Menu *col_menu);

Tag_Menu *tag_menu_new(Evas_Object *parent,
                        Enlil_Tag *tag);
void
            tag_menu_free(Tag_Menu *tag_menu);

Photo_Menu *photo_tag_menu_new(Evas_Object *parent,
			       Enlil_Photo *photo);
Photo_Menu *photo_menu_new(Evas_Object *parent,
                            Enlil_Photo *photo,
                            Eina_List *photos);
void photo_menu_init(List_Photo *enlil_photo,
		     Evas_Object *edje);
void photo_menu_free(Photo_Menu *photo_menu);

Panel_Image *panel_image_new(Evas_Object *parent,
			     Enlil_Photo *photo);
void
...
Panel_Geocaching *panel_geocaching_new(Evas_Object *parent,
                                       Enlil_Geocaching *geocaching);
void panel_geocaching_free(Panel_Geocaching **panel_geocaching);

/* Left list */
List_Left *list_left_new(Evas_Object *win);
void list_left_data_set(List_Left *list_album,
                        Enlil_Data *data);
void list_left_add(List_Left *list_album,
                   Enlil_Album *album);
void list_left_remove(List_Left *list_album,
                      Enlil_Album *album);
void list_left_update(List_Left *list_album,
                      Enlil_Album *album);

void list_left_col_add(List_Left *list_album,
                       Enlil_Collection *col);
void list_left_append_relative(List_Left *list_left,
                               Enlil_Album *album,
                               Elm_Genlist_Item *relative);
void list_left_col_album_add(List_Left *list_album,
                             Enlil_Collection *col,
                             Enlil_Album *album);
void list_left_col_album_remove(List_Left *list_album,
                                Enlil_Collection *col,
                                Enlil_Album *album);

void list_left_tag_add(List_Left *list_album,
                       Enlil_Tag *tag);

/* Map */
Map *map_new(Evas_Object *win);
void map_photo_add(Map *map,
                   Enlil_Photo *photo);
void map_photo_update(Map *map,
                      Enlil_Photo *photo);
void map_photo_remove(Map *map,
                      Enlil_Photo *photo);
void map_free(Map *map);
void map_geocaching_add(Map *map,
                        Enlil_Geocaching *gp);
void map_geocaching_update(Map *map,
                           Enlil_Geocaching *gp);
void map_geocaching_remove(Map *map,
                           Enlil_Geocaching *gp);

/* List of photos */
List_Photo *list_photo_new(Evas_Object *win);
void list_photo_data_set(List_Photo *list_photo,
                         Enlil_Data *enlil_data);
void list_photo_album_add(List_Photo *list_photo,
                          Enlil_Album *album);
void list_photo_album_append_relative(List_Photo *list_photo,
                                      Enlil_Album *album,
                                      PL_Header_Item *relative);
void list_photo_photo_add(List_Photo *list_photo,
                          Enlil_Album *album,
                          Enlil_Photo *photo);
void list_photo_photo_append_relative(List_Photo *list_photo,
                                      Enlil_Album *album,
                                      Enlil_Photo *photo,
...
Inwin *inwin_tag_new_new(Inwin_Del del_cb,
                         void *data,
                         Eina_List *photos);
Inwin *inwin_album_tag_new_new(Inwin_Del del_cb,
                               void *data,
                               Enlil_Album *album);
Inwin *inwin_collection_new_new(Inwin_Del del_cb,
                                void *data,
                                Enlil_Album *album);
Inwin *inwin_album_new_new(Inwin_Del del_cb,
                           void *data);
Inwin *inwin_photo_delete_new(Evas_Object *win,
                              Inwin_Del del_cb,
                              void *data,
                              Eina_List *photos);
Inwin *inwin_album_delete_new(Inwin_Del del_cb,
                              void *data,
                              Enlil_Album *album);
Inwin *inwin_photo_move_album_new(Inwin_Del del_cb,
                                  void *data,
                                  Eina_List *photos);
Inwin *inwin_photo_save_new(Inwin_Del del_cb,
                            Inwin_Apply apply_cb,
                            Inwin_Close close_cb,
                            void *data,
                            Enlil_Photo *photo);
Inwin *inwin_album_rename_new(Inwin_Del del_cb,
                              void *data,
                              Enlil_Album *album);
Inwin *inwin_save_as_file_exists_new(Inwin_Del del_cb,
                                     Inwin_Apply apply_cb,
                                     void *data,
                                     const char *file);
