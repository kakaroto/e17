...
static const E_Gadcon_Client_Class _gadcon_class = {
  ...
  "weather", {_gc_init, _gc_shutdown, _gc_orient, _gc_label, _gc_icon, _gc_id_new, NULL, NULL},
...
static Eina_Bool _weather_server_add(void *data, int type, void *event);
static Eina_Bool _weather_server_del(void *data, int type, void *event);
static Eina_Bool _weather_server_data(void *data, int type, void *event);
   ...
   if ((ev->button == 3) && (!weather_config->menu))
     {
        ...
        ma = e_menu_new();
        e_menu_post_deactivate_callback_set(ma, _weather_menu_cb_post, inst);
        weather_config->menu = ma;
