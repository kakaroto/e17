...
{
   GADCON_CLIENT_CLASS_VERSION, "bodhi-shutdown", 
     {
...
_gc_init(E_Gadcon *gc, const char *name, const char *id, const char *style) 
{
   ...
   if (!e_theme_edje_object_set(inst->o_base, 
                                "base/theme/modules/bodhi-shutdown", 
                                "e/modules/bodhi-shutdown/button")) 
     {
        char buff[PATH_MAX];

        snprintf(buff, sizeof(buff), "%s/e-module-bodhi-shutdown.edj", mod_dir);
...
_gc_icon(E_Gadcon_Client_Class *cc, Evas *evas) 
{
   ...
   edje_object_file_set(obj, buff, "icon");
   return obj;
}
...
_gc_id_new(E_Gadcon_Client_Class *cc) 
{
   ...
   snprintf(buff, sizeof(buff), "%s.%d", 
            _gc_class.name, eina_list_count(instances));
   return strdup(buff);
}
...
/* public functions */
EAPI E_Module_Api e_modapi = { E_MODULE_API_VERSION, "Bodhi Shutdown" };
...
e_modapi_init(E_Module *m) 
{
   ...
   snprintf(buff, sizeof(buff), "%s/locale", mod_dir);
