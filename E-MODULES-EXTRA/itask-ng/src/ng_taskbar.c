...
_ngi_taskbar_cb_drop_enter(void *data, const char *type, void *event_info)
{
   ...
   if (!ng->item_active) return; /* FIXME set edge_in before ?*/

   ITEM_MOUSE_IN(ng->item_active);

   if (box->dnd_timer) ecore_timer_del(box->dnd_timer);
...
_ngi_taskbar_item_new(Ngi_Box *box, E_Border *bd)
{
   ...
   if (!_ngi_taskbar_border_check(box, bd)) return;
   if (_ngi_taskbar_border_find(box, bd)) return;
