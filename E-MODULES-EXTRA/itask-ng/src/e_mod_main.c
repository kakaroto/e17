...
_ngi_win_cb_desk_show(void *data, int type, void *event)
{
   ...
   int fullscreen = _ngi_check_fullscreen(ev->desk);
...
ngi_new(Config_Item *cfg)
{
   ...
   if (cfg->autohide == AUTOHIDE_FULLSCREEN)
     {
        /* ng->show_bar = 1; */
...
_ngi_zoom_function(Ng *ng, double d, double *disp)
{
   ...
   *disp = erf(d / range) * range * (ng->zoom - 1.0) + 0.5;
} /* _ngi_zoom_function */
