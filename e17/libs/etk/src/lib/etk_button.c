...
static void _etk_button_mouse_clicked_cb(Etk_Object *object, Etk_Event_Mouse_Up_Down *event, void *data);
...
{
   return etk_widget_new(ETK_BUTTON_TYPE, "theme_group", "button", "focusable", TRUE, NULL);
}
...
{
   return etk_widget_new(ETK_BUTTON_TYPE, "theme_group", "button", "label", label, "focusable", TRUE, NULL);
}
...
static void _etk_button_constructor(Etk_Button *button)
{
   ...
   etk_signal_connect("mouse_clicked", ETK_OBJECT(button), ETK_CALLBACK(_etk_button_mouse_clicked_cb), NULL);
}
