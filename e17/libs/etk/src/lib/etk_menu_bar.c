...
enum _Etk_Menu_Bar_Signal_Id
{
   ETK_MENU_BAR_NUM_SIGNALS
};
...
static void _etk_menu_bar_child_added_cb(Etk_Object *object, void *child, void *data);
static void _etk_menu_bar_child_removed_cb(Etk_Object *object, void *child, void *data);
...
static Etk_Signal *_etk_menu_bar_signals[ETK_MENU_BAR_NUM_SIGNALS];
...
static void _etk_menu_bar_constructor(Etk_Menu_Bar *menu_bar)
{
   ...
   etk_signal_connect("child_added", ETK_OBJECT(menu_bar), ETK_CALLBACK(_etk_menu_bar_child_added_cb), NULL);
   etk_signal_connect("child_removed", ETK_OBJECT(menu_bar), ETK_CALLBACK(_etk_menu_bar_child_removed_cb), NULL);
}
...
{
   Evas_List *l;
   Etk_Container *menu_bar_container;
   
   if (!(menu_bar_container = ETK_CONTAINER(widget)) || !size_requisition)
      return;
   
   size_requisition->w = 0;
   size_requisition->h = 0;
   for (l = menu_bar_container->children; l; l = l->next)
   {
...
static void _etk_menu_bar_size_allocate(Etk_Widget *widget, Etk_Geometry geometry)
{
   ...
   if (!(menu_bar_container = ETK_CONTAINER(widget)))
      return;
...
/* Called when a child is added to the menu_bar */
static void _etk_menu_bar_child_added_cb(Etk_Object *object, void *child, void *data)
{
   Etk_Object *item;
   ...
   etk_widget_theme_set(ETK_WIDGET(item), ETK_WIDGET(item)->theme_file, "menu_bar_item");
   etk_signal_connect("mouse_up", item, ETK_CALLBACK(_etk_menu_bar_item_mouse_up_cb), NULL);
   etk_signal_connect("selected", item, ETK_CALLBACK(_etk_menu_bar_item_selected_cb), NULL);
   etk_signal_connect("deselected", item, ETK_CALLBACK(_etk_menu_bar_item_deselected_cb), NULL);
   etk_signal_connect("submenu_popped_down", item, ETK_CALLBACK(_etk_menu_bar_item_submenu_popped_down_cb), NULL);
}

/* Called when a child is removed from the menu_bar */
static void _etk_menu_bar_child_removed_cb(Etk_Object *object, void *child, void *data)
{
   Etk_Object *item;
   ...
   etk_signal_disconnect("mouse_up", item, ETK_CALLBACK(_etk_menu_bar_item_mouse_up_cb));
   etk_signal_disconnect("selected", item, ETK_CALLBACK(_etk_menu_bar_item_selected_cb));
   etk_signal_disconnect("deselected", item, ETK_CALLBACK(_etk_menu_bar_item_deselected_cb));
   etk_signal_disconnect("submenu_popped_down", item, ETK_CALLBACK(_etk_menu_bar_item_submenu_popped_down_cb));
}
...
static int _etk_menu_bar_mouse_move_cb(void *data, int type, void *event)
{
   ...
   for (l = ETK_CONTAINER(menu_bar)->children; l; l = l->next)
   {
