...
static void __free_evas_objects(Etox e);
static void __string_create_individual_evas_objects(Etox e,
						    Etox_Object_Bit bit);
static void __tab_create_individual_evas_objects(Etox e, Etox_Object_Bit bit);
...
static int
__etox_get_next_obstacle_to_the_right(Etox e, Etox_Data data,
				      double x, int *inside)
{
  Etox_Obstacle obst;
  int i;
  ...
  for (i = 0; i <= _etox_data_get_size(data); i++)
    {
      obst = (Etox_Obstacle) _etox_data_get(data, i);
  ...
  *y += e->etox_objects.h + e->padding;
  ...
  if (e->obstacles && !ewd_list_is_empty(e->obstacles))
	...
	return obj;
      ...
      data = _etox_data_new(ewd_list_nodes(obst_list) - 1);
      _etox_data_set_from_list(data, obst_list);
      _etox_data_sort(data, 0, _etox_data_get_size(data),
		      __compare_obstacles);
      i = __etox_get_next_obstacle_to_the_right(e, data, obj->x, &inside);
      if (i >= 0)
	{
	  obst = (Etox_Obstacle) _etox_data_get(data, i);
  ...
  while ((style_bit = (Etox_Style_Bit) ewd_list_next(string->style->bits)))
    {
      ...
      if (string->callback)
	_etox_callback_create(e, string->callback, ev_obj);
...
__tab_create_individual_evas_objects(Etox e, Etox_Object_Bit bit)
{
  ...
  if (tab->callback)
    _etox_callback_create(e, tab->callback, ev_obj);
...
_etox_create_evas_objects(Etox e)
{
  ...
  /* cleanup 'callback-junk'.. */
  if (e->callback_data)
    {
      ewd_list_destroy(e->callback_data);
      e->callback_data = NULL;
    }

  __free_evas_objects(e);
