...
ecore_mouse_down(Ecore_Event * ev)
{
  ...
  evas_set_color(evas, obj, 0, 0, 0, 100);
...
setup(void)
{
  ...
  evas =
    evas_new_all(ecore_display_get(), win, 0, 0, 400, 400, render_method,
...
main(int argc, char *argv[])
{
  ...
  obst = etox_obstacle_add(e, obstacle_x, obstacle_y, obstacle_w, obstacle_h);
  ...
  etox_set_text(e, ET_ALIGN(ETOX_ALIGN_TYPE_CENTER, ETOX_ALIGN_TYPE_LEFT),
		...
		ET_TEXT
		("As a result of meeting requests from users, Enlightenment over "),
		ET_TEXT
		("time has done some nasty hacks,\nbut now for the development of "),
		ET_TEXT
		("version 0.17.0, we have moved a lot of the design and core code "),
		ET_COLOR(c),
		ET_TEXT
		("into various subsystems than generalize some back end and let us "),
		ET_COLOR_END,
		ET_TEXT
		("not only use it in Enlightenment itself, but make this work "),
		ET_TEXT
		("available to everyone to use as they want. The result is various "),
		ET_ALIGN_END, ET_STYLE_END, ET_FONT("cinema", 8),
		ET_TEXT
		("lower level libraries that do lots of useful stuff for you and "
