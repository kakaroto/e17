...
void mouse_up(void *_data, Evas _e, Evas_Object _o, int _b, int _x, int _y)
{
	...
	ob = (Etox_Obstacle *) _data;

	if (_b == 2) {
		etox_obstacle_remove(et, ob);
		evas_hide(_e, _o);
		return;
	}
	...
	etox_obstacle_move(et, ob, ox + x - down_x, oy + y - down_y);
}
...
int main(int argc, char *argv[])
{
	...
	e = evas_new_all(disp, win, 128, 0, win_w, win_h, method,
			 216, 8 * 1024 * 1024, 8 * 1024 * 1024, FNTDIR);
...
*/
	et = etox_new_all(e, 10, 0, win_w - 20, win_h, 255, ETOX_ALIGN_LEFT);
	...
	etox_set_text(et, "The Etox Test Program\n"
		      "Etox is a text layout abstraction, built "
		      ...
		      "great. "
		      "E17 is definitely being worked on - it's just "
			...
			case ButtonPress:
				{
					...
					evas_event_button_down(e, mouse_x,
							       mouse_y,
							       button);
				}
				break;
			case ButtonRelease:
				{
					...
					mouse_x = ev.xbutton.x;
					mouse_y = ev.xbutton.y;
					evas_event_button_up(e, mouse_x,
							     mouse_y,
							     button);
				}
				break;
			case MotionNotify:
				{
					...
					evas_event_move(e, mouse_x,
							mouse_y);
				}
				break;
			case Expose:
				{
					evas_update_rect(e, ev.xexpose.x,
							 ev.xexpose.y,
							 ev.xexpose.width,
							 ev.xexpose.
							 height);
