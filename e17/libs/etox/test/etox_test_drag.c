...
void
etox_mouse_in(void *_data, Etox_Callback _cb, Etox _e, int _b, int _x, int _y)
{
  Etox_Part part = NULL;

  if (!(part = (Etox_Part) _data))
    return;
  etox_part_set_from_callback(_e, part, _cb);
  etox_part_prepend(_e, part, ET_FONT("cinema", 20), ET_END);
  etox_part_append(_e, part, ET_FONT_END, ET_END);
}

void
etox_mouse_out(void *_data, Etox_Callback _cb, Etox _e, int _b, int _x,
	       int _y)
{
  Etox_Part part = NULL;
  ...
  if (!(part = (Etox_Part) _data))
    return;
  etox_part_remove_changes(_e, part);
}
...
main(int argc, char *argv[])
{
  ...
  e = evas_new_all(disp, win, 128, 0, win_w, win_h, method,
		   216, 8 * 1024 * 1024, 2 * 1024 * 1024, FNTDIR);
  ...
  cb = etox_callback_new();
  etox_callback_add(cb, ETOX_CALLBACK_TYPE_MOUSE_IN, etox_mouse_in, part);
  etox_callback_add(cb, ETOX_CALLBACK_TYPE_MOUSE_OUT, etox_mouse_out, part);
