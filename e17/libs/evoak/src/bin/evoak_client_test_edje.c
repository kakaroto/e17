...
{
   evoak_object_move(o_edje, x, y);
   evoak_object_resize(o_edje, w, h);
}

static void
rais(void)
{
   evoak_object_raise(o_edje);
}
...
setup(Evoak *e)
{
   ...
   o = evoak_object_edje_add(e);
   if (!evoak_object_edje_file_set(o, PACKAGE_DATA_DIR"/data/e_logo.eet", "test"))
     {
	printf("ERROR setting edje file & collection\n");
     }
   evoak_object_show(o);
   o_edje = o;
...
main(int argc, char **argv)
{
   if (evoak_init())
	...
	ev = evoak_connect(NULL, "evoak_test_edje", "custom");
	if (ev)
	  {
