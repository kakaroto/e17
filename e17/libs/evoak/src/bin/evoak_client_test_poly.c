...
{
   evoak_object_move(o_poly, x, y);
   evoak_object_resize(o_poly, w, h);
}

static void
rais(void)
{
   evoak_object_raise(o_poly);
}
...
mouse_down(void *data, Evoak *e, Evoak_Object *obj, void *event_info)
{
   ...
   if (ev->button == 1)
	...
	evoak_object_polygon_points_clear(o_poly);
	n = 3 + (rand() % 10);
	for (i = 0; i < n; i++)
	  {
	     int x, y;
	     ...
	     x = oo_x + (rand() % oo_w);
	     y = oo_y + (rand() % oo_h);
	     evoak_object_polygon_point_add(o_poly, x, y);
	  }
	evoak_object_color_set(o_poly, 
			       rand() & 0xff,
			       rand() & 0xff,
			       rand() & 0xff,
			       rand() & 0xff);
...
main(int argc, char **argv)
{
   if (evoak_init())
	...
	ev = evoak_connect(NULL, "evoak_test_poly", "custom");
	if (ev)
	  {
