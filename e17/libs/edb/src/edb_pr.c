...
#ifndef lint
static const char sccsid[] = "@(#)edb_pr.c	10.40 (Sleepycat) 11/22/98";
#endif /* not lint */
...
static void __edb_proff __P((void *));
static void __edb_psize __P((DB_MPOOLFILE *));

/*
 * __edb_loadme --
 *	Force loading of this file.
 *
 * PUBLIC: void __edb_loadme __P((void));
 */
void
__edb_loadme()
{
 ...
 *
 * PUBLIC: FILE *__edb_prinit __P((FILE *));
 */
FILE *
__edb_prinit(fp)
	FILE *fp;
 ...
 *
 * PUBLIC: int __edb_dump __P((DB *, char *, int));
 */
int
__edb_dump(edbp, name, all)
	DB *edbp;
	char *name;
	int all;
{
	...
	if (name != NULL) {
		...
		fp = __edb_prinit(NULL);

	(void)__edb_predb(edbp);
	if (edbp->type == DB_HASH)
		(void)__edb_prhash(edbp);
	else
		(void)__edb_prbtree(edbp);
	fprintf(fp, "%s\n", DB_LINE);
	__edb_prtree(edbp->mpf, all);
	...
	DB *edbp;
{
	...
	fprintf(fp, "%s ", t);
	__edb_prflags(edbp->flags, fn, fp);
	fprintf(fp, "\n");
 ...
 *
 * PUBLIC: int __edb_prbtree __P((DB *));
 */
int
__edb_prbtree(edbp)
	DB *edbp;
{
	...
	i = PGNO_METADATA;
	if ((ret = memp_fget(edbp->mpf, &i, 0, (PAGE **)&mp)) != 0) {
	...
	for (i = mp->free, cnt = 0, sep = ", "; i != PGNO_INVALID;) {
		if ((ret = memp_fget(edbp->mpf, &i, 0, &h)) != 0)
			return (ret);
		i = h->next_pgno;
	...
	__edb_prflags(mp->flags, mfn, fp);
	(void)fprintf(fp, "\n");
	(void)memp_fput(edbp->mpf, mp, 0);
 ...
 *
 * PUBLIC: int __edb_prhash __P((DB *));
 */
int
__edb_prhash(edbp)
	DB *edbp;
{
	 ...
	 * to pass edbc's to this routine instead, then it could be.
	 */
	if (hcp->hdr == NULL) {
		pgno = PGNO_METADATA;
		if ((ret = memp_fget(edbp->mpf, &pgno, 0, &hcp->hdr)) != 0)
			return (ret);
		put_page = 1;
	} else
		put_page = 0;
	...
	(void)fflush(fp);
	if (put_page) {
		(void)memp_fput(edbp->mpf, (PAGE *)hcp->hdr, 0);
		hcp->hdr = NULL;
	}
	return (edbc->c_close(edbc));
}
 ...
 *
 * PUBLIC: int __edb_prtree __P((DB_MPOOLFILE *, int));
 */
int
__edb_prtree(mpf, all)
	DB_MPOOLFILE *mpf;
	int all;
{
	PAGE *h;
	...
	for (i = PGNO_ROOT;; ++i) {
		if (memp_fget(mpf, &i, 0, &h) != 0)
			break;
		(void)__edb_prpage(h, all);
		(void)memp_fput(mpf, h, 0);
	}
	(void)fflush(__edb_prinit(NULL));
	return (0);
}
 ...
 *
 * PUBLIC: int __edb_prnpage __P((DB_MPOOLFILE *, edb_pgno_t));
 */
int
__edb_prnpage(mpf, pgno)
	DB_MPOOLFILE *mpf;
	edb_pgno_t pgno;
{
	PAGE *h;
 ...
 *	-- Print out a page.
 *
 * PUBLIC: int __edb_prpage __P((PAGE *, int));
 */
int
__edb_prpage(h, all)
	PAGE *h;
	int all;
{
	...
	if (TYPE(h) == P_OVERFLOW) {
		...
		__edb_pr((u_int8_t *)h + P_OVERHEAD, OV_LEN(h));
		return (0);
	}
	...
	for (i = 0; i < NUM_ENT(h); i++) {
		...
		case P_HASH:
			...
			case H_OFFDUP:
				memcpy(&pgno,
				    HOFFDUP_PGNO(hk), sizeof(edb_pgno_t));
				fprintf(fp,
				    "%4lu [offpage dups]\n", (u_long)pgno);
				break;
			case H_DUPLICATE:
				...
				for (p = HKEYDATA_DATA(hk),
				    ep = p + len; p < ep;) {
					memcpy(&dlen, p, sizeof(edb_indx_t));
					p += sizeof(edb_indx_t);
					fprintf(fp, "\t\t");
					__edb_pr(p, dlen);
					p += sizeof(edb_indx_t) + dlen;
				}
				break;
			case H_KEYDATA:
				if (i != 0)
					__edb_pr(HKEYDATA_DATA(hk),
					    LEN_HKEYDATA(h, 0, i));
				else
					fprintf(fp, "%s\n", HKEYDATA_DATA(hk));
				break;
 ...
 *
 * PUBLIC: int __edb_isbad __P((PAGE *, int));
 */
int
__edb_isbad(h, die)
	PAGE *h;
 ...
 *
 * PUBLIC: void __edb_pr __P((u_int8_t *, u_int32_t));
 */
void
__edb_pr(p, len)
	u_int8_t *p;
 ...
 *
 * PUBLIC: int __edb_predbt __P((DBT *, int, FILE *));
 */
int
__edb_predbt(edbtp, checkprint, fp)
	DBT *edbtp;
	int checkprint;
	FILE *fp;
{
	 ...
	 * used by edb_dump(1) and edb_load(1).  This means that the format
	 * cannot change.
	 */
	if (checkprint) {
		for (len = edbtp->size, p = edbtp->data; len--; ++p)
			if (isprint(*p)) {
				if (*p == '\\' && fprintf(fp, "\\") != 1)
					return (EIO);
				if (fprintf(fp, "%c", *p) != 1)
 ...
 *
 * PUBLIC: void __edb_prflags __P((u_int32_t, const FN *, FILE *));
 */
void
__edb_prflags(flags, fn, fp)
	u_int32_t flags;
	FN const *fn;
	FILE *fp;
{
