...
static const char sccsid[] = "@(#)edb_apprec.c	10.33 (Sleepycat) 10/5/98";
#endif
 ...
 *	Perform recovery.
 *
 * PUBLIC: int __edb_apprec __P((DB_ENV *, u_int32_t));
 */
int
__edb_apprec(edbenv, flags)
	DB_ENV *edbenv;
	u_int32_t flags;
{
	DBT data;
	...
	if ((ret = __edb_txnlist_init(&txninfo)) != 0)
		return (ret);
...
first:		if ((ret = log_get(lp, &ckp_lsn, &data, DB_FIRST)) != 0) {
			if (ret == DB_NOTFOUND)
				...
				__edb_err(edbenv, "First log record not found");
			goto out;
		}
		open_lsn = ckp_lsn;
	} else if ((ret = __txn_ckp_read(data.data, &ckp_args)) != 0) {
		__edb_err(edbenv, "Invalid checkpoint record at [%ld][%ld]\n",
	...
	lsn = open_lsn;
	for (;;) {
		if (edbenv->tx_recover != NULL)
			ret = edbenv->tx_recover(lp,
			    &data, &lsn, TXN_OPENFILES, txninfo);
		else
			ret = __edb_dispatch(lp,
			    &data, &lsn, TXN_OPENFILES, txninfo);
	...
	if (edbenv->edb_verbose)
		__edb_err(lp->edbenv, "Recovery starting from [%lu][%lu]",
	...
	/* Now close all the edb files that are open. */
	 ...
	 */
	(void)time(&now);
	edbenv->tx_info->region->last_ckp = ckp_lsn;
	edbenv->tx_info->region->time_ckp = (u_int32_t)now;
	if ((ret = txn_checkpoint(edbenv->tx_info, 0, 0)) != 0)
		goto out;
	edbenv->tx_info->region->last_txnid = TXN_MINIMUM;

	if (edbenv->edb_verbose) {
		__edb_err(lp->edbenv, "Recovery complete at %.24s", ctime(&now));
		__edb_err(lp->edbenv, "%s %lx %s [%lu][%lu]",
		    "Maximum transaction id",
		    ((DB_TXNHEAD *)txninfo)->maxid,
		    "Recovery checkpoint",
		    (u_long)edbenv->tx_info->region->last_ckp.file,
		    (u_long)edbenv->tx_info->region->last_ckp.offset);
	}

	if (0) {
msgerr:		__edb_err(edbenv, "Recovery function for LSN %lu %lu failed",
