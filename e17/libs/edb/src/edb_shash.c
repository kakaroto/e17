...
#ifndef lint
static const char sccsid[] = "@(#)edb_shash.c	10.9 (Sleepycat) 4/10/98";
#endif /* not lint */
 ...
 *
 * PUBLIC: int __edb_tablesize __P((u_int32_t));
 */
int
__edb_tablesize(n_buckets)
	u_int32_t n_buckets;
{
	int i;
 ...
 *
 * PUBLIC: void __edb_hashinit __P((void *, u_int32_t));
 */
void
__edb_hashinit(begin, nelements)
	void *begin;
	u_int32_t nelements;
{
	u_int32_t i;
