...
static void       read_tiles_into_data(Tile* tiles, int num_rows, int num_cols, int width, int height, int bpp, DATA8** data);
...
*/
extern void blend_pixels (DATA8* src, int src_w, int src_h, DATA8* dest, int dest_w, int dest_h, int dest_x, int dest_y);
/* ... */
...
xcf_load_layer(void)
{
  ...
  read_tiles_into_data(layer->tiles, layer->num_rows, layer->num_cols,
...
static void
read_tiles_into_data(Tile* tiles, int num_rows, int num_cols,
		     int width, int height, int bpp, DATA8** data_p)
{
  int b, tile_x, tile_y, k, x, y, offset_x, offset_y;
  DATA8* data;
  DATA8* ptr;
  DATA8* ptr2;
  Tile*  t;
		   ...
		   int     data_length)
{
  ...
  DATA8 *xcfdata, *xcfodata, *xcfdatalimit, *dump;
