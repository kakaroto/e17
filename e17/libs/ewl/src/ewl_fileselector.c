...
__ewl_fileselector_realize(Ewl_Widget * w, void *ev_data, void *user_data)
{
	...
	printf("home: %s\n", home);
	__process_directory(fs, getenv("HOME"));

	ewl_widget_realize(fs->files);
	ewl_widget_realize(fs->dirs);
	ewl_widget_realize(fs->fbox);
	ewl_widget_realize(fs->dbox);

	DLEAVE_FUNCTION(DLEVEL_STABLE);
}

/*
 * When a configure event occurs, configure the fileselector's children.
 */
void
__ewl_fileselector_configure(Ewl_Widget * w, void *ev_data, void *user_data)
{
	...
	printf("in fs configure\n");

	fs = EWL_FILESELECTOR(w);

	ewl_widget_configure(fs->dbox);
	ewl_widget_configure(fs->fbox);
...
__process_directory(Ewl_Fileselector * fs, char *directory)
{
	...
	/* add 2 to dir_count for '.' and '..' */
	d_count += 2;


	printf("\n\ndone counting files in directory\n\n");
	...
	if (fs->dirs) {
		printf("dir table exists\n");
		ewl_table_reset(EWL_TABLE(fs->dirs), 1, d_count, dir_head);
	} else {
		printf("creating new dir table\n");
	...
	if (fs->files) {
		printf("file table exists\n");
		ewl_table_reset(EWL_TABLE(fs->files), 1, f_count, file_head);
	} else {
		printf("creating new files table\n\n");
	...
	ewl_callback_append(EWL_WIDGET(tc), EWL_CALLBACK_CLICKED,
			    __directory_clicked, fs);

	ewl_widget_set_data(tc->widget, (void *) tc, (int *) d_count);

	d_count--;
	printf("d_count should now be one: %d\n", d_count + 1);
	strcpy(fs->d_info[d_count].name, ".");
	strcpy(fs->d_info[d_count].path, dir);
	...
	printf("after callback append\n");

	ewl_widget_set_data(tc->widget, (void *) tc, (int *) d_count);

	printf("after set data\n");
