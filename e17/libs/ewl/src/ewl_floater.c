...
void ewl_floater_set_position(Ewl_Floater * f, int x, int y)
{
	...
	if (f->follows)
		...
		ewl_object_request_position(EWL_OBJECT(f), f->x, f->y);
...
ewl_floater_parent_configure_cb(Ewl_Widget * w, void *ev_data, void *user_data)
{
	 ...
	 */
	if (f->follows) {
		x = ewl_object_get_current_x(EWL_OBJECT(f->follows)) + f->x;
		y = ewl_object_get_current_y(EWL_OBJECT(f->follows)) + f->y;
	} else {
		x = f->x;
		y = f->y;
	}
