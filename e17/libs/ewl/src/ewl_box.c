...
__ewl_hbox_configure_normal(Ewl_Widget * w, int *rh)
{
	...
	*rh = REQUEST_H(w) - t - b;
	...
	while ((c = ewd_list_next(EWL_CONTAINER(w)->children)) != NULL)
	  {
		  if (EWL_OBJECT(w)->fill & EWL_FILL_POLICY_FILL)
		    {
			    if (!f)
				    f = ewd_list_new();
		  ...
		  if (MAXIMUM_W(c) > CURRENT_W(w))
			  REQUEST_W(c) = CURRENT_W(w) - l - r;
...
__ewl_hbox_configure_fillers(Ewl_Widget * w, Ewd_List * f, int rh)
{
	...
	while ((c = ewd_list_next(f)) != NULL)
	  {
		  if (MAXIMUM_W(c) < REQUEST_W(w) - ll - rr)
			  REQUEST_W(c) = MAXIMUM_W(w);
		  else
			  REQUEST_W(c) = REQUEST_W(w) - ll - rr;
		  ...
		  if (EWL_OBJECT(c)->align == EWL_ALIGNMENT_LEFT)
		  ...
		  else if (EWL_OBJECT(c)->align == EWL_ALIGNMENT_RIGHT)
		    {
			    REQUEST_X(c) =
                    ...
                    {
                            REQUEST_X(c) = REQUEST_X(w);
                            REQUEST_X(c) +=
                                    (REQUEST_W(w) / 2) -
                                    (REQUEST_W(c)) / 2;
                    }

		  REQUEST_Y(c) = y + box->spacing;
		  y += REQUEST_H(c)  +box->spacing;
