...
void __ewl_entry_update_cursor_pos(Ewl_Widget * w, void * ev_data, void * user_data);
...
__ewl_entry_key_down(Ewl_Widget * w, void *ev_data, void *user_data)
{
	...
	if (!strcmp(ev->key, "Left")) __ewl_entry_move_cursor_to_left(w);
	else if (!strcmp(ev->key, "Right")) __ewl_entry_move_cursor_to_right(w);
	else if (!strcmp(ev->key, "Home")) __ewl_entry_move_cursor_to_home(w);
	else if (!strcmp(ev->key, "End")) __ewl_entry_move_cursor_to_end(w);
	else if (!strcmp(ev->key, "BackSpace")) __ewl_entry_delete_to_left(w);
	else if (!strcmp(ev->key, "Delete")) __ewl_entry_delete_to_right(w);
	else if (!strcmp(ev->key, "Return")) {}
	else if (!strcmp(ev->key, "KP_Enter")) {}
	else if (ev->compose) __ewl_entry_insert_text(w, ev->compose);
...
__ewl_entry_theme_update(Ewl_Widget * w, void * ev_data, void * user_data)
{
	...
	size = (int) ewl_theme_data_get(w,"/appearance/entry/default/text/font_size");
...
__ewl_entry_update_cursor_pos(Ewl_Widget * w, void * ev_data, void * user_data)
{
	...
	if (EWL_WIDGET(e)->ebits_object)
		ebits_get_insets(EWL_WIDGET(e)->ebits_object,
		    		0, 0, &tt, &bb);
