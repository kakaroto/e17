...
static Ewl_View *ewl_tree2_view_freebox_view = NULL;
static Ewl_Widget *ewl_tree2_view_freebox_cb_widget_fetch(void *data,
							unsigned int col,
							unsigned int row);
 ...
 */
Ewl_View *
ewl_tree2_view_freebox_get(void)
{
	DENTER_FUNCTION(DLEVEL_STABLE);

	if (!ewl_tree2_view_freebox_view)
	{
		ewl_tree2_view_freebox_view = ewl_view_new();
		ewl_view_widget_fetch_set(ewl_tree2_view_freebox_view,
				ewl_tree2_view_freebox_cb_widget_fetch);
	}

	DRETURN_PTR(ewl_tree2_view_freebox_view, DLEVEL_STABLE);
}

static Ewl_Widget *
ewl_tree2_view_freebox_cb_widget_fetch(void *data __UNUSED__,
					...
					unsigned int row __UNUSED__)
{
	...
	tree = ewl_tree2_view_freebox_new();
 ...
 * @brief Creates and initializes a new Ewl_Tree2_View_Freebox widget
 */
Ewl_Widget *
ewl_tree2_view_freebox_new(void)
{
	Ewl_Widget *w;

	DENTER_FUNCTION(DLEVEL_STABLE);

	w = NEW(Ewl_Tree2_View_Freebox, 1);
	if (!w)
		DRETURN_PTR(NULL, DLEVEL_STABLE);

	if (!ewl_tree2_view_freebox_init(EWL_TREE2_VIEW_FREEBOX(w)))
	{
 ...
 */
int
ewl_tree2_view_freebox_init(Ewl_Tree2_View_Freebox *tv)
{
	DENTER_FUNCTION(DLEVEL_STABLE);
