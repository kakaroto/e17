...
void __ewl_selectionbar_configure(Ewl_Widget * w, void *ev_data, void *user_data);
void __ewl_selectionbar_parent_configure(Ewl_Widget *w, void *ev_data, void *user_data);
...
void __mouse_move_over_children(Ewl_Widget *w, void *user_data, void *ev_data);
...
void
__ewl_selectionbar_configure(Ewl_Widget * w, void *ev_data,
			     void *user_data)
{
	Ewl_Selectionbar *s;
	Ewl_Widget *child;
...
void
__ewl_selectionbar_parent_configure(Ewl_Widget *w, void *ev_data, void *user_data)
{
	Ewl_Selectionbar *s;
	Ewl_Object *o;
...
void __open_bar_cb(int val, void *ev_data)
{
	...
	if(CURRENT_H(o) < s->h) {
		...
		ecore_add_event_timer(open_string, 0.01, __open_bar_cb, ++val, w);
...
void __close_bar_cb(int val, void *ev_data)
{
	...
	if(CURRENT_H(o) > 10) {
		ewl_object_request_size(o, s->w, CURRENT_H(o)-(s->h/10)+5);
		ewl_object_request_size(so, s->w, CURRENT_H(o)-(s->h/10)+5);
		
		ecore_add_event_timer(close_string, 0.01, __close_bar_cb, ++val, w);
