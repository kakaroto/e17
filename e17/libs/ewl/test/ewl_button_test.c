...
void
__destroy_button_test_window(Ewl_Widget * w, void *ev_data,
			     void *user_data)
{
...
__create_button_test_window(Ewl_Widget * w, void *ev_data, void *user_data)
{
	...
	ewl_object_set_alignment(EWL_OBJECT(button[0]),
				 EWL_ALIGNMENT_LEFT);
	...
	ewl_object_set_alignment(EWL_OBJECT(button[1]),
				 EWL_ALIGNMENT_LEFT);
	...
	ewl_container_append_child(EWL_CONTAINER(main_vbox),
				   check_button[0]);
