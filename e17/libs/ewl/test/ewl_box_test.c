...
__toggle_child_fill_policy(Ewl_Widget * w, void *ev_data, void *user_data)
{
	...
	if (f == EWL_FILL_POLICY_NORMAL) {
		...
		ewl_object_set_maximum_size(EWL_OBJECT(w), 1 << 30,
					    1 << 30);
...
void
__toggle_child_horizontal_align(Ewl_Widget * w, void *ev_data,
				void *user_data)
{
	Ewl_Alignment a;
	char l[10];
...
void
__toggle_child_vertical_align(Ewl_Widget * w, void *ev_data,
			      void *user_data)
{
	Ewl_Alignment a;
	char l[10];
...
__create_box_test_window(Ewl_Widget * w, void *ev_data, void *user_data)
{
	...
	ewl_container_append_child(EWL_CONTAINER(vbox[0]),
				   vbox_button[0][0]);
	...
	ewl_container_append_child(EWL_CONTAINER(vbox[1]),
				   vbox_button[1][0]);
	...
	ewl_container_append_child(EWL_CONTAINER(hbox[1]),
				   hbox_button[0][0]);
	...
	ewl_container_append_child(EWL_CONTAINER(hbox[2]),
				   hbox_button[1][0]);
