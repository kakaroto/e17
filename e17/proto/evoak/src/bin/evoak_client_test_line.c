...
{
   evoak_object_move(o_line, x, y);
   evoak_object_resize(o_line, w, h);
}

static void
rais(void)
{
   evoak_object_raise(o_line);
}
...
mouse_down(void *data, Evoak *e, Evoak_Object *obj, void *event_info)
{
   ...
   if (ev->button == 1)
     {
	evoak_object_line_xy_set(o_line, 
				 oo_x + (rand() % oo_w), oo_y + (rand() % oo_h),
				 oo_x + (rand() % oo_w), oo_y + (rand() % oo_h));
	evoak_object_color_set(o_line, 
			       rand() & 0xff,
			       rand() & 0xff,
			       rand() & 0xff,
			       rand() & 0xff);
...
main(int argc, char **argv)
{
   if (evoak_init())
	...
	ev = evoak_connect(NULL, "evoak_test_line", "custom");
	if (ev)
	  {
