...
static Ecore_List *_fdo_paths_get(char *before, char *env_home, char *env, char *env_home_default, char *env_default, char *type,
...
static void _fdo_paths_exec_config(Ecore_List * paths, char *home, Ecore_List * extras, char *cmd);
...
fdo_paths_init()
{
   ...
   exit_handler = ecore_event_handler_add(ECORE_EXE_EVENT_DEL, _fdo_paths_cb_exe_exit, NULL);
   ...
   if (exit_handler && gnome_data)
      {
         ...
         _fdo_paths_exec_config(gnome_data, home, NULL, "gnome-config --datadir");
      }
	      ...
	      if (temp_list)
	         {
                    ...
                    while ((path = ecore_list_next(fdo_paths_kde_legacy)) != NULL)
              ...
              if (stat(temp, &path_stat) == 0)
                {
                 ...
                 path = fdo_paths_recursive_search(this_path, file, NULL, func, data);
              if (path && (!func))
                 break;
           }
      }
               ...
               char *kde)
{
   ...
   if (gnome_extra)	gnome_extras = ecore_list_from_paths(strdup(gnome_extra));
                  ...
                  while ((this_env = ecore_list_next(env_list)) != NULL)
                    {
                       ...
                       while ((this_type = ecore_list_next(types)) != NULL)
                         {
                            _fdo_paths_massage_path(path, home, this_env, this_type);
                            _fdo_paths_check_and_add(paths, path);
                         }
                    }
            ...
            while ((this_gnome = ecore_list_next(gnome_data)) != NULL)
               {
                  ...
                  while ((this_type = ecore_list_next(types)) != NULL)
                     {
                        _fdo_paths_massage_path(path, home, this_gnome, this_type);
                        _fdo_paths_check_and_add(paths, path);
                     }
               }
         }
...
static void
_fdo_paths_exec_config(Ecore_List *paths, char *home, Ecore_List * extras, char *cmd)
{
   Ecore_Exe *exe;
                       ...
                       if (ced->types)
                         {
                            ...
                            while ((this_type = ecore_list_next(ced->types)) != NULL)
                              {
                                 _fdo_paths_massage_path(path, ced->home, this_config, this_type);
                                 _fdo_paths_check_and_add(paths, path);
                              }
                         }
                       else
                         {
                            _fdo_paths_massage_path(path, ced->home, this_config, NULL);
                            _fdo_paths_check_and_add(paths, path);
                         }
                    }
