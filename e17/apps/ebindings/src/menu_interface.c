...
add_menu_notebook(GtkWidget *w, GtkWidget *note, int sheet)
{
	...
	gtk_table_attach(GTK_TABLE(table), elabel, 0, 1, 2, 3,
					GTK_FILL, 0, 2, 2);

	dentry = gtk_entry_new();
	gtk_widget_ref(dentry);
	gtk_widget_set_name(dentry, "desc_entry");
	gtk_table_attach(GTK_TABLE(table), dentry, 1, 3, 0, 1,
							GTK_FILL, 0, 2, 2);
	...
	gtk_table_attach(GTK_TABLE(table), ientry, 1, 2, 1, 2,
							GTK_FILL, 0, 2, 2);
	...
	gtk_table_attach(GTK_TABLE(table), ichange, 2, 3, 1, 2,
							GTK_FILL, 0, 2, 2);

	eentry = gtk_entry_new();
	gtk_widget_ref(eentry);
	gtk_widget_set_name(eentry, "exec_entry");
	gtk_table_attach(GTK_TABLE(table), eentry, 1, 3, 2, 3,
							GTK_FILL, 0, 2, 2);
	...
	gtk_signal_connect(GTK_OBJECT(ctree), "tree_select_row",
					GTK_SIGNAL_FUNC(menu_ctree_select_row_cb),
					NULL);
	/* conatiner is setup */
...
menu_action_button_clicked(GtkWidget *w, gpointer data)
{
	...
	if(!strcmp(wname, "new_menu_item_button"))	{
		...
		/* data is the ctree from the menu tab =) */
...
menu_ctree_delete_row_selected(GtkWidget *w)
{
	if((w) && (GTK_CLIST(GTK_CTREE(w))->selection))	{
		...
		if(next) gtk_ctree_select(GTK_CTREE(w), next);
	} else	{
							...
							GtkCTreeNode *cnode, gpointer data)
{
	...
	gtk_ctree_get_node_info(GTK_CTREE(tree), GTK_CTREE_NODE(cnode),
						&fields[0], NULL, NULL, NULL, NULL, NULL, NULL, NULL);
