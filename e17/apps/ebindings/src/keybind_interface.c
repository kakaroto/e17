...
add_keybinds_notebook(GtkWidget *w, GtkWidget *note, int sheet)	
{
	...
	gtk_table_attach(GTK_TABLE(table), changeb, 2, 3, 0, 1, 
							GTK_FILL, 0, 2, 2);
	...
	gtk_table_attach(GTK_TABLE(table), o_menu, 1, 3, 1, 2, GTK_FILL, 0,
					2, 2);
	...
	while((e = (eaction_item*)ewd_list_next(action_container.keys)))	{
		...
		FREE(key_fields[2]);
		FREE(key_fields[3]);
		if(!e->name) e->name = strdup("Key_Binding");
...
keybind_save_button_clicked(GtkWidget *w, gpointer data)
{
	...
	for(i = 0; i < k_entry.count; i++) {
		...
		else e->action = strdup(tmp);
		tmp = NULL;

		gtk_clist_get_text(GTK_CLIST(k_entry.list), i, 3, &tmp);
		if((!strcmp("Execute", e->action)) || 
			(!strcmp("Desktop", e->action)))	{
			if(tmp)	
				e->params = strdup(tmp);
			else
				fprintf(stderr, "Actions need parameters\n");
		}
		else e->params = strdup("");
		tmp = NULL;
...
keybind_parameter_entry_changed(GtkWidget *w, gpointer data)
{
	...
	if(node)	{
		 ...
		 * FIXME: g_strdup leak ?
		 */
		gtk_clist_set_text(GTK_CLIST(data), k_entry.current_row, 3,
							new_val);
...
keybind_add_button_clicked(GtkWidget *w, gpointer data)
{
	...
	retval = gtk_clist_insert(GTK_CLIST(data), 
							(k_entry.current_row +1), fields);
