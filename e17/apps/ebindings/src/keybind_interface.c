...
add_keybinds_notebook(GtkWidget * w, GtkWidget * note, int sheet)
{
   ...
   gtk_table_attach(GTK_TABLE(table), changeb, 2, 3, 0, 1, GTK_FILL, 0, 2,
		    2);
   ...
   gtk_table_attach(GTK_TABLE(table), paraml, 0, 1, 2, 3, GTK_FILL, 0, 2,
		    2);

   e_param = gtk_entry_new();
   gtk_table_attach(GTK_TABLE(table), e_param, 1, 3, 2, 3, GTK_FILL, 0, 2,
		    2);
