...
focus_mouse_autoraise_window(GtkWidget * w, gpointer data)
{
   ...
   if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(w)))
   {
      E_DB_INT_SET(buf, "/window/raise/auto", 1) e_db_flush();
   }
   else
   {
      E_DB_INT_SET(buf, "/window/raise/auto", 0) e_db_flush();
   }
...
focus_guides_location_change_cb(GtkWidget * w, gpointer data)
{
   ...
   {
      E_DB_FLOAT_SET(buf, "/guides/display/y", f) e_db_flush();
   }
   else
   {
...
add_focus_notebook(GtkWidget * w, GtkWidget * note, int sheet)
{
   ...
   follow_pointer =
      gtk_radio_button_new_with_label(NULL, "Focus Follows Pointer");
   ...
   gtk_table_attach(GTK_TABLE(table), follow_pointer, 0, 1, 0, 1, GTK_FILL, 0,
                    2, 2);
   ...
   gtk_table_attach(GTK_TABLE(table), hsep, 0, 2, 2, 3, GTK_FILL | GTK_EXPAND,
                    0, 2, 2);
   ...
   gtk_table_attach(GTK_TABLE(table), autoraise, 0, 1, 3, 4, GTK_FILL, 0, 2,
                    2);
   E_DB_INT_GET(buf, "/window/raise/auto", current_val,
                ok) gtk_signal_connect(GTK_OBJECT(autoraise), "toggled",
                                       GTK_SIGNAL_FUNC
                                       (focus_mouse_autoraise_window), NULL);
   if (current_val)
   ...
   /* Guides_x */
   guide_x_label =
      gtk_label_new("Horizontal Positioning (0 left most, "
                    "100 right most) ");
   ...
   /* Guides_y */
   guide_y_label =
      gtk_label_new("Vertical Positioning (0 top most, " "100 bottom most) ");
