...
focus_mouse_autoraise_window(GtkWidget *w, gpointer data)
{
	...
	if(gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(w)))	
		...
		E_DB_INT_SET(buf, "/window/raise/auto", 0)
...
focus_autoraise_timeout_cb(GtkWidget *w, gpointer data)	
{
	...
	snprintf(buf, 4096, "%s/.e/behavior/settings.db", getenv("HOME"));
	E_DB_FLOAT_SET(buf, "/window/raise/delay", f) 
...
void add_focus_notebook(GtkWidget *w, GtkWidget *note, int sheet)
{
	...
	follow_pointer = gtk_radio_button_new_with_label(NULL, 
											"Mouse Focus Follows Pointer");
	gtk_widget_set_name(GTK_WIDGET(follow_pointer), "follow_pointer");
	gtk_signal_connect(GTK_OBJECT(follow_pointer), "toggled", 
					GTK_SIGNAL_FUNC(focus_mouse_focus_type_cb),
					NULL);
	gtk_table_attach(GTK_TABLE(table), follow_pointer, 0, 1, 0, 1, GTK_FILL,
						0, 2, 2);
	
	follow_clicks = gtk_radio_button_new_with_label_from_widget(
										GTK_RADIO_BUTTON(follow_pointer), 
	...
	E_DB_INT_GET(buf, "/focus/mode", current_val, ok) 
	...
	/*  */
	autoraise = gtk_check_button_new_with_label(
			"Autoraise window on mouse focus switch");
	gtk_table_attach(GTK_TABLE(table), autoraise, 0, 1, 3, 4, GTK_FILL,
						0, 2, 2);
	E_DB_INT_GET(buf, "/window/raise/auto", current_val, ok) 
	gtk_signal_connect(GTK_OBJECT(autoraise), "toggled",
					GTK_SIGNAL_FUNC(focus_mouse_autoraise_window), NULL);
	if(current_val)	
	...
	gtk_table_attach(GTK_TABLE(table), scale_label, 0, 1, 4, 5, 0, 0,
					2, 2);
	
	/* */
	E_DB_FLOAT_GET(buf, "/window/raise/delay", flot, ok)
	if(ok)
	...
	gtk_table_attach(GTK_TABLE(table2), guide_x_label, 0, 1, 0, 1, GTK_FILL, 0,
						2, 2);
	E_DB_FLOAT_GET(buf, "/guides/display/x", _flot, ok)
	flot = (_flot * 100.00);
	if(ok)
	...
	gtk_table_attach(GTK_TABLE(table2), guide_x, 0, 1, 1, 2, GTK_FILL, 0,
						2, 2);
	...
	guide_y_label = gtk_label_new("Vertical Positioning (0 top most, "
								"100 bottom most) ");
	gtk_table_attach(GTK_TABLE(table2), guide_y_label, 1, 2, 0, 1, GTK_FILL, 0,
						2, 2);
	
	_flot = 0.00;
	E_DB_FLOAT_GET(buf, "/guides/display/y", _flot, ok)
	flot = (_flot * 100.00);
	if(ok)
	...
	gtk_table_attach(GTK_TABLE(table2), guide_y, 1, 2, 1, 2, GTK_FILL, 0,
						2, 2);
