...
void add_actions_extra_notebook(GtkWidget *w, GtkWidget *note, int sheet)
{
	...
	gtk_table_attach(GTK_TABLE(table), win_pos_l, 0, 1, 0, 1, GTK_FILL, 0,
					2, 2);

	o_menu = gtk_option_menu_new();
	gtk_widget_ref(o_menu);
	gtk_widget_set_name(o_menu, "win_pos_menu");
	gtk_table_attach(GTK_TABLE(table), o_menu, 1, 3, 0, 1, GTK_FILL, 0,
					2, 2);
	...
	gtk_widget_ref(o_menu);
	gtk_widget_set_name(o_menu, "mod_menu");
	gtk_table_attach(GTK_TABLE(table), o_menu, 1, 3, 4, 5, GTK_FILL, 0,
					2, 2);
