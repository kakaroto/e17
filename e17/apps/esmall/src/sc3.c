				   ...
				   value * lval1, value * lval2) 
{
		...
		if (lval1->ident == _constexpr) {	/* constant on left side; it is not yet loaded */
		...
		/*   is restricted to "add" and "subtract" operators */ 
			...
			if (lval2->ident == _constexpr) {	/* constant on rigth side */
			
				if (commutative(oper1)) {	/* test for commutative operators */
  ...
  restart:
		...
		if (matchtoken('[') || matchtoken('{') || matchtoken('(')) {
			...
			if (tok == '[' || tok == '{') {		/* subscript */
				...
				if (lval2.ident == _constexpr) {	/* constant expression */
...
static void setdefarray(cell * string, cell size) 
{
...
static void callfunction(symbol * sym) 
{
			...
			do {
				...
				if (matchtoken('_')) {
					...
					case _varargs:
						...
						} else if (lval.ident == _constexpr || lval.ident == _expression 
					...
					case _variable:
							...
							if (lval.ident == _label || lval.ident == _functn || lval.ident == _reffunc 
							||lval.ident == _array || lval.ident == _refarray)
	...
	/* check remaining function arguments (they may have default values) */ 
		for (argidx = 0; arg[argidx].ident != 0 && arg[argidx].ident != _varargs; argidx++) {
