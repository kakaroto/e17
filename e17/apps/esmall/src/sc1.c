...
sc_main (int argc, char **argv)
{
  ...
  setopt (argc, argv, inpfname, outfname, binfname, errfname, incfname,
	  includepath);
  if (strlen (errfname) != 0)
    unlink (errfname);		/* delete file on startup */
  else
    puts ("Small compiler " VERSION_STR
	  "\t\tCopyright (c) 1997-1999, ITB CompuPhase\n");
  setconfig (argv[0]);
	      ...
	      char *bname, char *ename, char *pname, char *incpath)
{
  ...
  for (arg = 1; arg < argc; arg++)
	...
	{
	  parserespf (&argv[arg][1], iname, oname, bname, ename, pname,
		      incpath);
	}
...
setconfig (char *root)
{
  ...
  if (strlen (includepath) == 0)
	...
	*ptr = '\0';
      ...
      if ((ptr = strrchr (includepath, DIRSEP_CHAR)) != NULL
	  || (ptr = strchr (includepath, ':')) != NULL)
	*(ptr + 1) = '\0';
      else
...
about (void)
{
  if (strlen (errfname) == 0)
	...
	("         -d1      [default] run-time checks, no symbolic information");
      ...
      puts
	("         -\\       use '\\' instead of '^' for control characters");
      ...
      if (isstatic)
	{
	  ...
	  sym =
	    addvariable (name, (cur_lit + glb_declared) * sizeof (cell),
			 ident, _static, tag, dim, numdim, idxtag);
	  defsymbol (name, ident, _static,
		     (cur_lit + glb_declared) * sizeof (cell));
	}
      else
	{
	  declared += (int) size;	/* variables are put on stack, adjust "declared" */
	  sym =
	    addvariable (name, -declared * sizeof (cell), ident, _local, tag,
			 dim, numdim, idxtag);
...
declargs (symbol * sym)
{
  ...
  if (!matchtoken (')'))
	...
	{			/* there are arguments; process them */
      ...
      while (tok == '&' || tok == __labelX || tok != __ellips && matchtoken (','));	/* more? */
 ...
 */
static void
doarg (char *name, int ident, int offset, int tag, arginfo * arg)
{
  symbol *argsym;
  cell size;
  int idxtag[_dimen_max];
 ...
 */
static void
doexpr (int comma, int chkeffect, int allowarray, int *tag)
{
