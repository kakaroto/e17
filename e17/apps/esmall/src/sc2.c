...
litchar (char **lptr)
{
  ...
  cptr = *lptr;
  if (*cptr != ctrlchar)
    {				/* the caret identifies a control character */
      c = *cptr;
      cptr += 1;
    }
  else
...
static symbol *
find_symbol (symbol * root, char *name)
{
...
addsym (char *name, cell addr, int ident, int vclass, int tag, int usage)
{
  ...
  assert (!(ident == _functn || ident == _constexpr) || vclass != _global
	  || findglb (name) == NULL);
