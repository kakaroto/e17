...
workspace_set_current_etching(Etching *e)
{
  ...
  if (e)
      ...
      for (l = bits->bits; l; l = l->next)
	{
	  ...
	  state = l->data;
	  evas_callback_add(ws.view_evas, state->object, CALLBACK_MOUSE_DOWN, handle_bit_mouse_down, state);
	  evas_callback_add(ws.view_evas, state->object, CALLBACK_MOUSE_UP, handle_bit_mouse_up, state);
	  evas_callback_add(ws.view_evas, state->object, CALLBACK_MOUSE_MOVE, handle_bit_mouse_move, state);
	}
...
workspace_update_selection_from_widget(void)
{
   ...
   if (selected)
	     ...
	     for (l = GTK_CLIST(w)->row_list; l; l = l->next)
		  ...
		  if (row->state == GTK_STATE_SELECTED)
		    selected->description->sync = 
		      evas_list_append(selected->description->sync, strdup(cell->text));
	       }
	  }
	...
	if ((selected->description->hilited.image) && (prev_i1) &&
	...
	if ((selected->description->clicked.image) && (prev_i1) &&
  ...
  if (selected_state)
    {
      ...
      evas_resize(ws.view_evas, ws.obj.sel, selected_state->w, selected_state->h);
...
workspace_update_sync_list(void)
{
  ...
  if (etching_get_selected_item(ws.e))
	...
	for (l = etching_get_selected_item(ws.e)->o->description->bits; l; l = l->next)
	  {
	    ...
	    for (ll = etching_get_selected_item(ws.e)->description->sync; ll; ll = ll->next)
	      {
		char *name;
   ...
   for (y = 0; y < (zoom->allocation.height + 1) / ws.zoom_scale; y++)
       ...
       for (i = 0; i < ws.zoom_scale; i++)
	 {
	   gdk_window_copy_area(pmap, gc, 
				0, (y * ws.zoom_scale) + i, 
				view->window,
				xx - ((zoom->allocation.width + 1) / (2*ws.zoom_scale)),
				yy - ((zoom->allocation.height + 1) / (2*ws.zoom_scale)) + y,
				(zoom->allocation.width + 1) / ws.zoom_scale,
				1);
	 }
     }
...
workspace_configure_handles(gpointer data)
{
  ...
  evas_move(ws.view_evas, ws.obj.handles.tl, ws.backing_x - w, ws.backing_y - h);

  evas_get_image_size(ws.view_evas, ws.obj.handles.tr, &w, &h);
  evas_move(ws.view_evas, ws.obj.handles.tr, ws.backing_x + ws.backing_w, ws.backing_y - h);
   
  evas_get_image_size(ws.view_evas, ws.obj.handles.bl, &w, &h);
  evas_move(ws.view_evas, ws.obj.handles.bl, ws.backing_x - w, ws.backing_y + ws.backing_h);
   
  evas_get_image_size(ws.view_evas, ws.obj.handles.br, &w, &h);
  evas_move(ws.view_evas, ws.obj.handles.br, ws.backing_x + ws.backing_w, ws.backing_y + ws.backing_h);

  evas_move(ws.view_evas, ws.obj.backing, ws.backing_x - 1, ws.backing_y  - 1);
  evas_resize(ws.view_evas, ws.obj.backing, ws.backing_w + 2, ws.backing_h + 2);
...
{
  evas_show(ws.view_evas, ws.obj.handles.tl = evas_add_image_from_file(ws.view_evas, PACKAGE_DATA_DIR"/pixmaps/handle1.png"));
  evas_show(ws.view_evas, ws.obj.handles.tr = evas_add_image_from_file(ws.view_evas, PACKAGE_DATA_DIR"/pixmaps/handle2.png"));
  evas_show(ws.view_evas, ws.obj.handles.bl = evas_add_image_from_file(ws.view_evas, PACKAGE_DATA_DIR"/pixmaps/handle3.png"));
  evas_show(ws.view_evas, ws.obj.handles.br = evas_add_image_from_file(ws.view_evas, PACKAGE_DATA_DIR"/pixmaps/handle4.png"));
  ...
  evas_show(ws.view_evas, ws.obj.backing = evas_add_image_from_file(ws.view_evas, PACKAGE_DATA_DIR"/pixmaps/backing.png"));
  ...
  evas_callback_add(ws.view_evas, ws.obj.handles.tl, CALLBACK_MOUSE_DOWN, handle_mouse_down, NULL);
  evas_callback_add(ws.view_evas, ws.obj.handles.tl, CALLBACK_MOUSE_UP, handle_mouse_up, NULL);
  evas_callback_add(ws.view_evas, ws.obj.handles.tl, CALLBACK_MOUSE_MOVE, handle_mouse_move, NULL);
  evas_callback_add(ws.view_evas, ws.obj.handles.tr, CALLBACK_MOUSE_DOWN, handle_mouse_down, NULL);
  evas_callback_add(ws.view_evas, ws.obj.handles.tr, CALLBACK_MOUSE_UP, handle_mouse_up, NULL);
  evas_callback_add(ws.view_evas, ws.obj.handles.tr, CALLBACK_MOUSE_MOVE, handle_mouse_move, NULL);
  evas_callback_add(ws.view_evas, ws.obj.handles.bl, CALLBACK_MOUSE_DOWN, handle_mouse_down, NULL);
  evas_callback_add(ws.view_evas, ws.obj.handles.bl, CALLBACK_MOUSE_UP, handle_mouse_up, NULL);
  evas_callback_add(ws.view_evas, ws.obj.handles.bl, CALLBACK_MOUSE_MOVE, handle_mouse_move, NULL);
  evas_callback_add(ws.view_evas, ws.obj.handles.br, CALLBACK_MOUSE_DOWN, handle_mouse_down, NULL);
  evas_callback_add(ws.view_evas, ws.obj.handles.br, CALLBACK_MOUSE_UP, handle_mouse_up, NULL);
  evas_callback_add(ws.view_evas, ws.obj.handles.br, CALLBACK_MOUSE_MOVE, handle_mouse_move, NULL);
  evas_callback_add(ws.view_evas, ws.obj.backing, CALLBACK_MOUSE_DOWN, handle_mouse_down, NULL);
  evas_callback_add(ws.view_evas, ws.obj.backing, CALLBACK_MOUSE_UP, handle_mouse_up, NULL);
  evas_callback_add(ws.view_evas, ws.obj.backing, CALLBACK_MOUSE_MOVE, handle_mouse_move, NULL);
...
void                     
workspace_set_normal_image(char *filename)
{
  GtkWidget *w;
...
void                     
workspace_set_hilited_image(char *filename)
{
  GtkWidget *w;
...
void                     
workspace_set_clicked_image(char *filename)
{
  GtkWidget *w;
...
void                     
workspace_set_disabled_image(char *filename)
{
  GtkWidget *w;
  ...
  if (ws.new_evas)
    {
      ...
      evas_set_output_viewport(ws.view_evas, 
			       0, 0, 
			       widget->allocation.width, 
			       widget->allocation.height);
	...
	if (!tile)
	  ws.obj.grid = evas_add_image_from_file(ws.view_evas,
	...
	e_db_flush();
      ...
      evas_callback_add(ws.view_evas, ws.obj.grid, CALLBACK_MOUSE_DOWN, handle_bg_mouse_down, NULL);
      ...
      ws.obj.pointer = evas_add_image_from_file(ws.view_evas, PACKAGE_DATA_DIR"/pixmaps/pointer.png");
      ...
      */
      ...
      ws.obj.selection_tl.abs = evas_add_image_from_file(ws.view_evas, PACKAGE_DATA_DIR"/pixmaps/handle_abs.png");
      ws.obj.selection_tl.rel = evas_add_image_from_file(ws.view_evas, PACKAGE_DATA_DIR"/pixmaps/handle_rel.png");
      ws.obj.selection_tl.adj = evas_add_image_from_file(ws.view_evas, PACKAGE_DATA_DIR"/pixmaps/handle_adj.png");
      ws.obj.selection_br.abs = evas_add_image_from_file(ws.view_evas, PACKAGE_DATA_DIR"/pixmaps/handle_abs.png");
      ws.obj.selection_br.rel = evas_add_image_from_file(ws.view_evas, PACKAGE_DATA_DIR"/pixmaps/handle_rel.png");
      ws.obj.selection_br.adj = evas_add_image_from_file(ws.view_evas, PACKAGE_DATA_DIR"/pixmaps/handle_adj.png");
      ...
      evas_callback_add(ws.view_evas, ws.obj.selection_tl.abs, CALLBACK_MOUSE_DOWN, handle_adjuster_mouse_down, NULL);
      evas_callback_add(ws.view_evas, ws.obj.selection_tl.abs, CALLBACK_MOUSE_UP, handle_adjuster_mouse_up, NULL);
      evas_callback_add(ws.view_evas, ws.obj.selection_tl.abs, CALLBACK_MOUSE_MOVE, handle_adjuster_mouse_move, NULL);
      evas_callback_add(ws.view_evas, ws.obj.selection_tl.rel, CALLBACK_MOUSE_DOWN, handle_adjuster_mouse_down, NULL);
      evas_callback_add(ws.view_evas, ws.obj.selection_tl.rel, CALLBACK_MOUSE_UP, handle_adjuster_mouse_up, NULL);
      evas_callback_add(ws.view_evas, ws.obj.selection_tl.rel, CALLBACK_MOUSE_MOVE, handle_adjuster_mouse_move, NULL);
      evas_callback_add(ws.view_evas, ws.obj.selection_tl.adj, CALLBACK_MOUSE_DOWN, handle_adjuster_mouse_down, NULL);
      evas_callback_add(ws.view_evas, ws.obj.selection_tl.adj, CALLBACK_MOUSE_UP, handle_adjuster_mouse_up, NULL);
      evas_callback_add(ws.view_evas, ws.obj.selection_tl.adj, CALLBACK_MOUSE_MOVE, handle_adjuster_mouse_move, NULL);
      evas_callback_add(ws.view_evas, ws.obj.selection_br.abs, CALLBACK_MOUSE_DOWN, handle_adjuster_mouse_down, NULL);
      evas_callback_add(ws.view_evas, ws.obj.selection_br.abs, CALLBACK_MOUSE_UP, handle_adjuster_mouse_up, NULL);
      evas_callback_add(ws.view_evas, ws.obj.selection_br.abs, CALLBACK_MOUSE_MOVE, handle_adjuster_mouse_move, NULL);
      evas_callback_add(ws.view_evas, ws.obj.selection_br.rel, CALLBACK_MOUSE_DOWN, handle_adjuster_mouse_down, NULL);
      evas_callback_add(ws.view_evas, ws.obj.selection_br.rel, CALLBACK_MOUSE_UP, handle_adjuster_mouse_up, NULL);
      evas_callback_add(ws.view_evas, ws.obj.selection_br.rel, CALLBACK_MOUSE_MOVE, handle_adjuster_mouse_move, NULL);
      evas_callback_add(ws.view_evas, ws.obj.selection_br.adj, CALLBACK_MOUSE_DOWN, handle_adjuster_mouse_down, NULL);
      evas_callback_add(ws.view_evas, ws.obj.selection_br.adj, CALLBACK_MOUSE_UP, handle_adjuster_mouse_up, NULL);
      evas_callback_add(ws.view_evas, ws.obj.selection_br.adj, CALLBACK_MOUSE_MOVE, handle_adjuster_mouse_move, NULL);
...
/*      else */
  ...
  evas_update_rect(ws.view_evas, ev->area.x, ev->area.y, ev->area.width, ev->area.height);
  workspace_queue_draw();
}
...
workspace_lower_selection(void)
{
  ...
  if (selected_state)
    {
      etching_get_bits(ws.e)->bits = evas_list_remove(etching_get_bits(ws.e)->bits, selected_state);
      etching_get_bits(ws.e)->bits = evas_list_prepend(etching_get_bits(ws.e)->bits, selected_state);
      etching_get_bits(ws.e)->description->bits = evas_list_remove(etching_get_bits(ws.e)->description->bits,
								   selected_state->description);
      etching_get_bits(ws.e)->description->bits = evas_list_prepend(etching_get_bits(ws.e)->description->bits,
								    selected_state->description);
