...
static void     handle_mouse_down(void *_data, Evas _e, Evas_Object _o, int _b,
				  int _x, int _y);
static void     handle_mouse_up(void *_data, Evas _e, Evas_Object _o, int _b,
				int _x, int _y);
static void     handle_mouse_move(void *_data, Evas _e, Evas_Object _o, int _b,
				  int _x, int _y);
...
static void     handle_bit_mouse_move(void *_data, Evas _e, Evas_Object _o,
				      int _b, int _x, int _y);
static void     handle_adjuster_mouse_down(void *_data, Evas _e, Evas_Object _o,
					   int _b, int _x, int _y);
static void     handle_adjuster_mouse_up(void *_data, Evas _e, Evas_Object _o,
					 int _b, int _x, int _y);
static void     handle_adjuster_mouse_move(void *_data, Evas _e, Evas_Object _o,
					   int _b, int _x, int _y);
...
workspace_init(void)
{
	...
	if (pref_get_render_method() == Hardware)
		...
		evas_set_output_method(ws.view_evas,
...
workspace_set_current_etching(Etching * e)
{
	...
	if (e) {
		...
		E_DB_STR_SET(pref_get_config(), "/paths/bit",
			     etching_get_filename(e));
		e_db_flush();
	}
}
...
workspace_update_selection_from_widget(void)
{
	...
	if (selected) {
		...
		selected->description->align.w =
			get_spin("content_alignment_h");
		selected->description->align.h =
			get_spin("content_alignment_v");
		...
		if (selected->description->sync) {
			...
			for (ll = selected->description->sync; ll;
			     ll = ll->next)
				free(ll->data);
...
/*		get_entry("img_normal", &(selected->description->normal.image));
		...
		for (l = ebits_get_state_names(bits); l; l = l->next) {
			...
			for (ll = selected->state_source_description; ll;
			     ll = ll->next) {
		...
		if (!strcmp(gtk_entry_get_text(GTK_ENTRY(w)), "Fill"))
			...
			selected->description->tile.w = 1;
			 ...
			 (gtk_entry_get_text(GTK_ENTRY(w)), "Tile Integer"))
...
workspace_update_widget_from_selection(void)
{
	...
	if (selected) {
		...
		if ((etching_get_selected_item(ws.e)->description->aspect.x > 0)
		    && (etching_get_selected_item(ws.e)->description->aspect.y >
			0))
		...
		else if (etching_get_selected_item(ws.e)->description->tile.w ==
			 1)
		...
		if (etching_get_selected_item(ws.e)->description->tile.h == 0)
			gtk_entry_set_text(GTK_ENTRY(w), "Fill");
		else if (etching_get_selected_item(ws.e)->description->tile.h ==
			 1)
			gtk_entry_set_text(GTK_ENTRY(w), "Tile");
		else if (etching_get_selected_item(ws.e)->description->tile.h ==
			 2)
...

		{
			...
			for (l = selected->description->state_description; l;
			     l = l->next) {
				...
				gtk_object_set_data(GTK_OBJECT(main_win), key,
						    state_d->image);
			}
		}
		/* update_image_list(); */
...
workspace_update_visible_selection(void)
{
	...
	if (selected_state) {
		...
		evas_set_line_xy(ws.view_evas, ws.obj.selection.b,
				 ...
				 selected_state->o->state.y +
				 selected_state->y + selected_state->h - 1);
		evas_get_image_size(ws.view_evas, ws.obj.selection_tl.abs, &w,
				    &h);
		...
		evas_get_image_size(ws.view_evas, ws.obj.selection_tl.rel, &w,
				    &h);
		evas_move(ws.view_evas, ws.obj.selection_tl.rel,
			  selected_state->o->state.x + selected_state->x - w,
			  selected_state->o->state.y + selected_state->y);
		evas_get_image_size(ws.view_evas, ws.obj.selection_tl.adj, &w,
				    &h);
		evas_move(ws.view_evas, ws.obj.selection_tl.adj,
			  selected_state->o->state.x + selected_state->x,
			  selected_state->o->state.y + selected_state->y - h);

		evas_get_image_size(ws.view_evas, ws.obj.selection_br.abs, &w,
				    &h);
		evas_move(ws.view_evas, ws.obj.selection_br.abs,
			  ...
			  selected_state->o->state.y + selected_state->y +
			  selected_state->h);
		evas_get_image_size(ws.view_evas, ws.obj.selection_br.rel, &w,
				    &h);
		evas_move(ws.view_evas, ws.obj.selection_br.rel,
			  ...
			  selected_state->o->state.y + selected_state->y +
			  selected_state->h - h);
		evas_get_image_size(ws.view_evas, ws.obj.selection_br.adj, &w,
				    &h);
...
workspace_zoom_redraw(int xx, int yy)
{
	...
	for (x = ((zoom->allocation.width + 1) / ws.zoom_scale) - 1; x >= 0;
	     x--) {
		int             i;
		      ...
		      int _y)
{
	...
	if ((ws.draft_mode) &&
		...
		if (evas_get_data(_e, _o, "clicked")) {
			...
			ebits_move(bits, ws.backing_x, ws.backing_y);
			ebits_resize(bits, ws.backing_w + 10,
				     ws.backing_h + 10);
			ebits_resize(bits, ws.backing_w, ws.backing_h);
			workspace_update_widget_from_selection();
			workspace_update_visible_selection();
...
workspace_configure_handles(gpointer data)
{
	...
	evas_move(ws.view_evas, ws.obj.backing, ws.backing_x - 1,
		  ws.backing_y - 1);
...
workspace_expose_event(GtkWidget * widget, GdkEventExpose * ev)
{
	if (ws.new_evas) {
		...
		evas_show(ws.view_evas, ws.obj.bg_rect);
		{
			...
			E_DB_STR_GET(pref_get_config(), "/grid/image", tile,
				     ok);
			...
			if (ok)
				evas_set_color(ws.view_evas, ws.obj.grid, r, g,
					       b, 255);
		...
		evas_callback_add(ws.view_evas, ws.obj.grid,
				  CALLBACK_MOUSE_DOWN, handle_bg_mouse_down,
				  NULL);
		...
		ws.obj.selection_tl.abs =
			evas_add_image_from_file(ws.view_evas,
						 PACKAGE_DATA_DIR
						 "/pixmaps/handle_abs.png");
		ws.obj.selection_tl.rel =
			evas_add_image_from_file(ws.view_evas,
						 PACKAGE_DATA_DIR
						 "/pixmaps/handle_rel.png");
		ws.obj.selection_tl.adj =
			evas_add_image_from_file(ws.view_evas,
						 PACKAGE_DATA_DIR
						 "/pixmaps/handle_adj.png");
		...
		evas_set_color(ws.view_evas, ws.obj.selection.l, 200, 50, 50,
			       200);
		evas_set_color(ws.view_evas, ws.obj.selection.r, 200, 50, 50,
			       200);
		evas_set_color(ws.view_evas, ws.obj.selection.t, 200, 50, 50,
			       200);
		evas_set_color(ws.view_evas, ws.obj.selection.b, 200, 50, 50,
			       200);
		...
		evas_callback_add(ws.view_evas, ws.obj.selection_tl.abs,
				  CALLBACK_MOUSE_UP, handle_adjuster_mouse_up,
				  NULL);
		evas_callback_add(ws.view_evas, ws.obj.selection_tl.abs,
				  CALLBACK_MOUSE_MOVE,
		...
		if (pref_splashscreen_enabled()) {
			show_splash(ws.view_evas, &ws.current_idle,
				    workspace_redraw);
		}
...
/*      else */
...

		{
			...
			cursor = gdk_cursor_new_from_pixmap(src, mask, &fg, &bg,
							    0, 0);
...
workspace_select_image(int index)
{
	...
	for (count = 0, l = etching_get_bits(ws.e)->bits; l;
	     l = l->next, count++) {
		if (count == index) {
...
workspace_raise_selection(void)
{
	...
	if (selected_state) {
		...
		if (selected_state->object)
			evas_raise(selected_state->o->state.evas,
...
workspace_lower_selection(void)
{
	...
	if (selected_state) {
		etching_get_bits(ws.e)->bits =
			evas_list_remove(etching_get_bits(ws.e)->bits,
					 selected_state);
		etching_get_bits(ws.e)->bits =
			evas_list_prepend(etching_get_bits(ws.e)->bits,
					  selected_state);
		...
		if (selected_state->object)
			evas_lower(selected_state->o->state.evas,
				   selected_state->object);
...
{
	if (etching_get_selected_item(ws.e)) {
		ebits_del_bit(etching_get_bits(ws.e),
			      etching_get_selected_item(ws.e));
		etching_set_selected_item(ws.e, NULL);
		workspace_update_visible_selection();
		workspace_update_widget_from_selection();
...
workspace_apply_settings(void)
{
	...
	ebits_resize(etching_get_bits(ws.e), ws.backing_w + 10,
		     ws.backing_h + 10);
	ebits_resize(etching_get_bits(ws.e), ws.backing_w, ws.backing_h);
	workspace_update_visible_selection();
	ebits_set_state(etching_get_bits(ws.e),
			etching_get_current_state(ws.e));
	workspace_update_widget_from_selection();
	workspace_queue_draw();
	etching_set_dirty(ws.e);
}
