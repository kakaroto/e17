...
pref_update_preview(void)
{
  if (!pref.new_pref_evas)
	...
	free(pref.grid_image_file);
      ...
      pref.grid_image_file = strdup(gtk_entry_get_text(GTK_ENTRY(gtk_object_get_data(GTK_OBJECT(pref_dialog), "entry1"))));
      evas_set_image_file(pref.pref_evas, pref.o_pref_image, pref.grid_image_file);
      evas_move(pref.pref_evas, pref.o_pref_image, 0, 0);
      evas_resize(pref.pref_evas, pref.o_pref_image, 99999, 99999);
      evas_set_color(pref.pref_evas, pref.o_pref_image, pref.colors[0], pref.colors[1], pref.colors[2], 255);
  ...
  if (pref.new_pref_evas)
    {
      ...
      evas_set_output_size(pref.pref_evas, widget->allocation.width, widget->allocation.height);
...
pref_set_defaults(void)
{
  ...
  g_snprintf(pref.etcher_config, sizeof(pref.etcher_config), "%s/.etcher.db", getenv("HOME"));
  E_DB_STR_SET(pref.etcher_config, "/grid/image", PACKAGE_DATA_DIR "/pixmaps/tile.png");  
...
pref_set_render_method(EtcherRenderMethod method)
{
  ...
  E_DB_INT_SET(pref.etcher_config, "/display/render_method", (int)pref.render_method);
  e_db_flush();
}
...
pref_set_zoom_method(EtcherZoomMethod method)
{
  ...
  E_DB_INT_SET(pref.etcher_config, "/display/zoom_method", (int)pref.zoom_method);
  e_db_flush();
}
					  ...
					  gpointer         user_data)
{
  ...
  if (!pref_dialog)
	...
	vis = evas_get_optimal_visual(pref.pref_evas, GDK_WINDOW_XDISPLAY(GDK_ROOT_PARENT()));
	gdk_pref_vis = gdkx_visual_get(XVisualIDFromVisual(vis));
	cmap = evas_get_optimal_colormap(pref.pref_evas, GDK_WINDOW_XDISPLAY(GDK_ROOT_PARENT()));
