		      ...
		      gint column, GdkEvent * event, gpointer user_data)
{
   ...
   states_entry = gtk_object_get_data(GTK_OBJECT(main_win), "states_entry");
   browse_state = gtk_object_get_data(GTK_OBJECT(main_win), "browse_state");
...
on_state_entry_ok_clicked(GtkButton * button, gpointer user_data)
{
   ...
   gtk_clist_get_text(GTK_CLIST(clist), states2_selected_row, 0, &row_data);
