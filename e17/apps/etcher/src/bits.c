...
static char    *_ebits_get_file(Ebits_Object_Bit_Description d, char *state);

#ifdef EDITOR
static int      _ebits_image_state_saved(Ebits_Object_Bit_State state, char *s);
#endif
...
_ebits_evaluate_fill(Ebits_Object_Bit_State state)
{
	...
	if (state->description->tile.w == EBITS_FILL_TILE) {
		...
		evas_get_image_size(state->o->state.evas, state->object, &im_w,
				    NULL);
		if (im_w > 0)
			fill_w = im_w;
	...
	if (state->description->tile.h == EBITS_FILL_TILE) {
		...
		evas_get_image_size(state->o->state.evas, state->object, NULL,
				    &im_h);
		if (im_h > 0)
			fill_h = im_h;
...
_ebits_object_calculate(Ebits_Object o)
{
	...
	for (l = o->bits; l; l = l->next) {
		...
		evas_resize(state->o->state.evas, state->object, state->w,
			    state->h);
		if (state->func_move)
		...
		if (state->func_resize)
			state->func_resize(state->func_data, state->w,
					   state->h);
	}
}
...
_ebits_calculate(Ebits_Object_Bit_State state)
{
	...
	if (state->description->rel1.name) {
		...
		state2 = ebits_get_bit_name(state->o,
					    state->description->rel1.name);
		if (state2) {
	...
	state->w =
		(state->w / state->description->step.x) *
		state->description->step.x;
	state->h =
		(state->h / state->description->step.y) *
		state->description->step.y;

	if ((state->description->aspect.x > 0)
	    && (state->description->aspect.y > 0)) {
		int             hh, ww;
		double          th, tw;

		tw = ((double) state->w /
		      (double) state->description->aspect.x);
		th = ((double) state->h /
		      (double) state->description->aspect.y);
		...
		for (l = state->callbacks; l; l = l->next) {
			...
			if (cb->type == CALLBACK_MOUSE_DOWN)
				cb->func(cb->data, state->o,
					 ...
					 state->o->state.y + state->y, state->w,
					 state->h);
		}
	}
...
_ebits_find_description(char *file)
{
	...
	for (l = __bit_descriptions; l; l = l->next) {
		...
		if (!strcmp(d->file, file)) {
			if (l != __bit_descriptions) {
				__bit_descriptions =
					evas_list_remove(__bit_descriptions, d);
				__bit_descriptions =
					evas_list_prepend(__bit_descriptions,
							  d);
			}
			if (d->references <= 0)
				__ebits_cache_zero--;
	...
	/* all the bits */
	{
		...
		for (i = 0; i < num_bits; i++) {
			...
			snprintf(key, sizeof(key), "/bits/bit/%i/color_class",
				 i);
			bit->color_class = e_db_str_get(db, key);
			snprintf(key, sizeof(key), "/bits/bit/%i/normal/image",
				 i);
			bit->normal.image = e_db_str_get(db, key);
			snprintf(key, sizeof(key), "/bits/bit/%i/hilited/image",
				 i);
			bit->hilited.image = e_db_str_get(db, key);
			snprintf(key, sizeof(key), "/bits/bit/%i/clicked/image",
				 i);
			bit->clicked.image = e_db_str_get(db, key);
			snprintf(key, sizeof(key),
				 "/bits/bit/%i/disabled/image", i);
			...
			num_sync = 0;
			snprintf(key, sizeof(key), "/bits/bit/%i/sync/count",
				 i);
			...
			for (j = 0; j < num_sync; j++) {
				...
				snprintf(key, sizeof(key),
					 "/bits/bit/%i/sync/%i", i, j);
				s = e_db_str_get(db, key);
				if (s)
					bit->sync =
						evas_list_append(bit->sync, s);
			}
...
_ebits_evaluate(Ebits_Object_Bit_State state)
{
	...
	if (state->description->normal.image)
		state->normal.image =
			imlib_load_image(state->description->normal.image);
	if (state->description->hilited.image)
		state->hilited.image =
	...
	for (l = state->description->state_description; l; l = l->next) {
		...
		if (!state_d->ss_d) {
			...
			state_d->ss_d =
				malloc(sizeof
				       (struct
					_Ebits_State_Source_Description));
			state_d->ss_d =
...
ebits_new_bit(Ebits_Object o, char *file)
{
							 ...
							 state->state));
	{
		evas_callback_add(o->state.evas, state->object,
				  CALLBACK_MOUSE_DOWN, _ebits_handle_mouse_down,
				  state);
		evas_callback_add(o->state.evas, state->object,
				  CALLBACK_MOUSE_UP, _ebits_handle_mouse_up,
				  state);
		evas_callback_add(o->state.evas, state->object,
				  CALLBACK_MOUSE_MOVE, _ebits_handle_mouse_move,
				  state);
		evas_callback_add(o->state.evas, state->object,
				  CALLBACK_MOUSE_IN, _ebits_handle_mouse_in,
				  state);
		evas_callback_add(o->state.evas, state->object,
				  CALLBACK_MOUSE_OUT, _ebits_handle_mouse_out,
				  state);
	}
		...
		if (d) {
			...
			if (d->bits) {
				for (l = d->bits; l; l = l->next) {
					...
					/* XXX */
					for (ll = bit->state_description; ll;
					     ll = ll->next) {
						Ebits_State_Description state_d;
...
ebits_load(char *file)
{
	...
	for (l = d->bits; l; l = l->next) {
		...
		bit = l->data;
		state = malloc(sizeof(struct _Ebits_Object_Bit_State));
		state = memset(state, 0,
			       sizeof(struct _Ebits_Object_Bit_State));
		...
		if (bit->normal.image) {
			...
			if (!state->normal.image)
				state->normal.image =
					imlib_load_image(bit->normal.image);
#endif
		}
		if (bit->hilited.image) {
			...
			if (!state->hilited.image)
				state->hilited.image =
					imlib_load_image(bit->hilited.image);
#endif
		}
		if (bit->clicked.image) {
			...
			if (!state->clicked.image)
				state->clicked.image =
					imlib_load_image(bit->clicked.image);
#endif
		}
		if (bit->disabled.image) {
			...
			if (!state->disabled.image)
				state->disabled.image =
					imlib_load_image(bit->disabled.image);
#endif
		}
		/* XXX */
...
ebits_free(Ebits_Object o)
{
	...
	if (o->bits) {
		...
		for (l = o->bits; l; l = l->next) {
			...
			/* XXX */
			for (ll = state->state_source_description; ll;
			     ll = ll->next) {
				Ebits_State_Source_Description ss_d;
	...
	for (l = o->bits; l; l = l->next) {
		...
		if ((state->description->normal.image) &&
			...
			if ((state->description->class) &&
				...
				evas_set_pass_events(o->state.evas,
						     state->object, 1);
			evas_set_color(o->state.evas, state->object, 255, 255,
				       255, 0);
		} else {
			state->object =
				evas_add_image_from_file(o->state.evas, buf);
			if ((state->description->class) &&
			    (!strcmp(state->description->class, "Decoration")))
				evas_set_pass_events(o->state.evas,
						     state->object, 1);
...
{
	w = (double) (((int) w / o->description->step.x) *
		      o->description->step.x);
	h = (double) (((int) h / o->description->step.y) *
		      o->description->step.y);
	...
	for (l = o->bits; l; l = l->next) {
		...
		if ((state->description->class) && (c)) {
			if (!strcmp(state->description->class, c)) {
				...
				cb = memset(cb, 0,
					    sizeof(struct _Ebits_Callback));
				cb->type = type;
				cb->func = func;
				cb->data = data;
				state->callbacks =
					evas_list_append(state->callbacks, cb);
			}
		}
	}
}
...
ebits_set_color_class(Ebits_Object o, char *cc, int r, int g, int b, int a)
{
	...
	for (l = o->bits; l; l = l->next) {
		...
		if (state->func_set_color_class)
			state->func_set_color_class(state->func_data, cc, r, g,
						    b, a);
	}
}
...
ebits_save(Ebits_Object o, char *file)
{
	...
	for (l = o->bits; l; l = l->next) {
		...
		/* XXX */
		for (ll = state->description->state_description; ll;
		     ll = ll->next) {
			Ebits_State_Description state_d;
			    ...
			    void *data)
{
	...
	for (l = o->bits; l; l = l->next) {
		...
		if (!strcmp(state->description->name, c)) {
			...
			if (state->func_set_clip) {
				state->func_set_clip(state->func_data,
						     state->object);
				evas_set_color(o->state.evas, state->object,
					       255, 255, 255, 255);
			}
			if ((o->state.visible) && (func_show))
				...
				func_hide(data);
			if ((state->description->color_class) &&
			    (func_set_color_class))
...
ebits_get_bit_names(Ebits_Object o)
{
	...
	for (l = o->bits; l; l = l->next) {
		...
		st = l->data;
		name_list =
			evas_list_append(name_list,
					 strdup(st->description->name));
	}
...
ebits_del_state_name(Ebits_Object o, char *name)
{
	...
	for (l = o->description->state_names; l; l = l->next) {
		if (!strncasecmp(l->data, name, len)) {
			FREE(l->data);
			o->description->state_names =
				evas_list_remove(o->description->state_names,
						 l->data);
			break;
		}
	}
...
ebits_add_bit_state(Ebits_Object_Bit_State bit, char *state, char *image)
{
	...
	for (l = bit->state_source_description; l; l = l->next) {
		...
		if (!strncasecmp(state, ss_d->name, len) && len == len2) {
			...
			if (!image || !strlen(image)) {
				evas_list_remove(bit->state_source_description,
						 ss_d);
	...
	if (!found) {
		...
		bit->description->state_description =
			evas_list_append(bit->description->state_description,
					 state_d);
	}
}
#endif
