...
flow_chart_timer(void *data)
{
  ...
  if (value < 0) 
	...
	edje_object_part_text_set(chart->edje, chart->edje_text_part, "error");
      chart->is_runable = 0;
...
void
flow_chart_callback_set(Flow_Chart *chart, void(*cb)(Flow_Chart *chart, void *data), void *data)
{
  ...
  if( (test = func()) != -1 )
    {
      ...
      if(chart->evas)chart->is_runable = 1;
    }
  else
    {
      chart->get_value_func = NULL;
...
{
  if( !chart->is_runable || chart->is_running )return;
...
{
  if( chart->is_running )flow_chart_stop(chart);
...
{
  if (!chart_con || !chart) return;
 
  if (!chart_con->charts) return;
  else
    {
