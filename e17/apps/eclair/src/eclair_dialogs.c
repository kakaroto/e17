...
static void _eclair_dialogs_open_fc_open(Eclair_Dialogs_Manager *dialogs_manager, Eclair_Dialog_Open_FC_Type fc_type);
static gboolean _eclair_dialogs_open_fc_on_ok(GtkWidget *widget, gpointer data);
static gboolean _eclair_dialogs_open_fc_on_delete(GtkWidget *widget, GdkEvent *event, gpointer data);
...
void eclair_dialogs_shutdown(Eclair_Dialogs_Manager *dialogs_manager)
{
   ...
   if (dialogs_manager->open_fc_widget && dialogs_manager->eclair && (last_location_path = gtk_file_chooser_get_current_folder(GTK_FILE_CHOOSER(dialogs_manager->open_fc_widget))))
   {
...
static gint _eclair_dialogs_update(gpointer data)
{
   ...
   {
      _eclair_dialogs_open_fc_open(dialogs_manager, dialogs_manager->should_open_open_fc);
      dialogs_manager->should_open_open_fc = ECLAIR_FC_NONE;
   }
...
static void _eclair_dialogs_open_fc_open(Eclair_Dialogs_Manager *dialogs_manager, Eclair_Dialog_Open_FC_Type fc_type)
{
   ...
   if (!dialogs_manager->open_fc_dialog)
   {
      ...
      dialogs_manager->open_fc_dialog = gtk_dialog_new_with_buttons("Add Files", NULL, GTK_DIALOG_NO_SEPARATOR, NULL);
      gtk_window_set_wmclass(GTK_WINDOW(dialogs_manager->open_fc_dialog), "eclair", "eclair");
      gtk_window_set_default_size(GTK_WINDOW(dialogs_manager->open_fc_dialog), 600, 420);
      dialogs_manager->open_fc_widget = gtk_file_chooser_widget_new(GTK_FILE_CHOOSER_ACTION_OPEN);
      gtk_file_chooser_set_local_only(GTK_FILE_CHOOSER(dialogs_manager->open_fc_widget), FALSE);
      if (eclair_config_get_prop_string(&eclair->config, "last_location", "path", &last_location_path))
      {
         if (strlen(last_location_path) > 0)
            gtk_file_chooser_set_current_folder(GTK_FILE_CHOOSER(dialogs_manager->open_fc_widget), last_location_path);
         free(last_location_path);
      }
      gtk_box_pack_start(GTK_BOX(GTK_DIALOG(dialogs_manager->open_fc_dialog)->vbox), dialogs_manager->open_fc_widget, TRUE, TRUE, 0);
   
      dialogs_manager->open_fc_all_button = gtk_button_new_with_label("All");
      gtk_box_pack_start(GTK_BOX(GTK_DIALOG(dialogs_manager->open_fc_dialog)->action_area), dialogs_manager->open_fc_all_button, TRUE, TRUE, 0);
      dialogs_manager->open_fc_none_button = gtk_button_new_with_label("None");
      gtk_box_pack_start(GTK_BOX(GTK_DIALOG(dialogs_manager->open_fc_dialog)->action_area), dialogs_manager->open_fc_none_button, TRUE, TRUE, 0);
      dialogs_manager->open_fc_ok_button = gtk_button_new_with_label("Add Files");
      gtk_box_pack_start(GTK_BOX(GTK_DIALOG(dialogs_manager->open_fc_dialog)->action_area), dialogs_manager->open_fc_ok_button, TRUE, TRUE, 0);
      dialogs_manager->open_fc_close_button = gtk_button_new_with_label("Close");
      gtk_box_pack_start(GTK_BOX(GTK_DIALOG(dialogs_manager->open_fc_dialog)->action_area), dialogs_manager->open_fc_close_button, TRUE, TRUE, 0);
   
      g_signal_connect_swapped(dialogs_manager->open_fc_all_button, "clicked", G_CALLBACK(gtk_file_chooser_select_all), dialogs_manager->open_fc_widget);
      g_signal_connect_swapped(dialogs_manager->open_fc_none_button, "clicked", G_CALLBACK(gtk_file_chooser_unselect_all), dialogs_manager->open_fc_widget);
      g_signal_connect(dialogs_manager->open_fc_ok_button, "clicked", G_CALLBACK(_eclair_dialogs_open_fc_on_ok), eclair);
      g_signal_connect_swapped(dialogs_manager->open_fc_close_button, "clicked", G_CALLBACK(gtk_widget_hide), dialogs_manager->open_fc_dialog);
      g_signal_connect(dialogs_manager->open_fc_dialog, "delete_event", G_CALLBACK(_eclair_dialogs_open_fc_on_delete), eclair);
   ...
   {
      gtk_file_chooser_set_select_multiple(GTK_FILE_CHOOSER(dialogs_manager->open_fc_widget), TRUE);
      gtk_button_set_label(GTK_BUTTON(dialogs_manager->open_fc_ok_button), "Add Files");
      gtk_window_set_title(GTK_WINDOW(dialogs_manager->open_fc_dialog), "Add Files");
   ...
   {
      gtk_file_chooser_set_select_multiple(GTK_FILE_CHOOSER(dialogs_manager->open_fc_widget), FALSE);
      gtk_button_set_label(GTK_BUTTON(dialogs_manager->open_fc_ok_button), "Load Playlist");
      gtk_window_set_title(GTK_WINDOW(dialogs_manager->open_fc_dialog), "Load Playlist");
