...
/*
 * entrance_auth_initialize - initialize PAM session, structures etc.
 * This function will call pam_start() and set the conversation
 * function and others.
 */
int
entrance_auth_initialize(Entrance_Auth e)
{
   int result = 0;
   int pamerr;
...
/*
 * entrance_auth_cmp - attempt to auth the user
 * @e The Entrance_Auth struct to attempt to validate on the system
 * Returns - 0 on success, 1 on error
 */
int
entrance_auth_cmp(Entrance_Auth e)
{
   int result = 0;
   int pamerr;

   if (entrance_auth_initialize(e) != E_SUCCESS)
   {
