...
static Entrance_Session e_session = NULL;
...
void
setup_events(Entrance_Session e)
{
   e_session = e;
   evas = e->evas;
...
e_key_down(Ecore_Event * ev)
{
   ...
   if (e->key)
   {
      ...
      else if (length > 1)
      {
         ...
         {
            password_or_user =
               entrance_return_key_cb(e_session, typebuffer.buf);
            memset(&typebuffer.buf, 0, USER_PASS_MAX);
            typebuffer.buf_index = 0;
         }
/*         else if (!(strcmp(e->key, "Up")))
         {
            entrance_select_prev_session(e_session);
         }
         else if (!(strcmp(e->key, "Down")))
         {
            entrance_select_next_session(e_session);
         }*/
