...
e_build_menu_db_poll(int val, void *data)
{
   ...
   /* kjb - and show it */
   /*   bm->menu->always_visible = bm->visible;
	bm->menu->edit_target = bm->edit_target;*/
   bm->menu->always_visible = 1;
...
e_build_menu_db_build_number(E_Build_Menu *bm, E_DB_File *db, int num)
{
   ...
   for (i2 = 0; i2 < num2; i2++)
	...
	/* kjb - mark menu */
	menuitem->db_num1 = num;
	menuitem->db_num2 = i2;
	exe ?  menuitem->exe = strdup(exe) : "";
	script ?  menuitem->script = strdup(script) : "";
	...
	if (sep) e_menu_item_set_separator(menuitem, 1);
	else
	  {
	     if (ok)
		  ...
		  /* kjb - inherit edit property */
		  /* Ugh, menu doesn't have this set yet, force it for now */
		  /* menu2->edit_target = menu->edit_target;*/
		  menu2->edit_target = 1;
