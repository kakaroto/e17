...
e_background_new(void)
{
   ...
   bg = malloc(sizeof(E_Background));
   memset(bg, 0, sizeof(E_Background));
   return bg;
}
   ...
   for (i = 0; i < num; i++)
	...
	if (bl->inlined)
	...
	else
	  {
        ...
        if (bl->file) bl->image = imlib_load_image(bl->file);
...
e_background_save(char *file, E_Background *bg)
{
   ...
   for (i = 0, l = bg->layers; l; l = l->next, i++)
	...
	bl = l->data;
	sprintf(buf, "/layers/%i/type", i);
	e_db_int_set(db, buf, bl->type);
	sprintf(buf, "/layers/%i/inlined", i);
	e_db_int_set(db, buf, bl->inlined);
	sprintf(buf, "/layers/%i/angle", i);
	e_db_float_set(db, buf, bl->angle);
	sprintf(buf, "/layers/%i/scroll.x", i);
	e_db_float_set(db, buf, bl->scroll.x);
	sprintf(buf, "/layers/%i/scroll.y", i);
	e_db_float_set(db, buf, bl->scroll.y);
	sprintf(buf, "/layers/%i/pos.x", i);
	e_db_float_set(db, buf, bl->pos.x);
	sprintf(buf, "/layers/%i/pos.y", i);
	e_db_float_set(db, buf, bl->pos.y);
	sprintf(buf, "/layers/%i/size.w", i);
	e_db_float_set(db, buf, bl->size.w);
	sprintf(buf, "/layers/%i/size.h", i);
	e_db_float_set(db, buf, bl->size.h);
	sprintf(buf, "/layers/%i/size.orig.w", i);
	e_db_int_set(db, buf, bl->size.orig.w);
	sprintf(buf, "/layers/%i/size.orig.h", i);
	e_db_int_set(db, buf, bl->size.orig.h);
	sprintf(buf, "/layers/%i/fill.w", i);
	e_db_float_set(db, buf, bl->fill.w);
	sprintf(buf, "/layers/%i/fill.h", i);
	e_db_float_set(db, buf, bl->fill.h);
	sprintf(buf, "/layers/%i/fill.orig.w", i);
	e_db_int_set(db, buf, bl->fill.orig.w);
	sprintf(buf, "/layers/%i/fill.orig.h", i);
	e_db_int_set(db, buf, bl->fill.orig.h);
	sprintf(buf, "/layers/%i/fg.r", i);
	e_db_int_set(db, buf, bl->fg.r);
	sprintf(buf, "/layers/%i/fg.g", i);
	e_db_int_set(db, buf, bl->fg.g);
	sprintf(buf, "/layers/%i/fg.b", i);
	e_db_int_set(db, buf, bl->fg.b);
	sprintf(buf, "/layers/%i/fg.a", i);
	e_db_int_set(db, buf, bl->fg.a);
	sprintf(buf, "/layers/%i/bg.r", i);
	e_db_int_set(db, buf, bl->bg.r);
	sprintf(buf, "/layers/%i/bg.g", i);
	e_db_int_set(db, buf, bl->bg.g);
	sprintf(buf, "/layers/%i/bg.b", i);
	e_db_int_set(db, buf, bl->bg.b);
	sprintf(buf, "/layers/%i/bg.a", i);
	e_db_int_set(db, buf, bl->bg.a);
	if (bl->file)
	  {
	     sprintf(buf, "/layers/%i/file", i);
	     e_db_str_set(db, buf, bl->file);
	  }
	if (bl->color_class)
	  {
	     sprintf(buf, "/layers/%i/color_class", i);
	     e_db_str_set(db, buf, bl->color_class);
	  }
	if (bl->inlined)
	  {
	     printf("inlined\n");
