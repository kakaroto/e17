 ...
 *
 *	@(#)edb_page.h	10.18 (Sleepycat) 12/2/98
 */
 ...
 *
 * Note that key/data lengths are often stored in edb_indx_t's -- this is
 ...
 * edb_indx_t, and storing it in one saves space.
 */
...
typedef struct _btmeta {
	...
	edb_pgno_t pgno;		/* 08-11: Current page number. */
...
typedef struct hashhdr {	/* Disk resident portion */
	...
	edb_pgno_t pgno;		/* 08-11: Page number (btree compatibility). */
 ...
 */
typedef struct _edb_page {
	DB_LSN	  lsn;		/* 00-07: Log sequence number. */
	edb_pgno_t pgno;		/* 08-11: Current page number. */
	edb_pgno_t prev_pgno;	/* 12-15: Previous page number. */
	edb_pgno_t next_pgno;	/* 16-19: Next page number. */
	edb_indx_t entries;	/* 20-21: Number of item pairs on the page. */
	edb_indx_t hf_offset;	/* 22-23: High free byte page offset. */
...
#define	P_OVERFLOW	7	/*        Overflow. */
	...
	edb_indx_t inp[1];	/* Variable length index of items. */
} PAGE;
 ...
 *
 * !!!
 * The btree/recno access method needs edb_recno_t bytes of space on the root
...
/* First free byte. */
#define	LOFFSET(pg)		(P_OVERHEAD + NUM_ENT(pg) * sizeof(edb_indx_t))
...
#define	HKEYDATA_PSIZE(len)						\
	(HKEYDATA_SIZE(len) + sizeof(edb_indx_t))
...
typedef struct _hoffpage {
	...
	edb_pgno_t pgno;		/* 04-07: Offpage page number. */
...
#define	HOFFPAGE_PSIZE		(HOFFPAGE_SIZE + sizeof(edb_indx_t))
...
#define	HOFFDUP_PSIZE		(HOFFDUP_SIZE + sizeof(edb_indx_t))
...
typedef struct _bkeydata {
	edb_indx_t len;		/* 00-01: Key/data item length. */
...
#define	BKEYDATA_PSIZE(len)						\
	(BKEYDATA_SIZE(len) + sizeof(edb_indx_t))
...
typedef struct _boverflow {
	edb_indx_t unused1;	/* 00-01: Padding, unused. */
	u_int8_t  type;		/*    02: Page type AND DELETE FLAG. */
	u_int8_t  unused2;	/*    03: Padding, unused. */
	edb_pgno_t pgno;		/* 04-07: Next page number. */
	u_int32_t tlen;		/* 08-11: Total length of item. */
} BOVERFLOW;
...
#define	BOVERFLOW_PSIZE							\
	(BOVERFLOW_SIZE + sizeof(edb_indx_t))
...
typedef struct _binternal {
	...
	edb_pgno_t  pgno;	/* 04-07: Page number of referenced page. */
	edb_recno_t nrecs;	/* 08-11: Subtree record count. */
...
#define	BINTERNAL_PSIZE(len)						\
	(BINTERNAL_SIZE(len) + sizeof(edb_indx_t))
 ...
 *
 * XXX
 * Why not fold this into the edb_indx_t structure, it's fixed length?
 */
typedef struct _rinternal {
	edb_pgno_t  pgno;	/* 00-03: Page number of referenced page. */
	edb_recno_t nrecs;	/* 04-07: Subtree record count. */
} RINTERNAL;
...
#define	RINTERNAL_PSIZE							\
	(RINTERNAL_SIZE + sizeof(edb_indx_t))
#endif /* _DB_PAGE_H_ */
