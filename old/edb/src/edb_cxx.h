 ...
 *
 *	@(#)edb_cxx.h	10.30 (Sleepycat) 11/22/98
 */
...
//    layer of edb (usually by dropping a prefix like "db_").
...
protected:
    ...
    // since its contained class is not allocated by edb.
    // (see comment at top)
...
public:
    ...
    int get(u_int32_t locker, u_int32_t flags, const Dbt *obj,
            edb_lockmode_t lock_mode, DbLock *lock);
    int id(u_int32_t *idp);
    int stat(DB_LOCK_STAT **statp, void *(*edb_malloc)(size_t));
    ...
    static int open(const char *dir, u_int32_t flags, int mode,
                    DbEnv* edbenv, DbLockTab **regionp);
    static int unlink(const char *dir, int force, DbEnv* edbenv);
...
public:
    int archive(char **list[], u_int32_t flags, void *(*edb_malloc)(size_t));
    int close();
    ...
    //
    int edb_register(Db *edbp, const char *name, DBTYPE type, u_int32_t *fidp);
    int edb_unregister(u_int32_t fid);

    // Create or remove new log files
    //
    static int open(const char *dir, u_int32_t flags, int mode,
                    DbEnv* edbenv, DbLog **regionp);
    static int unlink(const char *dir, int force, DbEnv* edbenv);
...
public:
    int close();
    int get(edb_pgno_t *pgnoaddr, u_int32_t flags, void *pagep);
...
public:
    ...
    //
    int edb_register(int ftype,
                    int (*pgin)(edb_pgno_t pgno, void *pgaddr, DBT *pgcookie),
                    int (*pgout)(edb_pgno_t pgno, void *pgaddr, DBT *pgcookie));

    int stat(DB_MPOOL_STAT **gsp, DB_MPOOL_FSTAT ***fsp,
             void *(*edb_malloc)(size_t));
    int sync(DbLsn *lsn);
...
public:
    ...
    int close();
    int stat(DB_TXN_STAT **statp, void *(*edb_malloc)(size_t));

    // Create or remove new txnmgr files
    //
    static int open(const char *dir, u_int32_t flags, int mode,
                    DbEnv* edbenv, DbTxnMgr **regionp);
    static int unlink(const char *dir, int force, DbEnv* edbenv);
...
public:
    ...
    typedef void *(*edb_malloc_fcn)(size_t);
    void set_malloc(edb_malloc_fcn);
...
private:
    // We can add data to this class if needed
    // since parent class is not allocated by edb.
    // (see comment at top)
};
...
// call appinit() before any other edb activity (e.g. opening files)
//
...
public:
    ...
    //
    DbEnv(const char *homeDir, char *const *edb_config, u_int32_t flags);

    // Use this constructor if you wish to *delay* the initialization
    // of the edb library.  This is useful if you need to set
    ...
    // complete the initialization of the edb library.
    //
    int appinit(const char *homeDir, char *const *edb_config, u_int32_t flags);
    ...
    // Panic callback.
    typedef void (*edb_paniccall_fcn)(DbEnv *, int);
    void set_paniccall(edb_paniccall_fcn);

    // Error message callback.
    typedef void (*edb_errcall_fcn)(const char *, char *);
    void set_errcall(edb_errcall_fcn);
...
public:
    ...
    int join(Dbc **curslist, u_int32_t flags, Dbc **edbcp);
    int put(DbTxn *txnid, Dbt *key, Dbt *data, u_int32_t flags);
    int stat(void *sp, void *(*edb_malloc)(size_t), u_int32_t flags);
    ...
    static int open(const char *fname, DBTYPE type, u_int32_t flags,
                    int mode, DbEnv *edbenv, DbInfo *info, Db **edbpp);

    static int xa_open(const char *fname, DBTYPE type, u_int32_t flags,
                    int mode, DbInfo *info, Db **edbpp);
private:
    // We can add data to this class if needed
    // since it is implemented via a pointer.
    // (see comment at top)
