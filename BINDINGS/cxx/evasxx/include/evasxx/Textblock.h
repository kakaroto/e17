#ifndef EVAS_TEXTBLOCK_H
#define EVAS_TEXTBLOCK_H
...
{
public:
  Textblock( Canvas& canvas );
  Textblock( Canvas& canvas, const Eflxx::Point &pos, const std::string &text );
  virtual ~Textblock();

  void setStyle( const TextblockStyle &style );
  const TextblockStyle getStyle () const;
