...
function autogen_lib {
    ...
    pwd=$(pwd)
    cd $what && \
        NOCONFIGURE=1 ./autogen.sh >& ../logs/autogen_${what}.log  || \
        (echo "Error!" && \
          cd $pwd && \
...
function download {
    ...
    svn export http://svn.enlightenment.org/svn/e/trunk/$what >& logs/svn_${what}.log || \
        (echo "Error!" && \
...
function tarball {
    what=$1
    echo -ne "Tarballing $what : "
    tar -czvf ${what}_svn${EFL_VERSION}.tar.gz ${what} >& logs/tarball_${what}.log || \
        (echo "Error!" && \
...
EFL_VERSION=$(svn info http://svn.enlightenment.org/svn/e/trunk/ | grep Revision | awk '{print $2}')
echo "${EFL_VERSION}"
