...
save_menus(GtkWidget * widget, gpointer user_data)
{
   ...
   buf = g_strjoin("/", homedir(getuid()), ".enlightenment", "file.menu", NULL);
...
tree_to_gnode(GtkCTree * ctree,
	      guint depth, GNode * gnode, GtkCTreeNode * cnode, gpointer data)
{
   ...
   gtk_ctree_node_get_text(GTK_CTREE(ctree), GTK_CTREE_NODE(cnode), 1, &col2);
   gtk_ctree_node_get_text(GTK_CTREE(ctree), GTK_CTREE_NODE(cnode), 2, &col3);
...
gint write_menu(GNode * node, gchar * file)
{
   ...
   if (file[0] != '/')
	...
	temp = g_strjoin("/", homedir(getuid()), ".enlightenment", file, NULL);
	g_free(file);
	file = temp;
     }
