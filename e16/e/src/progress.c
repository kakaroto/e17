...
CreateProgressbar(char *name, int width, int height)
{
   ...
   p->n_win = ECreateWindow(root.win, p->x + p->w - (p->h * 5), p->y,
			    (p->h * 5), p->h, 1);
   ...
   p->tnc = FindItem("PROGRESS_TEXT_NUMBER", 0, LIST_FINDBY_NAME,
		     LIST_TYPE_TCLASS);
   if (p->tnc)
      p->tnc->ref_count++;
...
SetProgressbar(Progressbar * p, int progress)
{
   ...
   pq = queue_up;
   queue_up = 0;
   TclassApply(p->inc, p->n_win, p->h * 5, p->h, 0, 0, STATE_CLICKED,
	       0, p->tnc, s);
