		  ...
		  Elogin_Image * lpbox, Elogin_Image * lp1, Elogin_Image * lp2)
{
   ...
   e_window_property_set(default_root, a, XA_PIXMAP, 32, &pm, 1);
   //      (unsigned char *)&pm, 1);
   ...
   imlib_blend_image_onto_image(lp1->im, 0,
				0, 0, lp1->w, lp1->h, 50, 25, lp1->w, lp1->h);
   imlib_blend_image_onto_image(lp2->im, 0,
				0, 0, lp2->w, lp2->h, 50, 55, lp2->w, lp2->h);
   ...
   imlib_context_set_image(lbox->im);
   imlib_free_image();
   imlib_context_set_image(llogo->im);
   imlib_free_image();
   imlib_context_set_image(lpbox->im);
   imlib_free_image();
   imlib_context_set_image(lp1->im);
   imlib_free_image();
   imlib_context_set_image(lp2->im);
   imlib_free_image();
}
...
Elogin_SetRootBG(void)
{
   ...
   e_window_set_background_pixmap(default_root, pm);
   e_window_resize(default_root, main_view->bg->w, main_view->bg->h);
   e_window_clear(default_root);
   e_window_show(default_root);
   e_sync();
   imlib_free_image();
...
Elogin_ViewNew(void)
{
   ...
   view->win = e_window_new(default_root);
