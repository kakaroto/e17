...
void
_mouse_in (void *_data, Evas _e, Evas_Object _o, int _b, int _x, int _y) {
	double x=0,y=0,w=0,h=0;
	
	printf("mouse in evas callback...\n");
	
	evas_get_geometry( _e, _o, &x, &y, &w, &h );
	evas_resize(_e,_o, raptor_w+RAPTOR_GROW_SIZE,raptor_h+RAPTOR_GROW_SIZE);
	evas_move(_e,_o, x-RAPTOR_GROW_SIZE/2, y - RAPTOR_GROW_SIZE/2);
	evas_set_image_fill(_e,_o,0,0,raptor_w+RAPTOR_GROW_SIZE,raptor_h+RAPTOR_GROW_SIZE);
	gevas_queue_redraw( (GtkgEvas*)_data );
}
void
_mouse_out (void *_data, Evas _e, Evas_Object _o, int _b, int _x, int _y) {
	double x=0,y=0,w=0,h=0;
...
void
_mouse_down (void *_data, Evas _e, Evas_Object _o, int _b, int _x, int _y) {
...
void
_mouse_up (void *_data, Evas _e, Evas_Object _o, int _b, int _x, int _y) {
...
void
_mouse_move (void *_data, Evas _e, Evas_Object _o, int _b, int _x, int _y) {
...
static gint 
cleanup_evas_raw_cb(GtkObject* o, gpointer data)
{
	Evas evas; 
...
void
setup_menu_raptor( GtkWidget* gevas ) 
{
	Evas evas = gevas_get_evas( GTK_GEVAS(gevas) );
    
	if((menu_raptor = evas_add_image_from_file(evas, PACKAGE_DATA_DIR"/raptor2.jpg"))) {
...
void make_text( GtkWidget* gevas ) 
{
	...
	ct = t2 = (GtkgEvasObj*)gevastext_new_full ( GTK_GEVAS(gevas), "andover", 70, "Evas");
	...
	ct = t3 = (GtkgEvasObj*)gevastext_new_full(GTK_GEVAS(gevas), "andover", 90, "Linux");
	...
	ct = t4 = (GtkgEvasObj*)gevastext_new_full(GTK_GEVAS(gevas), "andover", 50, "The Cat!");
	gevasobj_set_gevas( ct, gevas );
	...
	ct = t5 = (GtkgEvasObj*)gevastext_new_full(GTK_GEVAS(gevas), "andover", 55, "Purrs");
	gevasobj_set_gevas( ct, gevas );
	gevasobj_move( ct, 350, 400 );
	gevasobj_set_layer( ct, 1 );
//	gevasobj_show( ct );	
...
void
make_draw_icon( GtkWidget* gevas ) 
{
	GtkgEvasObj *ct;
...
//  gtk_container_add(GTK_CONTAINER(window), gevas);
  ...
  gevas_set_middleb_scrolls( GTK_GEVAS(gevas), 1, 
	...
	gtk_signal_connect(GTK_OBJECT(gevas),
                     "destroy",
                     GTK_SIGNAL_FUNC(cleanup_evas_raw_cb),
                     menu_raptor);
...
static gint 
gpopmenu_handle_cb(GtkWidget* window, gpointer data)
{
