...
//    printf("col_item_move_rel() x:%f y:%f\n",*x,*y);
...
void gevas_selectable_set_backref(GtkgEvasEvHSelectable * ev, GtkgEvasObj* o )
{
    ...
    gtk_signal_connect( o, "move_relative", GTK_SIGNAL_FUNC(col_item_move_relative), ev);
	...
	GtkgEvasEvHSelectable *ev )
{
    ...
    printf("col_item_move_absolute() x:%f y:%f\n",*x,*y);
...
gevasevh_selectable_set_arg(GtkObject * object, GtkArg * arg, guint arg_id)
{
	...
	switch (arg_id) {
		case ARG_SELECTED_OBJ:
            ...
            gtk_signal_connect( ev->selected, "move_absolute",
