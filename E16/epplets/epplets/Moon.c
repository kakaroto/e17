...
double              Moon(double T, double *LAMBDA, double *BETA, double *R,
			 double *AGE);
double              sine(double);
double              frac(double);
...
void
addsol(double COEFFL, double COEFFS, double COEFFG, double COEFFP, int P, int Q,
       int R, int S)
{
   double              X, Y;
...
double
Moon(double T, double *LAMBDA, double *BETA, double *R, double *AGE)
{
   double              T2;
   ...
   DLS = -6.40 * S1 - 1.89 * S6;
   DF =
      0.21 * S1 + 0.31 * S2 + 14.27 * S3 - 88.70 * S4 - 15.30 * S5 + 0.24 * S6 -
      1.86 * S7;
   DD = DL0 - DLS;
   ...
   L0 =
      TwoPi * frac(0.60643382 + 1336.85522467 * T - 0.00000313 * T2) +
      DL0 / ARC;
   L = TwoPi * frac(0.37489701 + 1325.55240982 * T + 0.00002565 * T2) +
      DL / ARC;
   LS =
      TwoPi * frac(0.99312619 + 99.99735956 * T - 0.00000044 * T2) + DLS / ARC;
   F = TwoPi * frac(0.25909118 + 1342.22782980 * T - 0.00000892 * T2) +
      DF / ARC;
   D = TwoPi * frac(0.82736186 + 1236.85308708 * T - 0.00000397 * T2) +
      DD / ARC;
   ...
   for (i = 1; i <= 4; ++i)
	...
	for (j = 2; j <= MAX; ++j)
	   addthe(CO[6 + j - 1][i], SI[6 + j - 1][i], CO[6 + 1][i],
		  SI[6 + 1][i], &CO[6 + j][i], &SI[6 + j][i]);
	for (j = 1; j <= MAX; ++j)
	  {
	     CO[6 - j][i] = CO[6 + j][i];
	     SI[6 - j][i] = -SI[6 + j][i];
   ...
   *BETA =
      (FAC * (18518.511 + 1.189 + GAM1C) * sin(S) - 6.24 * sin(3 * S) +
       N) / 3600.0;
...
double
NewMoon(double ax, double bx, double cx)
{
...
MiniMoon(double T, double *RA, double *DEC)
{
   ...
   DL =
      +22640.0 * sin(L) - 4586.0 * sin(L - 2.0 * D) + 2370.0 * sin(2.0 * D) +
   ...
   H = F - 2.0 * D;
   N = -526.0 * sin(H) + 44.0 * sin(L + H) - 31.0 * sin(-L + H) -
      23.0 * sin(LS + H) + 11.0 * sin(-LS + H) - 25.0 * sin(-2.0 * L + F) +
      21.0 * sin(-L + F);
