...
/* const strings */
static const char *_mbar_main_orientation[] = { "left", "right", "top", "bottom" };
...
_mbar_icon_pos_find(MBar_Bar *mbb, int x, int y)
{
   ...
   if ((e_gadman_client_edge_get(mbb->gmc) == E_GADMAN_EDGE_BOTTOM) || (e_gadman_client_edge_get(mbb->gmc) == E_GADMAN_EDGE_TOP))
     {
        ...
        if (((dx * dx) + (dy * dy)) > (e_config->drag_resist * e_config->drag_resist))
          {
             ...
             d = e_drag_new(ic->mbb->con, x, y, drag_types, 1, ic->app, -1, _mbar_bar_cb_finished);
...
_mbar_bar_cb_enter(void *data, const char *type, void *event)
{
   ...
   edje_object_signal_emit(o2, "set_orientation", _mbar_main_orientation[e_gadman_client_edge_get(mbb->gmc)]);
   evas_object_resize(o, mbb->mbar->conf->iconsize, mbb->mbar->conf->iconsize);
   ...
   switch (change)
     ...
     case E_GADMAN_CHANGE_MOVE_RESIZE:
        ...
        e_drop_handler_geometry_set(mbb->drop_handler,
                                    mbb->x + mbb->bar_inset.l, mbb->y + mbb->bar_inset.t,
                                    mbb->w - (mbb->bar_inset.l + mbb->bar_inset.r), mbb->h - (mbb->bar_inset.t + mbb->bar_inset.b));
        break;
