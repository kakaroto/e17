		       ...
		       E_Config_Dialog_Data * cfdata)
{
  ...
  ob =
    e_widget_check_add (evas, _("Allow windows to overlap this gadget"),
  ...
  if (mb->apps->subapps)
      ...
      for (l = mb->apps->subapps; l; l = l->next)
	{
	  ...
	  e_widget_ilist_append (il, ic, strdup (app->name), NULL, app,
				 strdup (app->name));
	}
    }
  ...
  ob =
    e_widget_button_add (evas, _("Edit Point"), NULL, _edit_point, il, cfd);
  e_widget_table_object_append (ot, ob, 1, 3, 1, 1, 1, 0, 1, 0);
  ob =
    e_widget_button_add (evas, _("Delete Point"), NULL, _delete_point, il,
			 cfd);
			  ...
			  E_Config_Dialog_Data * cfdata)
{
  ...
  e_widget_framelist_object_append (of, ob);
  ob =
    e_widget_slider_add (evas, 1, 0, _("%1.2f px/s"), 0.0, 1.0, 0.01, 0,
  ...
  ob =
    e_widget_slider_add (evas, 1, 0, _("%3.0f pixels"), 8.0, 128.0, 1.0, 0,
...
_mbar_point_new (Evas_Object * il, E_App * eap)
{
  ...
  edje_object_file_set (ic, buf, "icon");
  e_widget_ilist_append (il, ic, strdup (eap->name), NULL, eap,
			 strdup (eap->name));
}
