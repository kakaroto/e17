...
int DEVIANF(source_original_size_get) (DEVIANN *devian, int part, int *w, int *h)
{
   switch (devian->conf->source_type)
     {
#ifdef HAVE_PICTURE
     case SOURCE_PICTURE:
        return DEVIANF(source_picture_original_size_get) (devian->source, part, w, h);
#endif
#ifdef HAVE_RSS
     case SOURCE_RSS:
        return DEVIANF(source_rss_original_size_get) (devian->source, part, w, h);
#endif
#ifdef HAVE_FILE
     case SOURCE_FILE:
        return DEVIANF(source_file_original_size_get) (devian->source, w, h);
        break;
#endif
     case SOURCE_NO:
        break;
