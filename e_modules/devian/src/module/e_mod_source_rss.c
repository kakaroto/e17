...
int DEVIANF(source_rss_evas_object_get)(DEVIANN *devian,
                                          Evas_Object **rss0,
                                          Evas_Object **rss1)
{
  Source_Rss *source;
...
int DEVIANF(source_rss_original_size_get)(Source_Rss *source,
...
void DEVIANF(source_rss_dialog_infos)(Source_Rss *source)
{
  ...
  snprintf( buf, 4096,
	    ...
	    doc->name, doc->url,
	    doc->version, doc->host,
	    doc->link, doc->description);
  ...
  if (DEVIANM->conf->sources_rss_popup_news &&
      source->devian->conf->rss_popup_news)
    DEVIANF(popup_warn_add)(&source->devian->popup_warn,
...
nderline=on underline_color=#000>Link: %s</>",
