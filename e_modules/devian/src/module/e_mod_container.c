...
const char *DEVIANF(container_edje_load)(Evas_Object *edje_obj, char *part,
					 DEVIANN *devian,
					 int container_type, void *container)
{
  ...
  edje_object_part_geometry_get(edje_obj, "source0",
				NULL, NULL, &source_w, &source_h);
  border_w = (400 - source_w);
  border_h = (400 - source_h);
  DCONTAINER(("Theme borders: x:%d y:%d",
	      border_w,
	      border_h));
