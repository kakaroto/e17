...
static void _cb_item_open(void *data, Evas_Object *obj,
			  const char *emission, const char *source);
static void _cb_infos_set(void *data, Evas_Object *obj,
			 const char *emission, const char *source);
static void _cb_infos_unset(void *data, Evas_Object *obj,
			   const char *emission, const char *source);
static void _cb_infos_scroll(void *data, Evas_Object *obj,
			    const char *emission, const char *source);
...
int DEVIANF(data_rss_new)(Source_Rss *source)
{
  ...
  DEVIANF(data_rss_doc_set_new)(feed,
				feed->source->devian->conf->rss_doc,
...
int DEVIANF(data_rss_poll)(void *data, int force_retry)
{
  ...
  if (feed->proxy.port)
    feed->server = ecore_con_server_connect(ECORE_CON_REMOTE_SYSTEM,
					    feed->proxy.host, feed->proxy.port, feed);
  else
    feed->server = ecore_con_server_connect(ECORE_CON_REMOTE_SYSTEM,
					    feed->source->devian->conf->rss_doc->host, 80, feed);
...
Rss_Doc *DEVIANF(data_rss_doc_new)(Rss_Doc *s, int add)
{
  ...
  if (s)
	...
	if ( strncmp(s->url, "http://", 7) ) return NULL;
	if ( strlen(s->url) <= 7 ) return NULL;
	p = (char *)(s->url + 7);
	if ( !(p2 = strchr(p, '/')) ) return NULL;
	if (!s->description) s->description = evas_stringshare_add("");
	if (s->host) evas_stringshare_del(s->host);
	if ( !(s->host = _get_host_from_url(s->url)) )
	  return NULL;
	if (s->file) evas_stringshare_del(s->file);
      ...
      if (add)
	{
	  /* Check if s already exists */
	  {
		...
		if ( !strcmp(s->url, doc->url) ) return NULL;
	      }
	  }
	  ...
	  DEVIANM->conf->sources_rss_docs =
	    evas_list_append(DEVIANM->conf->sources_rss_docs, s);
	}
...
int DEVIANF(data_rss_doc_set_new)(Rss_Feed *feed, Rss_Doc *doc, const char *url)
{
  ...
  if (doc->user)
    {
      ...
      snprintf(buf, sizeof(buf),
               _(
                 "<hilight>Rss '%s' is aready in use !</hilight><br><br>"
                 "You can't set a rss document to more than one dEvian<br><br>"
                 "Url: %s"
                 ),
               doc->name, doc->url
               );
  ...
  feed->source->devian->conf->rss_url =
    evas_stringshare_add(feed->source->devian->conf->rss_doc->url);
...
void DEVIANF(data_rss_article_free)(Rss_Article *article)
{
  ...
  if (article->obj)
    {
      edje_object_signal_callback_del(article->obj, "infos_set",
				      "item", _cb_infos_set);
      edje_object_signal_callback_del(article->obj, "infos_unset",
				      "item", _cb_infos_unset);
      edje_object_signal_callback_del(article->obj, "open",
				      "item", _cb_item_open);
	  ...
	  if (!p)
	    {
	      snprintf(buf, sizeof(buf),
		       _(
			 "<hilight>Error reading '%s' !</hilight><br><br>"
	  ...
	  else
	    {
	      snprintf(buf, sizeof(buf),
		       _(
			 "<hilight>Error reading '%s' feed !</hilight><br><br>"
			 ...
			 "%100.100s"
		       ...
		       doc->name, doc->url, feed->buffer, p
		       );
	      e_module_dialog_show(_(MODULE_NAME " Module Error"), buf);
	    }
	}
...
static int _feed_sort(Ecore_List *list)
{
  ...
  if (!first->date_simple || !last->date_simple) return 1;

  cmp = strcmp(first->date_simple, last->date_simple);

  if (!cmp || cmp > 0) return 1;
...
static int _table_create(Rss_Feed *feed, Ecore_List *list, Ecore_List *old_list, Evas_Object *table)
{
  ...
  while ( (article = (Rss_Article *)ecore_list_next(list)) != NULL )
      ...
      else
	{
		  ...
		  if ( strcmp(feed->last_time, "") )
		    {
			  ...
			  feed->source->devian->conf->rss_popup_news)
			{
			  if (article->date_simple)
			    DEVIANF(popup_warn_add)(&feed->source->devian->popup_warn,
						  POPUP_WARN_TYPE_DEVIAN,
						  article->date_simple, feed->source->devian);
			  else
 			    DEVIANF(popup_warn_add)(&feed->source->devian->popup_warn,
						  POPUP_WARN_TYPE_DEVIAN,
						  "News", feed->source->devian);
			}
		    }
		  else
		    article->new = 0;
	  ...
	  e_table_pack_options_set(article->obj,
				   1, 1, /* fill */
				   1, 1, /* expand */
				   0, 0, /* align */
				   0, 0, /* min */
		       ...
		       const char *emission, const char *source)
{
  ...
  if (!url)
    {
      snprintf(buf, sizeof(buf),
               _(
                 "<hilight>Unable to open link</hilight><br><br>"
                 "Article '%s' has no link associated !"
                 ),
               article->title);
      e_module_dialog_show(_(MODULE_NAME " Module Error"), buf);
      return;
  ...
  if (ecore_file_app_installed(DEVIANM->conf->viewer_http))
  ...
  else
    {
      snprintf(buf, sizeof(buf),
               _(
                 "<hilight>Viewer %s not found !</hilight><br><br>"
