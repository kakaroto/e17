...
static void _check_overlap(int *px, int *py, int *pw, int *ph,
				 int tries, int org_x, int org_y);
static int _cb_timer(void *data);
static void _cb_edje_close(void *data, Evas_Object *obj,
			   const char *emission, const char *source);
static void _cb_edje_next(void *data, Evas_Object *obj,
			  const char *emission, const char *source);
static void _cb_edje_desactivate(void *data, Evas_Object *obj,
				 const char *emission, const char *source);
			    ...
			    const char *text, void *data)
{
  ...
  if (popw)
	...
	case POPUP_WARN_TYPE_DEVIAN:
	  devian = (DEVIANN *)data;
	  popw->name =
	    evas_stringshare_add(DEVIANF(source_name_get)(devian, -1));
	  edje_object_part_text_set(popw->face, "name", popw->name);
	  px = DEVIANM->canvas_w-(fw+20);
	  py = DEVIANM->canvas_h-(fh+20);
	  /* Log */
	  popw->log = evas_list_append(popw->log,
				       evas_stringshare_add(text));
	  /* Timer */
#ifdef HAVE_RSS
	...
	default: popw->name = evas_stringshare_add("");
	}
