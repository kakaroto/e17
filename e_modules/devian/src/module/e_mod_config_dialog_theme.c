...
static void _ilist_themes_general_append(E_Config_Dialog *cfd, Evas_Object *il,
					 Evas *evas);
#ifdef HAVE_PICTURE
static void _ilist_themes_picture_append(E_Config_Dialog *cfd, Evas_Object *il,
					 Evas *evas);
#endif
#ifdef HAVE_RSS
static void _ilist_themes_rss_append(E_Config_Dialog *cfd, Evas_Object *il,
					 Evas *evas);
#endif
#ifdef HAVE_FILE
static void _ilist_themes_file_append(E_Config_Dialog *cfd, Evas_Object *il,
					 Evas *evas);
#endif
static void _ilist_themes_popup_append(E_Config_Dialog *cfd, Evas_Object *il,
					 Evas *evas);
...
E_Config_Dialog *DEVIANF(config_dialog_theme)(void)
{
  ...
  cfd = e_config_dialog_new(DEVIANM->container, _(MODULE_NAME " Module Configuration - Theme"),
			    NULL, 0, v, NULL);
...
_fill_data(E_Config_Dialog_Data *cfdata)
{
  ...
  snprintf(buf, sizeof(buf), "%s/%s",
	   e_module_dir_get(DEVIANM->module),
	   "module_icon.png");
  ...
  if ( strcmp(cfdata->theme_general, DEVIANM->conf->theme_picture) )
    {
      ...
      DEVIANF(container_theme_change)(NULL, SOURCE_PICTURE,
  ...
  if ( strcmp(cfdata->theme_general, DEVIANM->conf->theme_rss) )
    {
      ...
      DEVIANF(container_theme_change)(NULL, SOURCE_RSS,
  ...
  if ( strcmp(cfdata->theme_general, DEVIANM->conf->theme_file) )
    {
      ...
      DEVIANF(container_theme_change)(NULL, SOURCE_FILE,
...
static void _invalid_theme_error_msg(void)
{
  ...
  snprintf(buf, sizeof(buf),
	   _(
	     "<hilight>Invalid theme</hilight><br><br>"
	     "You have selected an invalid theme.<br>"
	     "A valid theme has a dEvian icon in front of it."
	     ));
  e_module_dialog_show(_(MODULE_NAME " Module Error"), buf);
}
