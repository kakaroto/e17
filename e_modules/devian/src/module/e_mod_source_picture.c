...
int DEVIANF(source_picture_change)(DEVIANN *devian, int option)
{
  ...
  if (!DEVIANF(data_picture_cache_attach)(source,
					  !old_part, option))
    return 0;
...
int DEVIANF(source_picture_set_bg)(DEVIANN *devian)
{
  ...
  if (!strstr(file, ".edj"))
      ...
      else
	{
	  snprintf(buf, sizeof(buf),
		   _(
		     "<hilight>e17setroot needed</hilight><br><br>"
      ...
      else
	{
	  ...
	  e_lib_desktop_background_add(zone->container->num, zone->num, zone->desk_x_current, zone->desk_y_current, strdup(file));
	}
    }
  ...
  if (!ecore_file_exists(file))
    {
      snprintf(buf, sizeof(buf),
               _(
                 "<hilight>File %s doesnt exists !</hilight><br><br>"
		 "This file is in dEvian's picture list, but it seems you removed it from the disk.<br>"
		 "It cant be set opened in a viewer, sorry."
                 ),
               file
               );
      e_module_dialog_show(_(MODULE_NAME " Module Error"), buf);
...
int DEVIANF(source_picture_evas_object_get)(DEVIANN *devian,
...
int DEVIANF(source_picture_original_size_get)(Source_Picture *source,
...
void DEVIANF(source_picture_dialog_infos)(Source_Picture *source)
{
  ...
  snprintf( buf, 4096,
	    ...
	    DEVIANF(source_picture_name_get)(source, edje_part),
	    path,
...
char *DEVIANF(source_picture_infos_get)(Source_Picture *source, int part)
{
  ...
  if ( ecore_file_exists(picture->path) )
    {
      ...
      snprintf( buf_ext, sizeof(buf_ext),
		...
		date_ascii,
		(float)ecore_file_size(picture->path)/1000000.0 );
    }
  else
    {
      snprintf(buf_ext, sizeof(buf_ext),
	       "This file does not exist anymore on the disk !");
    }
  ...
  snprintf(buf, sizeof(buf),
	   ...
	   "<underline=on underline_color=#000>Path :</> %s<br><br>"
	   "%s",
...
void DEVIANF(source_picture_timer_change)(DEVIANN *devian, int active, int time)
{
  ...
  if (source->timer)
    ecore_timer_del(source->timer);
  source->timer = ecore_timer_add(time,
				  _cb_timer_picture_change,
				  source);
}
...
{
  source->histo = evas_list_prepend(source->histo,
                                    picture);
  source->histo_pos = 0;

  if (!evas_list_find(picture->sources_histo, source))
    picture->sources_histo = evas_list_append(picture->sources_histo,
					      source);
...
void DEVIANF(source_picture_histo_picture_detach)(Source_Picture *source, Picture *picture)
{
  ...
  picture->sources_histo = evas_list_remove(picture->sources_histo,
                                            source);
}
