...
E_Config_Dialog *
DEVIANF(config_dialog_main)(E_Container *con)
{
  E_Config_Dialog *cfd;
...
_fill_data(E_Config_Dialog_Data *cfdata)
{
  ...
  cfdata->viewer_http =
    strdup(conf->viewer_http);
  cfdata->viewer_image =
    strdup(conf->viewer_image);
  cfdata->viewer_file =
    strdup(conf->viewer_file);
...
#endif
#ifdef HAVE_RSS
  cfdata->sources_rss_timer_s =
    conf->sources_rss_timer_s/60;
  cfdata->sources_rss_popup_news =
    conf->sources_rss_popup_news;
  cfdata->sources_rss_popup_news_timer =
    conf->sources_rss_popup_news_timer;
#endif
#ifdef HAVE_FILE
  cfdata->sources_file_font_size =
    conf->sources_file_font_size;
#endif
...
#ifdef HAVE_PICTURE
  cfdata->data_picture_thumb_default_size =
    conf->data_picture_thumb_default_size;
  cfdata->sources_picture_set_bg_purge =
    conf->sources_picture_set_bg_purge;
#endif
#ifdef HAVE_FILE
  cfdata->sources_file_nb_lines_ini =
    conf->sources_file_nb_lines_ini;
  cfdata->sources_file_nb_lines_ini =
    conf->sources_file_nb_lines_ini;
#endif
}
...
_main_basic_apply_data(E_Config_Dialog *cfd, E_Config_Dialog_Data *cfdata)
{
...
#ifdef HAVE_PICTURE
  if ( strcmp(DEVIANM->conf->sources_picture_data_import_dir,
	      cfdata->sources_picture_data_import_dir) )
    {
...
#endif
#ifdef HAVE_RSS
  DEVIANM->conf->sources_rss_timer_s =
    cfdata->sources_rss_timer_s*60;
  ...
  if ((DEVIANM->conf->boxs_always_id != cfdata->boxs_always_id) ||
...
_main_advanced_apply_data(E_Config_Dialog *cfd, E_Config_Dialog_Data *cfdata)
{
  ...
  if ( strcmp(DEVIANM->conf->viewer_http,
	      cfdata->viewer_http) &&
       cfdata->viewer_http[0])
    {
      evas_stringshare_del(DEVIANM->conf->viewer_http);
      DEVIANM->conf->viewer_http =
	evas_stringshare_add(cfdata->viewer_http);
    }
  if ( strcmp(DEVIANM->conf->viewer_image,
	      cfdata->viewer_image) &&
       cfdata->viewer_image[0])
    {
      evas_stringshare_del(DEVIANM->conf->viewer_image);
      DEVIANM->conf->viewer_image =
	evas_stringshare_add(cfdata->viewer_image);
    }
  if ( strcmp(DEVIANM->conf->viewer_file,
	      cfdata->viewer_file) &&
       cfdata->viewer_file[0])
    {
      evas_stringshare_del(DEVIANM->conf->viewer_file);
      DEVIANM->conf->viewer_file =
	evas_stringshare_add(cfdata->viewer_file);
    }
  ...
  if (DEVIANM->conf->boxs_anim_ghost_timer != cfdata->boxs_anim_ghost_timer)
      ...
      for(l = DEVIANM->devians; l; l=evas_list_next(l))
	{
	  ...
	  if (devian->conf->container_type == CONTAINER_BOX)
	    {
	      DEVIANF(container_box_animation_start)(devian->container,
						     CONTAINER_BOX_ANIM_GHOST);
	    }
	}
    }
#ifdef HAVE_PICTURE
...
_main_advanced_create_widgets(E_Config_Dialog *cfd, Evas *evas, E_Config_Dialog_Data *cfdata)
{
  ...
  e_widget_frametable_object_append(of, ob,
				    0, 7, 2, 1,
				    1, 1, 1, 1);
  ob = e_widget_slider_add(evas, 1, 0, _("%1.0f secs"), 7.0, 120.0, 1.0, 0,
			   NULL, &(cfdata->boxs_anim_ghost_timer), 100);
  e_widget_frametable_object_append(of, ob,
				    0, 8, 2, 1,
				    1, 0, 1, 0);
  e_widget_table_object_append(o, of,
  ...
  ob = e_widget_slider_add(evas, 1, 0, _("%1.0f min"), (float)SOURCE_RSS_UPDATE_RATE_MIN/60, (float)SOURCE_RSS_UPDATE_RATE_MAX/60, 1.0, 0,
			   NULL, &(cfdata->sources_rss_timer_s), 130);
  e_widget_frametable_object_append(of, ob,
				    0, 2, 1, 1,
				    1, 0, 1, 0);
  ob = e_widget_check_add(evas, _("News popup active"), &(cfdata->sources_rss_popup_news));
