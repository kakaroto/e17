  ...
  if (cfg_main)
	...
	{  
	  ...
	  snprintf(buf, sizeof(buf),
		   _(
		     "<hilight>Configuration Upgraded</hilight><br><br>"
	  ...
	  if (cfg_main->conf_version > CONFIG_VERSION)
	    {
	      ...
	      snprintf(buf, sizeof(buf),
		       _(
			 "<hilight>Configuration Downgraded</hilight><br><br>"
			 "Your configuration of " MODULE_NAME " module<br>"
  ...
  for (l = DEVIANM->devians; l; l=evas_list_next(l))
    {
      ...
      cfg_main->devians_conf = evas_list_append(cfg_main->devians_conf,
						devian->conf);
...
DEVIAN_MAIN_CONF *DEVIANF(config_main_new)(void)
{
  ...
  cfg_main->sources_rss_popup_news_timer = POPUP_WARN_TIMER_DEFAULT;
  {
    ...
    cfg_main->sources_rss_docs =
      evas_list_append(cfg_main->sources_rss_docs, doc);
...
DEVIAN_CONF *DEVIANF(config_devian_new)(int source_type, DEVIAN_CONF *conf)
{
  ...
  if (conf)
	...
	cfg->rss_url = NULL;
      ...
      DEVIANF(container_box_random_pos_get)(NULL, &cfg->box_x, &cfg->box_y,
					  cfg->box_max_size_source);
...
static E_Config_DD *_devian_config_main_edd_init(void)
{
...
static E_Config_DD *_devian_config_devians_edd_init(void)
{
  E_Config_DD *devians;
...
#ifdef HAVE_RSS
static E_Config_DD *_devian_config_rss_docs_edd_init(void)
{
