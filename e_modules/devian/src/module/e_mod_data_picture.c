  ...
  if (list->nb_pictures_waiting > 2)
      ...
      if (DEVIANM->conf->sources_picture_data_import_thumbs_warn)
	{
	  ...
	  snprintf(buf, sizeof(buf),
		   _(
		     "<hilight>Creating thumbs</hilight><br><br>"
      ...
      else
	{
	  char buf[100];
	  int time = 3;
	  snprintf(buf, sizeof(buf), "%d pictures in thumbnail",
		   list->nb_pictures_waiting-2);
	  DEVIANF(popup_warn_add)(NULL, POPUP_WARN_TYPE_INFO_TIMER,
				  buf, &time);
	}
    }
					    ...
					    int edje_part, int histo_nb)
{
  ...
  if (histo_nb || source->histo_pos)
	...
	if ( cache->pos >
	     ( evas_list_count(cache->pictures) -
	       (int)(evas_list_count(cache->pictures) / 4) ) )
	  _picture_cache_fill();
...
static int _picture_list_local_add_dir(const char *dir, int recursive)
{
  ...
  if (!ecore_file_is_dir(dir))
    {
      snprintf(buf2, sizeof(buf2),
	       _(
		 "<hilight>Directory %s doesnt exists.</hilight><br><br>"
  ...
  if (!strcmp(DEVIANM->conf->sources_picture_data_import_dir, dir))
	...
	snprintf(buf2, sizeof(buf2),
		 _(
		   "<hilight>Directory %s is empty</hilight><br><br>"
		   "To import pictures, you have to put them"
		   "in this folder.<br>"
		   "They can be jpeg, gif, png, edj<br>"
		   "After import, if you can remove these files and the pictures still can<br>"
  ...
  while ((file = (char*)ecore_list_next(files)) != NULL)
	...
	continue;
      ...
      picture->picture_description->name =
	DEVIANF(data_picture_get_name_from_path)(picture->path,
						 DATA_PICTURE_INFOS_LEN);
	  ...
	  if ((th_w > w) && (th_h > h))
	    {
	      ...
	      DDATA(("File %s thumb exists (%dx%d),  but regen to %dx%d (del old %d)",
		     file, w, h, th_w, th_h, i));
	      e_thumb_generate_begin(picture->path, th_w, th_h,
				     DEVIANM->container->bg_evas,
				     &picture->picture,
				     _picture_local_thumb_cb,
				     picture);
	      list->nb_pictures_waiting++;
	      continue;
	    }
	  /* Thumb exists and good size */
	  DDATA(("File %s thumb exists and good size, add (%de)", file,
		 evas_list_count(list->pictures)));
	  picture->thumbed = 1;
...
static Picture *_picture_list_get_picture(Evas_List *pictures_list)
{
	...
	case DATA_PICTURE_BOTH:
	  {
	    ...
	    if (!i)
	      picture =
		_picture_list_get_picture(DEVIANM->picture_list_local->pictures);
	    else
	      picture =
		_picture_list_get_picture(DEVIANM->picture_list_net->pictures);
  ...
  l = evas_list_nth_list( pictures_list,
			  rand()%evas_list_count(pictures_list) );
  was_first = l;
...
static Picture *_picture_cache_get_picture_unused(void)
{
  ...
  if (cache->pos == -1)
    {
      l = evas_list_nth_list(cache->pictures,
			     rand()%evas_list_count(cache->pictures));
    }
  else
    {
      i = rand()%cache->pos;
...
static int _picture_free(Picture *picture, int force, int force_now)
{
  ...
  if (picture->from == DATA_PICTURE_LOCAL)
    {
      DEVIANM->picture_list_local->pictures =
	evas_list_remove(DEVIANM->picture_list_local->pictures, picture);
    }
  else
    {
      fprintf(stderr, "DEL FROM NET !!!!!!\n");
      DEVIANM->picture_list_net->pictures =
	evas_list_remove(DEVIANM->picture_list_net->pictures, picture);
    }
