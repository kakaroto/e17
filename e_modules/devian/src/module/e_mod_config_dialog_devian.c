...
E_Config_Dialog *
DEVIANF(config_dialog_devian)(E_Container *con, DEVIANN *devian)
{
  E_Config_Dialog *cfd;
...
void DEVIANF(config_dialog_devian_rss_doc_update)(Rss_Doc *doc)
{
  ...
  for(l = DEVIANM->devians; l; l=evas_list_next(l))
	   ...
	   (devian->dialog_conf) )
	{
	  ...
	  _ilist_rss_docs_append(devian->dialog_conf,
				 cfdata->sources_rss_docs_ilist,
				 DEVIANM->conf->sources_rss_docs);
	}
    }
}
#endif
...
_fill_data(E_Config_Dialog_Data *cfdata)
{
  ...
  if (cfdata->s_picture)
      ...
      if (cfdata->s_rss)
	{
	  if (d_conf->rss_doc)
	    {
	      cfdata->source_rss_url = 
		strdup(d_conf->rss_doc->url);
...
_basic_apply_data(E_Config_Dialog *cfd, E_Config_Dialog_Data *cfdata)
{
  ...
  if (cfdata->c_box)
      ...
      if (devian->conf->box_max_size_source != cfdata->container_box_size)
	{
	  ...
	  cfdata->c_box->max_size = devian->conf->box_max_size_source
	    + cfdata->c_box->theme_border_h;
	...
	{
	  DEVIANF(container_box_infos_text_change_set)(devian->container,
						       cfdata->container_box_infos_show);
	}
  ...
  if (cfdata->s_rss)
	...
	{
	  DEVIANF(data_rss_doc_set_new)(cfdata->s_rss->rss_feed,
					cfdata->source_rss_doc,
					NULL);
	}
...
_advanced_apply_data(E_Config_Dialog *cfd, E_Config_Dialog_Data *cfdata)
{
  ...
  if (cfdata->s_picture)
	...
	devian->source_func.timer_change(devian,
					 cfdata->source_picture_timer_yn,
					 cfdata->source_picture_timer);
      else
	if (devian->conf->picture_timer_s != cfdata->source_picture_timer )
	  devian->source_func.timer_change(devian,
					   devian->conf->picture_timer_active,
					   cfdata->source_picture_timer);
    }
  else
    {
      ...
      if (cfdata->s_rss)
	{
	  ...
	  if (devian->conf->rss_timer_active != cfdata->source_rss_timer_yn)
	    devian->source_func.timer_change(devian,
					     cfdata->source_rss_timer_yn,
					     cfdata->source_rss_timer*60);
	  else
	    if (devian->conf->rss_timer_s != cfdata->source_rss_timer )
	      devian->source_func.timer_change(devian,
					       devian->conf->rss_timer_active,
					       cfdata->source_rss_timer*60);
	  devian->conf->rss_popup_news = cfdata->source_rss_popup_news;
  ...
  if (cfdata->c_box)
    { 
      ...
      e_widget_frametable_object_append(of, ob,
					0, 2, 1, 1,
					1, 1, 1, 1);
      ob = e_widget_slider_add(evas, 1, 0, _("%1.0f"), 10.0, 255.0, 1.0, 0,
			       NULL, &(cfdata->container_box_alpha), 100);
      e_widget_frametable_object_append(of, ob,
					1, 2, 1, 1,
					1, 0, 1, 0);
      ...
      e_widget_frametable_object_append(of, ob,
					0, 6, 2, 1,
					1, 1, 1, 1);
      e_widget_table_object_append(o, of,
				   0, 0, 1, 1,
				   1, 1, 1, 1);
    }
  ...
  if(cfdata->s_picture)
      ...
      if (cfdata->s_rss)
	{
	  ...
	  e_widget_frametable_object_append(of, ob,
					    0, 1, 3, 1,
					    1, 1, 1, 1);
	  ob = e_widget_button_add(evas, "Add", NULL,
				   _cb_button_rss_doc_add, cfd, NULL);
	  e_widget_frametable_object_append(of, ob,
					    0, 2, 1, 1,
					    1, 1, 1, 1);
	  ob = e_widget_button_add(evas, "Edit", NULL,
				   _cb_button_rss_doc_edit, cfd, NULL);
	  e_widget_frametable_object_append(of, ob,
					    1, 2, 1, 1,
					    1, 1, 1, 1);
	  ob = e_widget_button_add(evas, "Del", NULL,
				   _cb_button_rss_doc_del, cfd, NULL);
	  e_widget_frametable_object_append(of, ob,
					    2, 2, 1, 1,
					    1, 1, 1, 1);
	  ...
	  e_widget_frametable_object_append(of, ob,
					    0, 3, 1, 1,
					    1, 1, 1, 1);
	  ob = e_widget_slider_add(evas, 1, 0, _("%1.0f"), 1.0, 20.0, 1.0, 0,
				   NULL, &(cfdata->source_rss_nb_items), 90);
	  e_widget_frametable_object_append(of, ob,
					    1, 3, 2, 1,
					    1, 0, 1, 0);
  ...
  if (cfdata->c_box)
    { 
      ...
      e_widget_frametable_object_append(of, ob,
					0, 6, 1, 1,
					1, 1, 1, 1);
      ob = e_widget_slider_add(evas, 1, 0, _("%1.0f"), 1.0, 3.0, 1.0, 0,
			       NULL, &(cfdata->container_box_speed), 100);
      e_widget_frametable_object_append(of, ob,
					1, 6, 1, 1,
					1, 0, 1, 0);
  ...
  if(cfdata->s_picture)
    {
      ...
      e_widget_frametable_object_append(of, ob,
					0, 1, 1, 1,
					1, 1, 1, 1);
      ob = e_widget_slider_add(evas, 1, 0, _("%1.0fs"), 3.0, 300.0, 5.0, 0,
			       NULL, &(cfdata->source_picture_timer), 100);
      e_widget_frametable_object_append(of, ob,
					0, 2, 1, 1,
					1, 0, 1, 0);
      e_widget_table_object_append(o, of,
      ...
      if (cfdata->s_rss)
	{
	  ...
	  e_widget_frametable_object_append(of, ob,
					    0, 6, 1, 1,
					    1, 1, 1, 1);
	  ob = e_widget_slider_add(evas, 1, 0, _("%1.0f min"), (float)SOURCE_RSS_UPDATE_RATE_MIN/60, (float)SOURCE_RSS_UPDATE_RATE_MAX/60, 1.0, 0,
				   NULL, &(cfdata->source_rss_timer), 90);
	  e_widget_frametable_object_append(of, ob,
					    1, 6, 2, 1,
					    1, 0, 1, 0);
  ...
  if (!doc->user)
    {
      ...
      DEVIANM->conf->sources_rss_docs =
	evas_list_remove(DEVIANM->conf->sources_rss_docs, doc);
  ...
  else
    {
      ...
      snprintf(buf, sizeof(buf),
	       _(
                 "<hilight>Rss '%s' is in use !</hilight><br><br>"
                 "You can't delete it while its in use.<br><br>"
                 "Owner is %s"
                 ),
	       doc->name, doc->user->source->devian->id
	       );
