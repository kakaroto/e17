...
static void
_cb_monitor(void *data, Ecore_File_Monitor *em,
...
int DEVIANF(source_file_evas_object_get)(DEVIANN *devian,
					    Evas_Object **file0,
					    Evas_Object **file1)
{
  Source_File *source;
...
int DEVIANF(source_file_original_size_get)(Source_File *source,
					   int *w, int *h)
{
  *w = 300;
...
void DEVIANF(source_file_update_change)(DEVIANN *devian, int active, int time)
{
  ...
  if (active && !source->monitor)
    {
      source->monitor = ecore_file_monitor_add(source->devian->conf->file_path,
					       _cb_monitor, source);
