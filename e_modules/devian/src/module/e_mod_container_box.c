...
static void _cb_mouse_down(void *data, Evas* e, Evas_Object* obj,
			   void* event_info);
static void _cb_mouse_in(void *data, Evas* e, Evas_Object* obj,
			 void* event_info);
static void _cb_mouse_out(void *data, Evas* e, Evas_Object* obj,
			  void* event_info);

static void _cb_edje_part_change(void *data, Evas_Object *obj,
				 const char *emission, const char *source);
static void _cb_edje_part_change_start(void *data, Evas_Object *obj,
				       const char *emission, const char *source);
static void _cb_edje_part_change_stop(void *data, Evas_Object *obj,
				      const char *emission, const char *source);
static void _cb_edje_part_change_wanted(void *data, Evas_Object *obj,
				       const char *emission, const char *source);
static void _cb_edje_part_previous_wanted(void *data, Evas_Object *obj,
					  const char *emission, const char *source);
static void _cb_edje_set_bg(void *data, Evas_Object *obj,
			    const char *emission, const char *source);
...
int DEVIANF(container_box_add)(DEVIANN *devian, int edje_part)
{
  ...
  if ( !(box->theme = DEVIANF(container_edje_load)(box->edje, "devian/box",
						   devian, CONTAINER_BOX, box)) )
    {
  ...
  evas_object_event_callback_add(box->edje, EVAS_CALLBACK_MOUSE_DOWN,
				 _cb_mouse_down, box);
  evas_object_event_callback_add(box->edje, EVAS_CALLBACK_MOUSE_IN,
				 _cb_mouse_in, box);
  evas_object_event_callback_add(box->edje, EVAS_CALLBACK_MOUSE_OUT,
				 _cb_mouse_out, box);

  /* Callbacks for controls on edje */
  edje_object_signal_callback_add(box->edje, "pause",
				  "button_playpause",
				  _cb_edje_part_change_stop, box);
  edje_object_signal_callback_add(box->edje, "play",
				  "button_playpause",
				  _cb_edje_part_change_start, box);
  edje_object_signal_callback_add(box->edje, "set_bg",
				  "button_set_bg",
				  _cb_edje_set_bg, box);
  edje_object_signal_callback_add(box->edje, "refresh",
				  "button_refresh",
				  _cb_edje_part_change_wanted, box);
  edje_object_signal_callback_add(box->edje, "previous",
				  "button_previous",
				  _cb_edje_part_previous_wanted, box);
  ...
  box->max_size = devian->conf->box_max_size_source
    + box->theme_border_h;
...
void DEVIANF(container_box_del)(Container_Box *container)
{
  ...
  evas_object_event_callback_del(container->edje, EVAS_CALLBACK_MOUSE_DOWN,
				 _cb_mouse_down);
  evas_object_event_callback_del(container->edje, EVAS_CALLBACK_MOUSE_IN,
				 _cb_mouse_in);
  evas_object_event_callback_del(container->edje, EVAS_CALLBACK_MOUSE_OUT,
				 _cb_mouse_out);

  /* Callbacks for controls */
  edje_object_signal_callback_del(container->edje, "pause",
				  "button_playpause",
				  _cb_edje_part_change_stop);
  edje_object_signal_callback_del(container->edje, "play",
				  "button_playpause",
				  _cb_edje_part_change_start);
  edje_object_signal_callback_del(container->edje, "set",
				  "button_background",
				  _cb_edje_set_bg);
  edje_object_signal_callback_del(container->edje, "refresh",
				  "button_refresh",
				  _cb_edje_part_change_wanted);
  edje_object_signal_callback_del(container->edje, "previous",
				  "button_previous",
				  _cb_edje_part_previous_wanted);
  ...
  if (devian->source)
	...
	action = 1;
      edje_object_message_send(box->edje, EDJE_MESSAGE_INT,
			       DEVIAN_CONTAINER_BOX_TO_EDJE_MSG_PART_CHANGE_SET,
			       &action);
	...
	action = 0;
      edje_object_message_send(box->edje, EDJE_MESSAGE_INT,
			       DEVIAN_CONTAINER_BOX_TO_EDJE_MSG_SHOW_SET_BG, &action);
	...
	action = 0;
      edje_object_message_send(box->edje, EDJE_MESSAGE_INT,
			       DEVIAN_CONTAINER_BOX_TO_EDJE_MSG_SHOW_PREVIOUS, &action);
...
void DEVIANF(container_box_resize_auto)(DEVIANN *devian)
{
  ...
  if (!box->devian->dying)
	...
	{
	  double ratio;
	  if (!DEVIANF(source_original_size_get)(box->devian, box->edje_part,
						 &w_org, &h_org))
	    {
	      printf("Cant get source's size !!!\n");
	      w_org = 1;
	      h_org = 1;
	    }
...
int DEVIANF(container_box_edje_part_change)(Container_Box *container)
{
  ...
  DEVIANF(source_evas_object_get)(container->devian,
				  &source0, &source1);
  ...
  if(!container->edje_part)
      ...
      if (container->gmc)
	{
	  if (container->devian->size_policy == SIZE_POLICY_AUTO)
		...
		e_gadman_client_aspect_set(container->gmc,
					   ((float)w/(float)h),
					   ((float)w/(float)h));
	    }
	  else
      ...
      if (DEVIANM->conf->boxs_nice_trans)
	{
	  ...
	  edje_object_signal_callback_add(container->edje, "source_transition_0-1_end",
					  "th",
					  _cb_edje_part_change, container);
	}
      else
	{
	  edje_object_signal_emit(container->edje, "source_transition_q_0-1_go", "");
	  edje_object_signal_callback_add(container->edje, "source_transition_q_0-1_end",
					  "th",
					  _cb_edje_part_change, container);
	}
      ...
      if (DEVIANM->conf->boxs_nice_trans)
	{
	  ...
	  edje_object_signal_callback_add(container->edje, "source_transition_1-0_end",
					  "th",
					  _cb_edje_part_change, container);
	}
      else
	{
	  ...
	  edje_object_signal_callback_add(container->edje, "source_transition_q_1-0_end",
					  "th",
					  _cb_edje_part_change, container);
	}
...
int DEVIANF(container_box_infos_init)(Container_Box *box)
{
  ...
  evas_textblock_style_set(tb_style,
                           "DEFAULT='font=Vera font_size=10 align=left color=#000000ff wrap=word'"
                           "br='\n'");
...
int DEVIANF(container_box_infos_text_change)(Container_Box *box, char *text)
{
  ...
  evas_object_resize(box->infos_tb, 
		     box->infos_scrollframe_w-10, 20);
  
  evas_object_textblock_text_markup_set(box->infos_tb, text);
  
  evas_object_textblock_size_formatted_get(box->infos_tb, &w, &h);
  evas_object_resize(box->infos_tb,
		     box->infos_scrollframe_w-10, h);
  
  DCONTAINER(("Tb Infos: w:%d h:%d",
	      w, h));
...
void DEVIANF(container_box_infos_text_scroll)(Container_Box *box)
{
  ...
  if ( (tb_h - pos_y) > (whole_h-15) )
    e_scrollframe_child_pos_set(box->infos_scrollframe,
				pos_x, pos_y+(whole_h-50)); /* -> Next page */
  else
    if ( pos_y )
...
void DEVIANF(container_box_infos_text_change_set)(Container_Box *box, int action)
{
  ...
  edje_object_message_send(box->edje, EDJE_MESSAGE_INT,
			   DEVIAN_CONTAINER_BOX_TO_EDJE_MSG_INFOS_CHANGE_SET, &action);
}

void DEVIANF(container_box_infos_pos_set)(Container_Box *box)
{
  edje_object_message_send(box->edje, EDJE_MESSAGE_INT,
			   DEVIAN_CONTAINER_BOX_TO_EDJE_MSG_INFOS_POS_SET,
			   &box->devian->conf->box_infos_pos);
}

void DEVIANF(container_box_loading_state_change)(Container_Box *box, int state)
{
  edje_object_message_send(box->edje, EDJE_MESSAGE_INT,
			   DEVIAN_CONTAINER_BOX_TO_EDJE_MSG_LOADING_STATE,
			   &state);
}
...
int DEVIANF(container_box_animation_start)(Container_Box *box, int anim_num)
{
  ...
  switch (anim_num)
	...
	box->anim->timer = ecore_timer_add(DEVIANM->conf->boxs_anim_ghost_timer,
					   _cb_timer_anim_ghost,
					   box);
	break;
      }
  ...
  if (anim_num)
    {
      if (!box->animator &&
	  (anim_num != CONTAINER_BOX_ANIM_GHOST))
	box->animator = ecore_animator_add(_cb_ecore_animator, box);
...
static int _gadman_init(Container_Box *box)
{
  ...
  e_gadman_client_min_size_set(box->gmc, 
			       CONTAINER_BOX_SIZE_MIN,
			       CONTAINER_BOX_SIZE_MIN);
  e_gadman_client_max_size_set(box->gmc,
			       CONTAINER_BOX_SIZE_MAX,
			       CONTAINER_BOX_SIZE_MAX);
  e_gadman_client_auto_size_set(box->gmc,
				(box->w),
				(box->h));
  ...
  */
  ...
  e_gadman_client_padding_set(box->gmc,
			      10, 10,
			      10, 10);
	  ...
	  if (box->devian->size_policy == SIZE_POLICY_AUTO)
	      ...
	      if (w>h)
		{
		  ...
		  box->devian->conf->box_max_size_source =
		    box->w - box->theme_border_w;
		}
	      else
		{
		  box->max_size = box->h;
		  box->devian->conf->box_max_size_source =
		    box->h - box->theme_border_h;
		}
	    }
	  else
	    {
				 ...
				 const char *emission, const char *source)
{
  ...
  DEVIANF(source_evas_object_get)(devian,
				  &source0, &source1);
  ...
  if(!strcmp(emission, "source_transition_0-1_end"))
      ...
      if (devian->source_info.provide_double_buf)
	{
      ...
      edje_object_signal_callback_del(obj, "source_transition_0-1_end",
				      "th",
				      _cb_edje_part_change);
      container->in_transition = 0;
	...
	{ 
	  ...
	  edje_object_signal_callback_del(obj, "source_transition_q_0-1_end",
					  "th",
					  _cb_edje_part_change);
	  container->in_transition = 0;
	  DEVIANF(source_detach)(devian, 0);
      ...
      else
	{
	  if(!strcmp(emission, "source_transition_1-0_end"))
	      ...
	      if (devian->source_info.provide_double_buf)
		{
	      ...
	      edje_object_signal_callback_del(obj, "source_transition_1-0_end",
					      "th",
					      _cb_edje_part_change);
	      container->in_transition = 0;
	      ...
	      if(!strcmp(emission, "source_transition_q_1-0_end"))
		{
		  ...
		  edje_object_signal_callback_del(obj, "source_transition_q_1-0_end",
						  "th",
						  _cb_edje_part_change);
		  container->in_transition = 0;
		  DEVIANF(source_detach)(devian, 1);
...
static int _cb_ecore_animator(void *data)
{
  ...
  switch (box->devian->conf->box_anim)
	...
	if (box->anim->data[1])
	  {
		...
		if (box->anim->data[3] < 0)
		  box->anim->data[3] = 0;
		evas_object_color_set(box->edje, 255, 255, 255,
				      box->anim->data[3]);
		if (box->anim->data[3] == 0)
		  {
		    box->anim->data[2] = 1;
    ...
    default: box->devian->conf->box_anim = CONTAINER_BOX_ANIM_NO;
    }
  ...
  if (!box->in_resize &&
      (!box->devian->conf->box_anim ||
       (box->devian->conf->box_anim==CONTAINER_BOX_ANIM_GHOST &&
	!box->anim->data[1])))
    {
