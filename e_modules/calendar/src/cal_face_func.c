...
calendar_face_new(E_Container *con,Calendar *calendar)
{
   ...
   check = edje_object_file_set(face->topimage_object, calendar->conf->TopImage_path,
									"default");
    //Make background object
   face->background_object = edje_object_add(con->bg_evas); 
   check = edje_object_file_set(face->background_object, calendar->conf->Background_path,
									"back");
   ...
   if (!calendar->conf->UserCS)
   	check = edje_object_file_set(face->label_object, calendar->conf->label_path,
								"default");
   else
   	check = edje_object_file_set(face->label_object, calendar->conf->label_path,
								"user");
   set_day_label(face->label_object, calendar->conf->DayofWeek_Start, calendar->conf->view_month,
   ...
   for (i;i<35;i++)
   {
	...
	if (!calendar->conf->UserCS)
    	   check = edje_object_file_set(face->weekend_object[i]->obj,
			 		calendar->conf->weekend_path, "default");
	else
    	   check = edje_object_file_set(face->weekend_object[i]->obj,
	...
	if (!calendar->conf->UserCS)
   	   check = edje_object_file_set(face->weekday_object[i]->obj,
 					calendar->conf->weekday_path,"default");
	else
   	   check = edje_object_file_set(face->weekday_object[i]->obj,
...
void calendar_face_free(Calendar_Face *face)
{
   ...
   if (face->con) e_object_unref(E_OBJECT(face->con));
   ...
   if (face->sub) e_object_del(E_OBJECT(face->sub));
   if (face->sub2) e_object_del(E_OBJECT(face->sub2));
   if (face->sub3) e_object_del(E_OBJECT(face->sub3));
   free(face);
}
