...
e_modapi_config(E_Module *m)
{
   ...
   mnt = m->data;
   if (mnt) _config_mount_module(mnt->con, mnt);
   return 1;
}
...
_parse_file(char *fl, Mount_Point *mntpoint)
{
   ...
   *s = 0;
   for (; fgets(s, sizeof(s), f);)
	...
	if (!(*s) || (*s == '\n') || (*s == '#')) continue;
	token = strtok(s, " \t");
	if (token) device = strdup(token);
	token = NULL;
	token = strtok(NULL, " \t");
	if (token) path = strdup(token);
	if (path)
	  {
	     if (!strcmp(mntpoint->path, path)) mounted = 1;
	  }
     }
   fclose(f);
   free(device);
...
_is_mounted(Mount_Point *mntpoint)
{
   ...
   /* Check proc mounts */
   mounted = _parse_file("/proc/mounts", mntpoint);
   if (mounted <= 0)
     {
	/* Check mtab */
	mounted = _parse_file("/etc/mtab", mntpoint);
...
_mount_free(Mount *mnt)
{
   ...
   if (mnt->config_menu_options) e_object_del(E_OBJECT(mnt->config_menu_options));
...
_mount_box_free(Mount_Box *mntbox)
{
   ...
   if (mntbox->bar_object) evas_object_del(mntbox->bar_object);
   if (mntbox->box_object) evas_object_del(mntbox->box_object);
   if (mntbox->event_object) evas_object_del(mntbox->event_object);

   if (mntbox->gmc) e_gadman_client_save(mntbox->gmc);
   if (mntbox->gmc) e_object_del(E_OBJECT(mntbox->gmc));
...
_mount_box_icon_free(Mount_Icon *mnticon)
{
   ...
   if (mnticon->bg_object) evas_object_del(mnticon->bg_object);
   if (mnticon->icon_object) evas_object_del(mnticon->icon_object);
   if (mnticon->overlay_object) evas_object_del(mnticon->overlay_object);
   if (mnticon->txt_object) evas_object_del(mnticon->txt_object);
   free(mnticon);
}
...
#define D mntbox->conf_box_edd
		  ...
		  if (!_mount_box_init(mntbox)) return NULL;
		  ...
		  if (!mntbox->conf->enabled) _mount_box_disable(mntbox);
	       }
	  }
     }
   return mnt;
...
_mount_box_disable(Mount_Box *mntbox)
{
   ...
   if (mntbox->box_object) evas_object_hide(mntbox->box_object);
   if (mntbox->event_object) evas_object_hide(mntbox->event_object);
   e_config_save_queue();
}
...
_mount_box_cb_intercept_move(void *data, Evas_Object *o, Evas_Coord x, Evas_Coord y)
{
   ...
   if (mntbox->bar_object) evas_object_move(mntbox->bar_object, x, y);
   if (mntbox->event_object) evas_object_move(mntbox->event_object, x, y);
   if (mntbox->box_object) evas_object_move(mntbox->box_object, x, y);
}
...
_mount_box_cb_intercept_resize(void *data, Evas_Object *o, Evas_Coord w, Evas_Coord h)
{
   ...
   if (mntbox->bar_object) evas_object_resize(mntbox->bar_object, w, h);
   if (mntbox->event_object) evas_object_resize(mntbox->event_object, w, h);
   if (mntbox->box_object) evas_object_resize(mntbox->box_object, w, h);
}
...
_mount_icon_cb_intercept_move(void *data, Evas_Object *o, Evas_Coord x, Evas_Coord y)
{
   ...
   mnticon = data;
   evas_object_move(o, x, y);
   if (mnticon->bg_object) evas_object_move(mnticon->bg_object, x, y);
   if (mnticon->icon_object) evas_object_move(mnticon->icon_object, x, y);
   if (mnticon->overlay_object) evas_object_move(mnticon->overlay_object, x, y);
   if (mnticon->txt_object) evas_object_move(mnticon->txt_object, x, y);
}
...
_mount_icon_cb_intercept_resize(void *data, Evas_Object *o, Evas_Coord w, Evas_Coord h)
{
   ...
   mnticon = data;
   evas_object_resize(o, w, h);
   if (mnticon->bg_object) evas_object_resize(mnticon->bg_object, w, h);
   if (mnticon->overlay_object) evas_object_resize(mnticon->overlay_object, w, h);
   if (mnticon->icon_object) evas_object_resize(mnticon->icon_object, w, h);
   if (mnticon->txt_object) evas_object_resize(mnticon->txt_object, w, h);
}
...
_mount_mountpoint(Mount_Icon *mnticon)
{
   ...
   if (x) ecore_exe_tag_set(x, "Mount");
}
...
_unmount_mountpoint(Mount_Icon *mnticon)
{
   ...
   x = ecore_exe_run(buf, mnticon);
   if (x) ecore_exe_tag_set(x, "Unmount");
}
...
_eject_mountpoint(Mount_Icon *mnticon)
{
   ...
   x = ecore_exe_run(buf, mnticon);
   if (x) ecore_exe_tag_set(x, "Eject");
}
...
_mount_box_cb_mouse_down(void *data, Evas *e, Evas_Object *obj, void *event_info)
{
   ...
   if (ev->button == 3)
	...
	e_menu_activate_mouse(mntbox->menu, e_zone_current_get(con), ev->output.x, ev->output.y,1,1,E_MENU_POP_DIRECTION_DOWN, ev->timestamp);
