...
static void _volume_config_menu_new(Volume* v)
{
...
static void _face_add_cb(void *data, E_Menu* mn __UNUSED__, E_Menu_Item* mi __UNUSED__)
{
	...
	if (face)
	{
		...
		ldata->con_config->faces = 
			evas_list_append(ldata->con_config->faces, face->conf);
		/* Use automatic size */
...
Evas_List* _get_active_mixers(Mixer_System_Name* msn, Config* conf)
{
	Evas_List *l,*l1,*l3;
	Evas_List *ret;
	...
	{
		Evas_List *mixers;
		for (mixers = msn->mixer_names;
				mixers; mixers = evas_list_next(mixers))
		{
	...
	for(l = conf->containers; l; l = evas_list_next(l))
	{
		...
		for(l1 = con_conf->faces; l1; l1 = evas_list_next(l1))
		{
			...
			for(l2 = face_conf->systems;
					l2; l2 = evas_list_next(l2))
			{
				...
				for(l3 = system_conf->mixers;
						l3; l3 = evas_list_next(l3))
				{
					Config_Mixer* mixer_conf;
					Evas_List *mixers;
					mixer_conf = evas_list_data(l3);
							...
							mixers; mixers = evas_list_next(mixers))
					{
						 ...
						 * passed to mixer_open
						 */
						if(!strcmp(mixer_name->real, mixer_conf->real)
								&& mixer_conf->active)
						{
							DBG(stderr,"%s: adding %s\n", __FUNCTION__,
									mixer_name->card);
...
/* MAIN INIT FUNCTION */
Volume* e_volume_init(void* data __UNUSED__)
{
	Volume *volume;
	...
	for (l = managers; l; l = l->next)
	{
		...
		for (l2 = man->containers; l2; l2 = l2->next)
		{
			...
			if(!cl)
			{
				...
				volume->conf->containers = 
					evas_list_append(volume->conf->containers, con_config);
			}
			else
			{
				con_config = cl->data;
			...
			while(i++ < faces_count)
			{
				...
				if (face)
				{
					...
					if (!face_config_list)
					{
						...
						face->conf->orient = ORIENT_HORIZ;
						con_config->faces = 
							evas_list_append(con_config->faces, face->conf);
						/* Use automatic size */
						face->gmc->use_autoh = 1;
						face->gmc->use_autow = 1;
					}
...
static int _volume_mixer_create(Mixer* mixer, Evas* ev,
		Volume_Face* face, int ref)
{
	...
	for(l = mixer->elems; l; l = evas_list_next(l))
	{
		...
		slider = e_volume_slider_create(melem, ev, face->conf->orient,
				conf->balance, face, ref);
...
int e_volume_mixers_create(Volume_Face* face, Mixer* mixer, Evas* evas, int ref)
{
...
int e_volume_mixers_free(Volume_Face* face, Mixer* mixer, int unref)
{
	while(face->sliders)
	{
		...
		DBG(stderr,"slider->melem->mixer = %p",
				slider->melem->mixer);
...
static int _volume_get_entry_height(Volume_Face* face, Evas* evas)
{
	Evas_Object *obj;
...
int e_volume_shutdown(Volume* volume)
{
	/* Shut down module */
	Evas_List *l;
