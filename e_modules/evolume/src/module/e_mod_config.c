...
Evas_List *_evas_list_copy(Evas_List* src)
{
	Evas_List *l, *ret;
...
//	DBG(stderr,"in start of : %s\n", __FUNCTION__);
	conf_elem_edd = E_CONFIG_DD_NEW("Volume_Config_Elem",
			Config_Mixer_Elem);
#undef T
#undef D
...
#define D conf_mixer_edd
	...
	conf_mixer_system_edd = E_CONFIG_DD_NEW("Volume_Config_Mixer_System",
			Config_Mixer_System);
#undef T
#undef D
...
#define D conf_face_edd
	...
	conf_container_edd = E_CONFIG_DD_NEW("Volume_Config_Container",
			Config_Container);
#undef T
#undef D
...
Config_Mixer_System* e_volume_config_mixer_system_get(
		Mixer_System_Name* msn, Config_Face* conf)
{
	Evas_List *l;
...
Config_Mixer* e_volume_config_mixer_get(Mixer_Name* mixer_name, Config_Face* conf)
{
	Evas_List *l;
...
Config_Mixer_Elem* e_volume_config_melem_get(Mixer_Elem* melem, Config_Face* conf)
{
	...
	if(!l)
	{
		...
		mixer_conf->elems = evas_list_append(mixer_conf->elems,
				elem);
	...
	DBG(stderr,"MIXER CONFIG NOT CREATED FOR %s->%s->%s\n",
			melem->mixer->system->name->system, melem->mixer->name->real,
			melem->name);
	return NULL;
}
