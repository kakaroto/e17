...
static void
_volume_slider_volume_set(Mixer_Slider *slider)
{
   double vol, vol1;
   int left, right;
   Mixer_Elem *melem;

   edje_object_part_drag_value_get(slider->drag_volume, "volume_bar_drag", &vol, &vol1);
   ...
   if (slider->drag_balance)
     {
      ...
      edje_object_part_drag_value_set(slider->drag_volume, "volume_bar_drag", 0.5f, vol1);
...
static void
_volume_slider_level_drag_state_cb(void *data, Evas_Object *object __UNUSED__, const char *emission, const char *source __UNUSED__)
{
   Mixer_Slider *slider;
...
static void
_volume_slider_level_drag_cb(void *data,
                             Evas_Object *object __UNUSED__, const char *emission __UNUSED__, const char *source __UNUSED__)
{
...
static void
_volume_slider_balance_drag_cb(void *data,
                               Evas_Object *object __UNUSED__, const char *emission __UNUSED__, const char *source __UNUSED__)
{
   Mixer_Slider *slider;
   ...
   edje_object_part_drag_value_get(slider->drag_balance, "volume_bar_drag", &bal, NULL);
...
static void
_volume_slider_level_mouse_wheel_cb(void *data, Evas *e __UNUSED__, Evas_Object *obj __UNUSED__, void *event_info)
{
...
static void
_volume_slider_balance_mouse_wheel_cb(void *data __UNUSED__,
                                      Evas *e __UNUSED__, Evas_Object *obj __UNUSED__, void *event_info __UNUSED__)
{
#if 0
   Evas_Event_Mouse_Wheel *ev;
   int l, r;
...
static int
_edje_object_file_set_orient(Evas_Object *obj, const char *file, const char *part, int orient)
{
   char buf[1024];
...
Mixer_Slider *
e_volume_slider_create(Mixer_Elem *melem, Evas *ev, int orient, int balance, Volume_Face *face, int ref)
{
   Mixer_Slider *slider;
   ...
   _edje_object_file_set_orient(slider->name, module_theme, "volume/mixer/name", orient);
   edje_object_part_text_set(slider->name, "mixer_name", melem->name);

   /* Num num, sweet part :-) */
   edje_object_part_swallow(slider->swallow_object, "label", slider->name);

   /* Update volume drag object */
   {
      slider->drag_volume = (Evas_Object *)edje_object_add(ev);
      _edje_object_file_set_orient(slider->drag_volume, module_theme, "volume/mixer/drag", orient);

      edje_object_signal_callback_add(slider->drag_volume, "drag", "volume_bar_drag", _volume_slider_level_drag_cb, (void *)slider);

      edje_object_signal_callback_add(slider->drag_volume, "drag,*",
                                      "volume_bar_drag", _volume_slider_level_drag_state_cb, (void *)slider);

      evas_object_event_callback_add(slider->drag_volume, EVAS_CALLBACK_MOUSE_WHEEL, _volume_slider_level_mouse_wheel_cb, slider);

      evas_object_data_set(slider->drag_volume, "melem", (void *)melem);

      /* Num num, sweet part :-) */
      if (!balance)
         edje_object_part_swallow(slider->swallow_object, "level", slider->drag_volume);
      else
         edje_object_part_swallow(slider->swallow_object, "level_b", slider->drag_volume);
   ...
   if (balance)
     {
        ...
        _edje_object_file_set_orient(slider->drag_balance, module_theme, "volume/mixer/drag", orient);

        edje_object_signal_callback_add(slider->drag_balance, "drag",
                                        "volume_bar_drag", _volume_slider_balance_drag_cb, (void *)slider);

        evas_object_event_callback_add(slider->drag_balance,
                                       EVAS_CALLBACK_MOUSE_WHEEL, _volume_slider_balance_mouse_wheel_cb, slider);

        evas_object_data_set(slider->drag_balance, "melem", (void *)melem);

        /* Num num, sweet part :-) */
        edje_object_part_swallow(slider->swallow_object, "balance", slider->drag_balance);
     }
...
Mixer_Slider *
e_volume_slider_create_from_conf(Config_Mixer_Elem *conf, Volume_Face *face, int ref)
{
   Mixer_Elem *melem;
...
void
e_volume_slider_free(Mixer_Slider *slider, int unref)
{
   if (!slider)
      return;
