...
int e_volume_first_run()
{
	e_module_dialog_show("eVolume",
			...
			"<br>"
			"Thanks for using eVolume, Pavel Boldin aka davinchi<br>"
			);

	return 1;
}

static int _conf_sort_cb(void *d1, void* d2)
{
...
static int _slider_sort_cb(void *d1, void* d2)
{
	Mixer_Slider *s1, *s2;
	Config_Mixer_Elem* c1;
	Config_Mixer_Elem* c2;
...
Evas_List *e_util_sliders_sort(Evas_List* sliders)
{
	...
	ret = evas_list_sort(sliders,
			evas_list_count(sliders),
			_slider_sort_cb );
...
Mixer* e_util_mixer_get(Config_Mixer* mixer_conf, Volume* volume)
{
}
