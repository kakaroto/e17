...
static void _taskbar_cb_empty_mouse_down (void *data, Evas * e,
					  Evas_Object * obj,
					  void *event_info);
...
static void _taskbar_icon_signal_emit (Taskbar_Icon * ic, char *sig,
				       char *src);
static void _taskbar_cb_obj_moveresize (void *data, Evas * e,
					Evas_Object * obj, void *event_info);
static void _taskbar_cb_menu_configure (void *data, E_Menu * m,
					E_Menu_Item * mi);
static void _taskbar_cb_menu_post (void *data, E_Menu * m);
static void _taskbar_cb_icon_mouse_in (void *data, Evas * e,
				       Evas_Object * obj, void *event_info);
static void _taskbar_cb_icon_mouse_out (void *data, Evas * e,
					Evas_Object * obj, void *event_info);
static void _taskbar_cb_icon_mouse_down (void *data, Evas * e,
					 Evas_Object * obj, void *event_info);
static void _taskbar_cb_icon_mouse_up (void *data, Evas * e,
				       Evas_Object * obj, void *event_info);
static void _taskbar_cb_icon_move (void *data, Evas * e, Evas_Object * obj,
				   void *event_info);
static void _taskbar_cb_icon_resize (void *data, Evas * e, Evas_Object * obj,
				     void *event_info);
static int _taskbar_cb_event_border_add (void *data, int type, void *event);
static int _taskbar_cb_event_border_remove (void *data, int type,
					    void *event);
static int _taskbar_cb_event_border_iconify (void *data, int type,
					     void *event);
static int _taskbar_cb_event_border_uniconify (void *data, int type,
					       void *event);
static int _taskbar_cb_event_border_icon_change (void *data, int type,
						 void *event);
static int _taskbar_cb_event_border_zone_set (void *data, int type,
					      void *event);
static int _taskbar_cb_event_border_desk_set (void *data, int type,
					      void *event);
...
static int _taskbar_cb_event_border_property (void *data, int type,
					      void *event);
static int _taskbar_cb_event_desk_show (void *data, int type, void *event);
static int _taskbar_cb_event_border_urgent_change (void *data, int type,
						   void *event);
...
_gc_init (E_Gadcon * gc, const char *name, const char *id, const char *style)
{
  ...
  inst->ci = _taskbar_config_item_get (id);
  if (!inst->ci->id)
  ...
  evas_object_event_callback_add (o, EVAS_CALLBACK_RESIZE,
				  _taskbar_cb_obj_moveresize, inst);
  taskbar_config->instances =
    evas_list_append (taskbar_config->instances, inst);

  // Fill on initial config
  _taskbar_config_updated (inst->ci);
...
_gc_shutdown (E_Gadcon_Client * gcc)
{
  ...
  taskbar_config->instances =
    evas_list_remove (taskbar_config->instances, inst);
...
_gc_orient (E_Gadcon_Client * gcc)
{
  ...
  switch (gcc->gadcon->orient)
    ...
    case E_GADCON_ORIENT_CORNER_BR:
      ...
      e_gadcon_client_aspect_set (gcc, inst->taskbar->gccw,
				  inst->taskbar->gcch);
      break;
...
_taskbar_new (Evas * evas, E_Zone * zone)
{
  ...
  snprintf (buf, sizeof (buf), "%s/taskbar.edj",
	    e_module_dir_get (taskbar_config->module));
  if (!e_theme_edje_object_set
      (b->o_gccbox, "base/theme/modules/taskbar", "modules/taskbar/box"))
...
static void
_taskbar_cb_empty_mouse_down (void *data, Evas * e, Evas_Object * obj,
			      void *event_info)
{
  ...
  if (!b->borders)
      ...
      if (!b->o_empty)
	{
	  ...
	  evas_object_event_callback_add (b->o_empty,
					  ...
					  _taskbar_cb_empty_mouse_down, b);
  ...
  while ((bd = e_container_border_list_next (bl)))
	...
	continue;
      if ((((bd->desk == e_desk_current_get (b->zone)) || (bd->sticky))
	   && (bd->zone == b->zone)) || (b->inst->ci->show_all))
	{
...
_taskbar_icon_new (Taskbar * b, E_Border * bd)
{
  ...
  snprintf (buf, sizeof (buf), "%s/taskbar.edj",
	    e_module_dir_get (taskbar_config->module));
  if (!e_theme_edje_object_set
      (ic->o_holder, "base/theme/modules/taskbar", "modules/taskbar/icon"))
...
{
  ic->o_icon =
    e_border_icon_add (ic->border, evas_object_evas_get (ic->taskbar->o_box));
  ...
  if (ic->o_holder)
    {
      ...
      edje_object_message_signal_process (ic->o_holder);
    }
  if (ic->o_icon)
    {
      edje_object_signal_emit (ic->o_icon, sig, src);
      edje_object_message_signal_process (ic->o_icon);
    }
  if (ic->o_holder2)
    {
      edje_object_signal_emit (ic->o_holder2, sig, src);
      edje_object_message_signal_process (ic->o_holder2);
    }
  if (ic->o_icon2)
    {
      edje_object_signal_emit (ic->o_icon2, sig, src);
      edje_object_message_signal_process (ic->o_icon2);
    }
}

static void
_taskbar_cb_obj_moveresize (void *data, Evas * e, Evas_Object * obj,
			    void *event_info)
{
  Instance *inst;
...
_taskbar_cb_menu_configure (void *data, E_Menu * m, E_Menu_Item * mi)
{
  ...
  b = (Taskbar *) data;
  _config_taskbar_module (b->inst->ci);
}
...
static void
_taskbar_cb_icon_mouse_in (void *data, Evas * e, Evas_Object * obj,
			   void *event_info)
{
  Taskbar_Icon *ic;

  ic = (Taskbar_Icon *) data;
  if (ic->taskbar->inst->ci->highlight)
  ...
  edje_object_part_text_set (ic->taskbar->o_gccbox, "active_label",
			     ic->label);
  if (ic->taskbar->inst->ci->show_label)
    _taskbar_icon_signal_emit (ic, "label_active", "");
}

static void
_taskbar_cb_icon_mouse_out (void *data, Evas * e, Evas_Object * obj,
			    void *event_info)
{
  Taskbar_Icon *ic;
...
static void
_taskbar_cb_icon_mouse_down (void *data, Evas * e, Evas_Object * obj,
			     void *event_info)
{
  ...
  if ((ev->button == 3) && (!taskbar_config->menu))
    {
      ...
      e_gadcon_canvas_zone_geometry_get (ic->taskbar->inst->gcc->gadcon, &cx,
					 &cy, &cw, &ch);
      if (evas_key_modifier_is_set (ev->modifiers, "Alt"))
	{
	  ...
	  mn = e_menu_new ();
	  e_menu_post_deactivate_callback_set (mn, _taskbar_cb_menu_post,
					       NULL);
	  taskbar_config->menu = mn;
	  ...
	  e_gadcon_client_util_menu_items_append (ic->taskbar->inst->gcc, mn,
						  0);
	...
	{
	  e_int_border_menu_show (ic->border, cx + ev->output.x,
				  cy + ev->output.y, 0, ev->timestamp);

	}

      evas_event_feed_mouse_up (ic->taskbar->inst->gcc->gadcon->evas,
				ev->button, EVAS_BUTTON_NONE, ev->timestamp,
				NULL);
...
static void
_taskbar_cb_icon_mouse_up (void *data, Evas * e, Evas_Object * obj,
			   void *event_info)
{
...
static void
_taskbar_cb_icon_move (void *data, Evas * e, Evas_Object * obj,
		       void *event_info)
{
  Taskbar_Icon *ic;
  Evas_Coord x, y;
...
static void
_taskbar_cb_icon_resize (void *data, Evas * e, Evas_Object * obj,
			 void *event_info)
{
  Taskbar_Icon *ic;
  Evas_Coord w, h;
  ...
  for (l = taskbar_config->instances; l; l = l->next)
    {
      ...
      inst = l->data;
      if ((inst->taskbar->zone == ev->border->zone) || inst->ci->show_all)
	{
	  _taskbar_icon_check_add (inst->taskbar, ev->border);
	}
      else
  ...
  for (l = taskbar_config->instances; l; l = l->next)
    {
	  ...
	  || (ev->border->sticky) || (inst->ci->show_all))
	{
	  _taskbar_icon_check_add (inst->taskbar, ev->border);
	}
      else
  ...
  for (l = taskbar_config->instances; l; l = l->next)
    {
      ...
      inst = l->data;
      if ((inst->taskbar->zone == ev->desk->zone) && !(inst->ci->show_all))
	{
	  _taskbar_empty (inst->taskbar);
	  _taskbar_fill (inst->taskbar);
	  _gc_orient (inst->gcc);
...
#define D conf_edd
  ...
  taskbar_config->handlers =
    evas_list_append (taskbar_config->handlers,
		      ecore_event_handler_add (E_EVENT_BORDER_DESK_SET,
					       _taskbar_cb_event_border_desk_set,
					       NULL));
  taskbar_config->handlers =
    evas_list_append (taskbar_config->handlers,
		      ecore_event_handler_add (E_EVENT_BORDER_ZONE_SET,
					       _taskbar_cb_event_border_zone_set,
					       NULL));
  taskbar_config->handlers =
    evas_list_append (taskbar_config->handlers,
		      ecore_event_handler_add (E_EVENT_BORDER_FOCUS_IN,
					       _taskbar_cb_window_focus_in,
					       NULL));
  taskbar_config->handlers =
    evas_list_append (taskbar_config->handlers,
		      ecore_event_handler_add (E_EVENT_BORDER_FOCUS_OUT,
					       _taskbar_cb_window_focus_out,
					       NULL));
  taskbar_config->handlers =
    evas_list_append (taskbar_config->handlers,
		      ecore_event_handler_add (E_EVENT_BORDER_PROPERTY,
					       _taskbar_cb_event_border_property,
					       NULL));
  taskbar_config->handlers =
    evas_list_append (taskbar_config->handlers,
		      ecore_event_handler_add (E_EVENT_DESK_SHOW,
					       _taskbar_cb_event_desk_show,
					       NULL));
  taskbar_config->handlers =
    evas_list_append (taskbar_config->handlers,
		      ecore_event_handler_add (E_EVENT_BORDER_URGENT_CHANGE,
					       _taskbar_cb_event_border_urgent_change,
					       NULL));
  ...
  while (taskbar_config->handlers)
    {
      ...
      taskbar_config->handlers =
	evas_list_remove_list (taskbar_config->handlers,
			       taskbar_config->handlers);
    }
  if (taskbar_config->menu)
    {
