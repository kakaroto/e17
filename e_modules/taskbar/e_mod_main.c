...
_taskbar_icon_new(Taskbar *b, E_Border *bd)
{
   ...
   if (!e_theme_edje_object_set(ic->o_holder, "base/theme/modules/taskbar",
			   "modules/taskbar/icon"))
   ...
   if (!e_theme_edje_object_set(ic->o_holder2, "base/theme/modules/taskbar",
...
_taskbar_cb_icon_mouse_down(void *data, Evas *e, Evas_Object *obj, void *event_info)
{
   ...
   if ((ev->button == 3) && (!taskbar_config->menu))
	...
	evas_event_feed_mouse_up(ic->taskbar->inst->gcc->gadcon->evas, ev->button,
				 EVAS_BUTTON_NONE, ev->timestamp, NULL);
	...
	if (((dx * dx) + (dy * dy)) >
	    (e_config->drag_resist * e_config->drag_resist))
	  {
	     ...
	     d = e_drag_new(ic->taskbar->inst->gcc->gadcon->zone->container,
			    x, y, drag_types, 1,
			    ic->border, -1, _taskbar_cb_drag_finished);
...
_taskbar_inst_cb_enter(void *data, const char *type, void *event_info)
{
   ...
   e_theme_edje_object_set(o, "base/theme/modules/taskbar",
			   "modules/taskbar/drop");
   e_theme_edje_object_set(o2, "base/theme/modules/taskbar",
			   "modules/taskbar/drop_overlay");
   ...
   else e_box_pack_end(inst->taskbar->o_box, o);
...
_taskbar_inst_cb_move(void *data, const char *type, void *event_info)
{
   ...
   else e_box_pack_end(inst->taskbar->o_box, inst->taskbar->o_drop);
   e_box_pack_options_set(inst->taskbar->o_drop,
			  1, 1, /* fill */
			  0, 0, /* expand */
			  0.5, 0.5, /* align */
			  1, 1, /* min */
...
_taskbar_inst_cb_drop(void *data, const char *type, void *event_info)
{
   ...
   if (ic2)
	...
	if (!ic2) goto atend;
	b = inst->taskbar;
	if (_taskbar_icon_find(b, bd)) return;
	ic = _taskbar_icon_new(b, bd);
	if (!ic) return;
